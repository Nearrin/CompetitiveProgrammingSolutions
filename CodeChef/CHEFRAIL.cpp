...
namespace divisor {
template <class T> auto divisor(vector<pair<T, int>> a, ll n) {
    vector<int> r;
    vector<ll> suffix(a.size() + 1, 1);
    rv(i, a) {
        ll tmp = qow(a[i].first, a[i].second);
        suffix[i] = tmp;
        suffix[i] *= suffix[i + 1];
    }
    function<void(int, T)> dfs = [&n, &suffix, &dfs, &a, &r](int i, T now) {
        if (now > 100000)
            return;
        if (n / (now * suffix[i]) > 100000)
            return;
        if (i == int(a.size())) {
            r.push_back(now);
            return;
...
vi mydiv[100010];
ll check(const vi &x, const vi &y) {
    ...
    elif (v > 0) xr.push_back(v);
    vi inxl(max(xl) + 1), inxr(max(xr) + 1);
    for (auto v : xl)
        inxl[v] = 1;
    for (auto v : xr)
        inxr[v] = 1;
    for (auto v : y) {
        ...
        if (!tmp)
            continue;
        for (auto d : mydiv[abs(v)]) {
            ll d2 = tmp / d;
            if (d < inxl.size() && inxl[d] && d2 < inxr.size() && inxr[d2])
                ++ans;
        }
    }
    return ans;
...
int main() {
    fast, prec(12);
    lp(i, 1, 100000) mydiv[i] = divisor::divisor(
        integer_factorization_3::shankss_square_forms_factorization(ll(i) * i),
        ll(i) * i);
    // int cnt = 0;
    // lp(i, 1, 100000) cnt += mydiv[i].size();
    // ou(cnt);
    many {
        int n, m;
        cin >> n >> m;
