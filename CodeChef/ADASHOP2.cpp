...
int main() {
    ...
    many {
        ...
        vpii ans;
        int t = (r - c) / 2;
        ans.emplace_back(r - t, c + t);
        ans.emplace_back(1, 1);
        lp(i, 1, 8) {
            ans.emplace_back(i, i);
            if (2 * i - 1 <= 8)
                ans.emplace_back(1, 2 * i - 1), ans.emplace_back(2 * i - 1, 1);
            else {
                ans.emplace_back(2 * i - 8, 8), ans.emplace_back(8, 2 * i - 8);
            }
            ans.emplace_back(i, i);
        }
        vpii ans2;
        for (auto t : ans)
            if (ans2.empty() || ans2.back() != t)
                ans2.push_back(t);
        vvi vis(9, vi(9));
        vis[r][c] = 1;
        ans = ans2;
        ou(ans.size());
        for (auto tmp : ans)
            ou(tmp.first, tmp.second), vis[tmp.first][tmp.second] = 1;
        /*lp(i, 1, 8) {
            lp(j, 1, 8) cout << vis[i][j] << " ";
            cout << endl;
        }*/
    }
    return 0;
