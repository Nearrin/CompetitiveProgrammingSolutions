...
int main() {
    ...
    many {
        ll n, k;
        vector<ll> a, min_give, max_give, min_get;
        cin >> n >> k;
        a.resize(n);
        min_get.resize(n);
        max_give.resize(n);
        ...
        lv(i, a) {
            ...
            min_get[i] = (k - a[i] % k) % k;
            if (i)
                min_give[i] += min_give[i - 1], max_give[i] += max_give[i - 1];
        }
        rv(i, a) {
            if (i + 1 < a.size())
                min_get[i] += min_get[i + 1];
        }
        ll ans = inf<ll>();
        for (int i = 0; i <= a.size(); ++i) {
            ll mig, mxg;
            if (i)
                mig = min_give[i - 1], mxg = max_give[i - 1];
            else
                mig = 0, mxg = 0;
            ll mir;
            if (i != a.size())
                ...
                mir = 0;
            ll t1_max = (mxg - mig) / k;
            // give =mig+t1*k   t1 in [0,t1_max]
            // take= mir+t2*k t2 in [0,inf)
            // remain=mig-mir+ t3*k t3 in (-inf,t1_max]
            if (mig - mir + k * t1_max < 0) {
            } else if (mig - mir >= 0) {
                tmin(ans, (mig - mir) % k);
            } else {
                ll tmp = mir - mig;
