...
inte inv_f[10010], g[10010], h[10010];
inte lagrange_interpolation_fast(const vector<inte> &y, inte x) {
    g[0] = h[0] = x;
    for (int i = 1; i < int(y.size()); ++i) {
        g[i].v = x.v - i;
        if (g[i].v < 0)
    ...
    inte r = 0;
    for (int i = 0; i < int(y.size()); ++i) {
        inte t = inv_f[i] * inv_f[y.size() - 1 - i];
        if (i)
            t *= g[i - 1];
        if (i + 1 != int(y.size()))
            t *= h[i + 1];
...
int main() {
    fast, prec(12);
    inv_f[0] = 1;
    lp(i, 1, 10009) inv_f[i] = inv_f[i - 1] / i;
    many {
        ll n, k;
        cin >> n >> k;
        vector<inte> y;
        lp(i, 0, k + 2 - 1) {
            y.push_back(inte(i + 1) ^ k);
            if (y.size() > 1)
                y.back() += y[y.size() - 2];
        }
        auto f = [&y](inte x) { return lagrange_interpolation_fast(y, x); };
        inte ans = 0;
        for (auto tmp : divisor::divisor_with_mobius_function(
                 integer_factorization_2::pollards_rho_algorithm(n))) {
            auto d = tmp.first, m = tmp.second;
            auto td = inte(d % inte::p);
            ans += m * (td ^ k) * f((n / d - 1) % inte::p);
        }
        cout << ans << endl;
    }
    return 0;
