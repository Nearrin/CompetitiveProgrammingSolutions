...
int main() {
    ...
    many {
        vpii a;
        in(a);
        auto prefix = [](const vpii &a) {
            vpii prefix_second(a.size());
            lv(i, a) {
                if (i == 0)
                    prefix_second[i] = {a[i].second, a[i].second};
                else {
                    prefix_second[i] = prefix_second[i - 1];
                    tmin(prefix_second[i].first, a[i].second);
                    tmax(prefix_second[i].second, a[i].second);
                }
            }
            return prefix_second;
        };
        auto solve = [&](vpii &a) {
            sort(a);
            auto p = prefix(a);
            ...
            reve(a);
            ll ans = inf<ll>();
            auto diff = [](pii b) -> ll { return b.second - b.first; };
            lv(i, a) {
                ll t = 0;
                if (i)
                    t += diff(p[i - 1]) * (a[i - 1].first - a[0].first);
                t += diff(s[i]) * (a.back().first - a[i].first);
                tmin(ans, t);
            }
            return ans;
        };
