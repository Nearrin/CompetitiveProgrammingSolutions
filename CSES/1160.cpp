...
int main() {
    ...
    in(n, q);
    vi p(n + 1), degree(n + 1);
    using data = pair<int, unordered_map<int, int> *>;
    auto merge = [&](data a, data b) {
        if (a.second->size() < b.second->size())
            swap(a, b);
        for (auto [u, d] : *b.second)
            a.second->insert({u, d + b.first - a.first});
        return a;
    };
    vector<data> s(n + 1);
    lp(u, 1, n) {
        in(p[u]);
        ++degree[p[u]];
        s[u].second = new unordered_map<int, int>;
        s[u].second->insert({u, 0});
    }
    queue<int> c;
    lp(u, 1, n) if (!degree[u]) c.push(u);
    ...
    while (c.size()) {
        ...
        for (auto [u, i] : query[v]) {
            auto t = s[v].second->find(u);
            if (t != s[v].second->end())
                answer[i] = t->second + s[v].first;
        }
        ++s[v].first;
        s[p[v]] = merge(s[p[v]], s[v]);
        if (!--degree[p[v]])
            c.push(p[v]);
        p[v] = 0;
    }
    ...
    lp(u, 1, n) if (p[u]) {
        ...
        lv(i, a) {
            int v = a[i];
            p[v] = 0;
            for (auto [w, d] : *s[v].second)
                index[w] = {d + s[v].first, i};
        }
        lv(i, a) {
            int v = a[i];
            for (auto [w, j] : query[v])
                if (index[w].first != -1)
                    answer[j] = index[w].first + i +
                                (i < index[w].second) * a.size() -
                                index[w].second;
        }
        lv(i, a) {
            int v = a[i];
