...
int main() {
    ...
    many {
        ...
        ll state = 0;
        ll dest = 0;
        lp(i, 1, s) lp(j, 1, r) state = (state << 3) + j - 1;
        lp(j, 1, r) lp(i, 1, s) dest = (dest << 3) + j - 1;
        queue<pli> q;
        unordered_map<ll, pair<ll, pii>> v;
        q.push(make_pair(state, 0));
        v[state] = pair<ll, pii>();
        int ans;
        while (true) {
            auto tmp = q.front();
            // print_state(tmp.first, r * s);
            // getchar();
            q.pop();
            if (tmp.first == dest) {
                ans = tmp.second;
            ...
            lp(i, 1, s * r - 1) lp(j, i + 1, s * r) {
                ll a = tmp.first >> (3 * (s * r - i));
                ll b = tmp.first >> (3 * (s * r - j));
                b -= a << (3 * (j - i));
                int la = 3 * i;
                int lb = 3 * (j - i);
                int lc = 3 * s * r - la - lb;
                ll c = tmp.first & ((1ll << lc) - 1);
                ll u = (b << (lc + la)) + (a << lc) + c;
                if (!v.count(u)) {
                    q.push(make_pair(u, tmp.second + 1));
                    v[u] = {tmp.first, {la / 3, lb / 3}};
                }
            }
        }
        vpii ttt;
        while (dest != state) {
            auto t = v[dest];
            ttt.push_back(t.second);
            // print_state(dest, r * s);
            dest = t.first;
        }
        ou(goog, ans);
        reve(ttt);
