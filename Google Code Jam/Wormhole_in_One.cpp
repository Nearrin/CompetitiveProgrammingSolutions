...
int gcd2(int a, int b) { return b ? gcd2(b, a % b) : a; }
pii direction(pii a) {
    if (a.first == 0) {
        return {0, 1};
    }
    ...
    int d = gcd2(a.first, abs(a.second));
...
int main() {
    ...
    many {
        vpii a;
        in(a);
        unordered_map<pii, unordered_set<int>> mp;
        int ans = 1;
        lp(i, 0, a.size() - 1) {
            lp(j, i + 1, a.size() - 1) {
                mp[direction(a[i] - a[j])].insert(i);
                mp[direction(a[i] - a[j])].insert(j);
            }
        }
        for (auto tmp : mp) {
            auto d = tmp.first;
            int sm = tmp.second.size();
            if (tmax(ans, sm + min(2 - sm % 2, (int)a.size() - sm))) {
                // ou(d);
                // ou(vis);
            }
        }
