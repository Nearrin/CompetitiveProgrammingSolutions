...
int main() {
    ...
    many {
        ...
        lp(i, 0, n - 1) lp(j, 0, m - 1) {
            candidates.emplace_back(i, j), allcost += a[i][j];
            l[i][j] = make(i, j - 1);
            r[i][j] = make(i, j + 1);
            u[i][j] = make(i - 1, j);
            d[i][j] = make(i + 1, j);
        ...
        ll ans = 0;
        auto A = [&](pii p) { return a[p.first][p.second]; };
        auto L = [&](pii p) -> pii & { return l[p.first][p.second]; };
        auto R = [&](pii p) -> pii & { return r[p.first][p.second]; };
        auto U = [&](pii p) -> pii & { return u[p.first][p.second]; };
        auto D = [&](pii p) -> pii & { return d[p.first][p.second]; };
        auto vis = vect<int>({}, n, m);
        auto del = vect<int>({}, n, m);
        ans += allcost;
        while (candidates.size()) {
            ...
            for (auto xy : candidates) {
                ...
                if (a[x][y] * tc < ts)
                    confirmed.emplace_back(x, y);
            }
            debu(candidates, confirmed);
            candidates.clear();
            for (auto xy : confirmed) {
                int x = xy.first, y = xy.second;
                if (l[x][y] != pii{-1, -1})
                    R(l[x][y]) = r[x][y];
                if (r[x][y] != pii{-1, -1})
