...
int main() {
    ...
    many {
        ...
        if (b < 0)
            in(b);
        vi jump(b, -1), move(b, -1), val(b, -1);
        int diff_pos = -1;
        for (int cur = 0; cur < b / 2;) {
            int cnt = 10;
            if (diff_pos != -1) {
                int t1 = query(diff_pos), t2 = query(diff_pos + 1);
                cnt -= 2;
                if ((t1 ^ t2) != move[diff_pos])
                    reve(move);
                val[diff_pos] = t1;
                rp(j, diff_pos - 1, 0) val[j] = val[j + 1] ^ move[j];
                lp(j, diff_pos + 1, cur - 1) val[j] = val[j - 1] ^ move[j - 1];
                lp(j, 0, cur - 1) val[b - j - 1] = val[j] ^ jump[j];
            }
            rp(i, min(cnt / 2, b / 2 - cur), 1) {
                int t1 = query(cur), t2 = query(b - cur - 1);
                jump[cur] = jump[b - cur - 1] = t1 ^ t2;
                val[cur] = t1, val[b - cur - 1] = t2;
                if (cur) {
                    move[cur - 1] = val[cur - 1] ^ val[cur];
                    move[b - cur - 1 + 1] =
                        val[b - cur - 1 + 1] ^ val[b - cur - 1];
                    if (move[cur - 1] != move[b - cur - 1 + 1])
                        diff_pos = cur - 1;
                }
                ++cur;
            }
            str ans;
            lp(i, 0, b - 1) ans.push_back((val[i] < 0 ? 2 : val[i]) + '0');
            cerr << ans << endl;
            if (diff_pos == -1)
                diff_pos = 0;
            cerr << "diff_pos " << diff_pos + 1 << endl;
        }
        str ans;
        lp(i, 0, b - 1) ans.push_back(val[i] + '0');
        ou(ans), cout.flush();
        str res;
        in(res);
        if (res != "Y")
