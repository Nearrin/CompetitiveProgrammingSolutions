...
int main() {
    ...
    many {
        ...
        in(n, m);
        vi a(n);
        map<int, vi> alpha, beta;
        lp(i, 1, n - 1) {
            in(a[i]);
            if (a[i] < 0) {
                alpha[-a[i]].push_back(i);
            } else {
                beta[a[i]].push_back(i);
            }
        }
        vvi e(n);
        vpii edges;
        ...
        while (alpha.size() || beta.size()) {
            if (!alpha.size() || alpha.begin()->first > donecnt) {
                auto batch = beta.begin()->second;
                ...
                for (auto u : batch) {
                    for (auto v : e[u]) {
                        if (dis[v] != inf()) {
                            ans[{u, v}] = ans[{v, u}] = max(a[u] - dis[v], 1);
                        }
                    }
                }
                for (auto u : batch)
                ...
                auto batch = alpha.begin()->second;
                alpha.erase(alpha.begin());
                int thisd = inf();
                for (auto u : batch) {
                    for (int v : e[u]) {
                        if (dis[v] != inf()) {
                            thisd = min(thisd, dis[v] + 1);
                        }
                    }
                }
                thisd = max(thisd, mxd + 1);
                for (auto u : batch)
                    dis[u] = thisd;
                for (auto u : batch) {
                    for (auto v : e[u]) {
                        if (dis[v] != inf()) {
                            ans[{u, v}] = ans[{v, u}] = max(dis[u] - dis[v], 1);
                        }
                    }
                }
                for (auto u : batch)
