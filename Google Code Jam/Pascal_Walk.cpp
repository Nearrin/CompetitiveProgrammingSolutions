...
int main() {
    ...
    many {
        ...
        in(n);
        int d = 0;
        while (path(n).front().first - 1 > d)
            --n, ++d;
        auto ans2 = path(n);
        // ou(ans2, n, d);
        deque<pii> ans(begin(ans2), end(ans2));
        while (ans.front().first != 1) {
            --d;
            ans.push_front(
                {ans.front().first - 1, max(1, ans.front().second - 1)});
        }
        while (d)
            ans.push_back({ans.back().first + 1, ans.back().second}), --d;
        ou(goog);
        // debu(ans.size());
        for (auto t : ans)
            ou(t);
