...
int main() {
    ...
    many {
        ...
        uniq(x);
        set<pii> segments;
        lv(i, x) if (i) segments.insert({x[i - 1], x[i]});
        map<pii, int> covered;
        ll cur_peri = 0, ans = 1;
        lv(i, l) {
            while (true) {
                auto it = segments.lower_bound({l[i], -inf()});
                if (it == segments.end() || it->first >= l[i] + w[i])
                    break;
                int left = it->first, right = it->second, height = h[i];
                // ou(left, right, height);
                segments.erase(it);
                ...
                // ou(touchleft, touchright, cur_peri);
            }
