...
struct Seg {
    Seg(int a, int b, int c) : left(a), right(b), hei(c) {
        wei = right - left;
    };
    int left, right, hei, wei;
};
ostream &operator<<(ostream &s, const Seg &a) {
    cout << "(" << a.left << "," << a.right << "," << a.hei << ")";
    return s;
}
int main() {
    ...
    many {
        ...
        lv(i, l) {
            // ou(i, "start");
            if (segments.empty() || segments.back().right <= l[i]) {
                tmp.push_back({l[i], l[i] + w, h[i]});
            } else {
                tmp.push_back({segments.back().right, l[i] + w, h[i]});
                while (segments.size() && segments.back().left >= l[i]) {
                    tmp.push_back(segments.back());
                    tmax(tmp.back().hei, h[i]);
                    segments.pop_back();
                    segans.pop_back();
                }
                if (tmp.back().left > l[i]) {
                    if (segments.empty() || segments.back().right <= l[i]) {
                        tmp.push_back({l[i], tmp.back().left, h[i]});
                    } else {
                        tmp.push_back({l[i], segments.back().right,
                                       max(h[i], segments.back().hei)});
                        tmp.push_back(segments.back());
                        tmp.back().right = l[i];
                        tmp.back().wei = tmp.back().right - tmp.back().left;
                        segments.pop_back();
                        segans.pop_back();
                    }
                }
            ...
            reve(tmp);
            for (auto t : tmp) {
                if (tmp2.empty() || tmp2.back().right == t.left) {
                    tmp2.push_back(t);
                } else {
                    tmp2.push_back({tmp2.back().right, t.left, h[i]});
                    tmp2.push_back(t);
                }
            }
            swap(tmp, tmp2);
            ...
            while (tmp.size()) {
                if (segments.empty() ||
                    segments.back().right < tmp.back().left) {
                    segments.push_back(tmp.back());
                    tmp.pop_back();
                    segans.push_back(2ll * segments.back().wei +
                                     segments.back().hei * 2ll +
                                     (segans.size() ? segans.back() : 0));
                    segans.back() %= int(1e9 + 7);
                }
                elif (segments.back().hei >= tmp.back().hei) {
                    segments.push_back(tmp.back());
                    // ou(segments.back(), segments.back().wei,
                    //  segments.back().hei);
                    tmp.pop_back();
                    segans.push_back(2ll * segments.back().wei +
                                     (segans.size() ? segans.back() : 0));
            ...
            ansp = ansp * segans.back() % int(1e9 + 7);
            // ou(i, "end", segans.back());
        }
        ou(goog, ansp);
    }
    return 0;
