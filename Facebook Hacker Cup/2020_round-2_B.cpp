...
int main() {
    ...
    many {
        ...
        double p;
        in(n, p);
        auto choose = [](int x) { return x * (x - 1) / 2; };
        auto f = lru_cache::dense(n, n)([&](auto self, int i, int j) -> double {
            if (i < 0 || j < 0)
                return 0.;
            if (i == 0 && j == 0)
                return 0.;
            double t = 1;
            auto tmp = choose(i + j + 1);
            t += choose(i) * 1. / tmp * self(i - 1, j);
            t += choose(j) * 1. / tmp * self(i, j - 1);
            t += i * j * 1. / tmp * p * self(i - 1, j);
            t += i * j * 1. / tmp * (1 - p) * self(i, j - 1);
            t += i * 1. / tmp * p * self(i - 1, j);
            t += j * 1. / tmp * (1 - p) * self(i, j - 1);
            return t;
        });
        ou(goog);
        lp(i, 0, n - 1) ou(f(i, n - i - 1));
    }
    return 0;
}
