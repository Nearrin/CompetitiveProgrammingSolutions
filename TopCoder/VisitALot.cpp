...
struct VisitALot {
    pair<int, string> walk(int R, int C, vector<int> &obsr, vector<int> &obsc) {
        auto vis = vector<vector<int>>(R, vi(C));
        str ans;
        lv(i, obsr) vis[obsr[i]][obsc[i]] = 1;
        vector<int> cands;
        function<void(int, int)> dfs = [&](int i, int j) {
            ...
            if (cands.empty())
                return;
            auto t = cands[rint(0, (int)cands.size() - 1)];
            ans.push_back(t);
            if (t == 'S')
                ++i;
            if (t == 'N')
        ...
        dfs(0, 0);
        return make_pair((int)ans.size(), ans);
    }
    string travel(int R, int C, vector<int> obsr, vector<int> obsc) {
        pair<int, str> ans(-1, "");
        lp(i, 1, 2500) { tmax(ans, walk(R, C, obsr, obsc)); }
        return ans.second;
    }
};
int main_() {
