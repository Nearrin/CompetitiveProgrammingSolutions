...
ll count(ll s,ll d,ll n){
    ll ans=0;
    for(ll i=0;i<=n;++i){
        int len=conv<ll,str>(s+d*i).size();
        ll l=i,r=n+1;
        while(l+1<r){
            ll m=(l+r)/2;
            if(conv<ll,str>(s+d*m).size()==len){
                l=m;
            }else{
                r=m;
            }
...
struct FixedNumberOfDigits{
    long long sum(int start, int step, long long numberOfDigits){
        ll l=0,r=1;
        if(conv<ll,str>(start).size()>=numberOfDigits){
            return conv<str,ll>(conv<ll,str>(start).substr(0,numberOfDigits));
        }
        while(count(start,step,r)<numberOfDigits){
            r*=2;
        }
        while(l+1<r){
            ll m=(l+r)/2;
            if(count(start,step,m)<numberOfDigits){
                l=m;
            }else r=m;
        }
        return conv<str,ll>(conv<ll,str>(start+step*r).substr(0,numberOfDigits-count(start,step,l)));
    }
};
int main_() {
