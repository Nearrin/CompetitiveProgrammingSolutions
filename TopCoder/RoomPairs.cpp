...
struct RoomPairs {
    vector<string> construct(int n, int m, int k) {
        vector<str> a(2 * n + 1, str(2 * m + 1, ' '));
        lp(i, 0, n) lp(j, 0, m) { a[2 * i][2 * j] = '+'; }
        lp(j, 0, m - 1) a[0][2 * j + 1] = a[2 * n][2 * j + 1] = '-';
        lp(i, 0, n - 1) a[2 * i + 1][0] = a[2 * i + 1][2 * m] = '|';
        if (k <= n - 1) {
            lp(i, 0, k - 1) { lp(j, 0, m - 1) a[2 * i + 2][2 * j + 1] = '-'; }
            return a;
        }
        if (k <= m - 1) {
            lp(j, 0, k - 1) { lp(i, 0, n - 1) a[2 * i + 1][2 * j + 2] = '|'; }
            return a;
        }
        if (-k + (n - 1) * m + n * (m - 1) <= n - 1) {
            lp(i, 0, n - 2) { lp(j, 0, m - 1) a[2 * i + 2][2 * j + 1] = '-'; }
            lp(j, 0, m - 2) { lp(i, 0, n - 1) a[2 * i + 1][2 * j + 2] = '|'; }
            lp(i, 0, -k + (n - 1) * m + n * (m - 1) - 1) {
                lp(j, 0, m - 1) a[2 * i + 2][2 * j + 1] = ' ';
            }
            return a;
        }
        if (-k + (n - 1) * m + n * (m - 1) <= m - 1) {
            lp(i, 0, n - 2) { lp(j, 0, m - 1) a[2 * i + 2][2 * j + 1] = '-'; }
            lp(j, 0, m - 2) { lp(i, 0, n - 1) a[2 * i + 1][2 * j + 2] = '|'; }
            lp(j, 0, -k + (n - 1) * m + n * (m - 1) - 1) {
                lp(i, 0, n - 1) a[2 * i + 1][2 * j + 2] = ' ';
            }
            return a;
        }
        if (k % 2 == (n - 1) % 2) {
...
int main_() {
    ...
    while (1) {
        int n, m, k;
        in(n, m, k);
        for (auto v : RoomPairs().construct(n, m, k))
            ou(v);
    }
    return 0;
}
