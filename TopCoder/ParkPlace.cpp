...
struct ParkPlace {
    vector<string> construct(int n, vector<string> a) {
        int d_4[4][2] = {{-1, 0}, {1, 0}, {0, 1}, {0, -1}};
        vvi need(n, vi(n, 2)), can(n, vi(n, 0));
        function<bool(int, int)> upda = [&](int x, int y) {
            if (x < 0 || x >= n || y < 0 || y >= n)
                return 0;
            if (a[x][y] == '#')
                return 0;
            ...
            if (can[x][y] == need[x][y] && need[x][y])
                return 1;
            return 0;
        };
        vector<vector<vpii>> eg(n, vector<vpii>(n));
        queue<pii> q;
        int allcnt = 0;
        ...
        while (q.size()) {
            ...
            for (int i = 0; i < 4; ++i) {
                ...
                if (a[nx][ny] == '#')
                    continue;
                if (need[nx][ny]) {
                    eg[x][y].push_back(pii(nx, ny));
                    eg[nx][ny].push_back(pii(x, y));
                    --need[nx][ny];
                    --need[x][y];
                    for (int j = 0; j < 4; ++j) {
                        if (upda(x + d_4[j][0], y + d_4[j][1]) &&
                            !inq[x + d_4[j][0]][y + d_4[j][1]]) {
                            q.push(pii(x + d_4[j][0], y + d_4[j][1]));
                            inq[x + d_4[j][0]][y + d_4[j][1]] = 1;
                        }
                        if (upda(nx + d_4[j][0], ny + d_4[j][1]) &&
                            !inq[nx + d_4[j][0]][ny + d_4[j][1]]) {
                            q.push(pii(nx + d_4[j][0], ny + d_4[j][1]));
                            inq[nx + d_4[j][0]][ny + d_4[j][1]] = 1;
                        }
                    }
                }
            }
        ...
        lp(x, 0, n - 1) lp(y, 0, n - 1) {
            ...
            if (a[x][y] == '#')
                continue;
            ans.push_back(conv<int, str>(x) + " " + conv<int, str>(y) + " ");
            dfs(x, y);
        }
        return ans;
    }
...
int main_() {
    ...
    cin.tie(0);
    for (auto t : ParkPlace().construct(4, {"#..#", "....", "....", "#..#"}))
        ou(t);
    return 0;
}
