...
struct SoccerStadium {
    int maximumGames(int n, int m, vector<string> a) {
        vvi color(n, vi(m, -1));
        int d_4[4][2] = {{-1, 0}, {1, 0}, {0, 1}, {0, -1}};
        function<void(int, int, int, int, bool)> dfs =
            [&](int x, int y, int old, int newc, bool cross) {
                color[x][y] = newc;
                for (int i = 0; i < 4; ++i) {
                    ...
                    if (color[nx][ny] != old)
                        continue;
                    if (!cross &&
                        a[2 * x + 1 + d_4[i][0]][2 * y + 1 + d_4[i][1]] == '|')
                        continue;
                    if (!cross &&
                        a[2 * x + 1 + d_4[i][0]][2 * y + 1 + d_4[i][1]] == '-')
                        continue;
                    dfs(nx, ny, old, newc, cross);
                }
            };
        int cnt = 0;
        lp(i, 0, n - 1) lp(j, 0, m - 1) if (color[i][j] == -1)
            dfs(i, j, -1, cnt++, 0);
        while (1) {
            int oo = -1, nn = -1, xx, yy;
            lp(x, 0, n - 1) lp(y, 0, m - 1) {
                ...
                for (int i = 0; i < 4; ++i) {
                    ...
                    if (color[nx][ny] != color[x][y])
                        t.push_back(color[nx][ny]);
                }
                lv(i, t) lv(j, t) if (i < j && t[i] == t[j]) {
                    oo = color[x][y];
                    xx = x, yy = y;
                    nn = t[i];
                    goto wokao;
        ...
        set<int> tttt;
        lp(x, 0, n - 1) lp(y, 0, m - 1) { tttt.insert(color[x][y]); }
        return tttt.size();
    }
};
int main_() {
