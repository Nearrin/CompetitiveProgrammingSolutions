...
struct Hamiltons {
    vector<int> construct(int n, int l) {
        vector<vector<int>> d(n, vi(n)), nope(n, vi(n));
        lv(u, d) lv(v, d) d[u][v] = l;
        lp(u, 0, n - 2) {
            int v = u + 1;
            d[u][v] = d[v][u] = 1;
            nope[u][v] = nope[v][u] = 1;
        }
        nope[0][n - 1] = nope[n - 1][0] = 1;
        vi stk, ans;
        vi cand;
        lv(u, d) cand.push_back(u);
        random_shuffle(rang(cand));
        function<void(int)> dfs = [&](int l) {
            if (l == n) {
                if (!nope[stk.back()][stk.front()]) {
                    ans = stk;
                }
            } else {
                for (auto u : cand) {
                    if (find(rang(stk), u) != stk.end())
                        continue;
                    if (stk.size() && nope[stk.back()][u])
                        continue;
                    stk.push_back(u);
                    dfs(l + 1);
                    stk.pop_back();
        ...
        vi a;
        lp(u, 0, n - 1) lp(v, u + 1, n - 1) a.push_back(d[u][v]);
        return a;
    }
};
int main_() {
