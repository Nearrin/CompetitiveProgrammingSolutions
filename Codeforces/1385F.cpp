...
int n,k;
vvi e;
vi up_siz,dw_siz,up_max,dw_max,pr;
...
void build_up(int u,int p){
    ...
    for(auto v:e[u])if(v!=p){
        up_max[v]=t-dw_max[v];
        //if(u==0)ou(t,v,dw_max[v]);
    }
    t=up_siz[u]==up_max[u]*k+1;
    for(auto v:e[u])if(v!=p){
        t+=dw_siz[v]==dw_max[v]*k+1;
    }
    for(auto v:e[u])if(v!=p){
        int t2=t-(dw_siz[v]==dw_max[v]*k+1);
        up_max[v]+=t2/k;
    }
     for(auto v:e[u])if(v!=p){
        build_up(v,u);
...
int main() {
    ...
    many{
        ...
        if(k==1){
            ...
            up_siz=dw_siz=up_max=dw_max=vi(n);
            build_down(0,-1);
            build_up(0,-1);
            int ans=0;
            lp(u,0,n-1){
                ...
                int cur=0;
                if(u){
                    good+=up_siz[u]==up_max[u]*k+1;
                    cur+=up_max[u];
                }
                //ou(u+1,good,cur);
                for(auto v:e[u]){
                    if(v==pr[u])continue;
                      good+=dw_siz[v]==dw_max[v]*k+1;
                    cur+=dw_max[v];
                }
                //ou(u+1,good,cur);
                cur+=good/k;
