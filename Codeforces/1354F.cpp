...
int main() {
    ...
    many {
        ...
        sort(a, [](pair<pii, int> u, pair<pii, int> v) {
            return u.first.second < v.first.second;
        });
        int common = 0;
        for (auto v : a)
            common += (k - 1) * v.first.second;
        auto f = vect<int>({-inf()}, n + 1, k + 1);
        auto g = vect<int>({}, n + 1, k + 1);
        lp(i, 0, n) f[i][0] = common;
        lv(i, a) lp(j, 1, min(k, i + 1)) {
            f[i + 1][j] = max(f[i][j], f[i][j - 1] + a[i].first.first -
                                           (k - j) * a[i].first.second);
            if (f[i + 1][j] == f[i][j]) {
                g[i + 1][j] = 0;
            } else {
                g[i + 1][j] = 1;
        ...
        rp(i, n, 1) {
            if (!k)
                break;
            if (g[i][k])
                choosen.push_back(a[i - 1].second), --k;
        }
        reve(choosen);
        int tmp = choosen.back();
        choosen.pop_back();
        lp(i, 1, n) if (find(choosen, i) == choosen.end() && i != tmp)
            choosen.push_back(i),
            choosen.push_back(-i);
        choosen.push_back(tmp);
        ou(choosen.size());
        ou(choosen);
    }
    return 0;
