...
int dfs(node *u, node *best) {
    ...
    if (u->i) {
        u->f = 1 + u->p->f;
        if (dis(u, best) + best->f > dis(u, u->p) + u->p->f)
            best = u->p;
        if (have[u->i])
            tmin(u->f, dis(u, best) + best->f);
        ans[u->i] = u->f;
        // ou(u->i, u->f);
    }
    lp(i, 0, 25) {
        if (0 && u->c[i]) {
            ou(u->i, u->c[i]->i, char(i + 'a'));
        }
        int tmp = dfs(u->c[i], best);
        u->s += tmp;
        cnt += tmp;
    }
    return cnt;
...
int main() {
    ...
    scanf("%d", &k);
    vi que(k);
    lp(i, 0, k - 1) scanf("%d", &que[i]), have[que[i]] = 1;
    dfs(nodes[0], nodes[0]);
    lp(i, 0, k - 1) {
        int tmp = que[i];
        if (i == k - 1)
