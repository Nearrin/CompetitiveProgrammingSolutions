...
template <class T> struct add_min {
    int i;
    T v, d;
    add_min(T _v = 0, int _i = -1) : i(_i), v(_v), d(0) {}
...
int main() {
    ...
    int ans = n;
    segment_tree::segment_tree<statistic::add_min<int>> tree(
        vector<statistic::add_min<int>>(2 * n));
    int one = 1;
    int mone = -1;
    int ans2 = -1;
    lp(i, 0, n - 1) {
        if (l[i] == -1) {
            if (i)
                tree.modify_segment(0, i - 1, one);
        } else {
            tree.modify_segment(l[i] + 1, i, one);
            tree.modify_single(i, {tree.query_single(l[i]).v + 1, -1});
        }
    }
    lp(i, n, 2 * n - 1) {
        // ou(i);
        if (r[i - n] >= i) {
            tree.modify_segment(0, i - 1, mone);
        } else {
            tree.modify_segment(0, r[i - n] - 1, mone);
        }
        if (l[i] <= i - n) {
            tree.modify_segment(0, i - 1, one);
        } else {
        ...
        // ou(i, tree.query_segment(i - n + 1, i).v);
        if (tmin(ans, tree.query_segment(i - n + 1, i).v))
            ans2 = i - n + 1;
    }
    ou(ans + 1, ans2 % n);
    return 0;
