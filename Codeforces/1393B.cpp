...
int main() {
    ...
    lp(i, 1, q) {
        ...
        vi tmp;
        auto it = cur.begin();
        while (it != cur.end() && tmp.size() < 3) {
            tmp.push_back(*it);
            ++it;
        }
        if (tmp.size() >= 1 && tmp[0] >= 8)
            ans = 1;
        if (tmp.size() >= 2 && tmp[0] >= 4 && tmp[1] >= 4)
            ans = 1;
        if (tmp.size() >= 2 && tmp[0] >= 6 && tmp[1] >= 2)
            ans = 1;
        if (tmp.size() >= 3 && tmp[0] >= 4 && tmp[1] >= 2 && tmp[2] >= 2)
            ans = 1;
        YEP(ans);
    }
    return 0;
