...
int main() {
    ...
    rv(i, h) if (i + 1 != h.size()) tmax(h[i], h[i + 1]);
    rv(i, v) if (i + 1 != v.size()) tmax(v[i], v[i + 1]);
    int dy = 0;
    lv(i, a) {
        if (a[i].second <= v[a[i].first])
            tmax(dy, v[a[i].first] - a[i].second + 1);
    }
    int ans = dy;
    int dx = 0;
    while (1) {
        ...
        dy = 0;
        lv(i, a) {
            if (a[i].second <= v[a[i].first + dx])
                tmax(dy, v[a[i].first + dx] - a[i].second + 1);
        }
        tmin(ans, dx + dy);
    }
    ou(ans);
