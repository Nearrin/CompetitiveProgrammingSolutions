...
namespace minimum_nondecreasing_subsequence_decompsition {
template <class T>
vector<int> minimum_nondecreasing_subsequence_decompsition(const T &a) {
    map<typename T::value_type, int> m;
    vector<int> c;
    for (auto v : a) {
        auto i = m.upper_bound(v);
        if (i == m.begin())
            m[v] = m.size();
        else {
            --i;
...
int main() {
    ...
    in(n, s);
    assert(n == s.size());
    vi color = minimum_nondecreasing_subsequence_decompsition::
        minimum_nondecreasing_subsequence_decompsition(s);
    appl(++(*i), color);
    ou(max(color));
    ou(color);
    return 0;
