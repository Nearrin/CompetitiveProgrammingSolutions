...
vi manachers_algorithm(str s) {
    ...
    lp(i, 1, s.size() - 2) {
        ...
        int &pi = p.back();
        if (bg > i)
            pi = min(bg - i, p[(who << 1) - i - 1]);
        else
            pi = 1;
        for (; s[i + pi] == s[i - pi]; pi++)
            ;
        if (i + pi > bg) {
            bg = i + pi;
            who = i;
...
int main() {
    ...
    many {
        ...
        for (auto c : s)
            t.push_back('#'), t.push_back(c);
        t.push_back('#');
        auto p = manachers_algorithm(t);
        vi m(t.size(), 0);
        lv(i, t) {
            if (t[i] == t[t.size() - i - 1])
                m[i] = i + 1;
        ...
        lp(i, 1, t.size() - 1) tmax(m[i], m[i - 1]);
        int ans = -1, ansl, ansr, ansd;
        // ou(t);
        lv(i, t) {
            ...
            if (r + d >= t.size() - 1)
                d = t.size() - 1 - r;
            // ou(i, l, r, d, d * 2 + r - l + 1);
            if (l - d == 0 || r + d == t.size() - 1)
                if (d * 2 + r - l + 1 > ans) {
                    ans = d * 2 + r - l + 1;
                    ansl = l;
                    ansr = r;
        ...
        anss.reserve(s.size());
        lv(i, t) {
            if (i < ansd || i >= t.size() - ansd || (i >= ansl && i <= ansr)) {
                if (t[i] != '#')
                    anss.push_back(t[i]);
            }
        }
