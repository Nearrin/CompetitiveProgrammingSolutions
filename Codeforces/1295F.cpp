...
using inte = integer<int>;
void poly_add(vector<inte> &a, inte b) {
    for (auto &v : a)
        v += b;
}
void poly_sum(vector<inte> &a) {
    a.push_back(lagrange_interpolation(a, (int)a.size()));
    for (int i = 1; i < a.size(); ++i)
        a[i] += a[i - 1];
}
struct poly {
    poly() : p{1}, l(0), r(998244351) {}
    vector<inte> p;
    int l, r;
};
int main() {
    ...
    for (auto [l, r] : a) {
        ...
        for (int i = 0; i < pre.size(); ++i) {
            poly_sum(pre[i].p);
            poly_add(pre[i].p,
                     -lagrange_interpolation(pre[i].p, pre[i].l - 1) + t);
            t = lagrange_interpolation(pre[i].p, pre[i].r);
        }
        vector<poly> now;
        for (auto &p : pre) {
            int inl = (p.l <= l && p.r >= l);
    ...
    ou(lagrange_interpolation(pre.back().p, 998244351) / q * 998244352);
    return 0;
}
