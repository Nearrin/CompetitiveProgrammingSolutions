...
int main() {
    ...
    for (auto v : c)
        b.push_back(v);
    sort(b);
    vl prefix(b.size()), suffix(b.size());
    vi prefix_second(b.size()), suffix_second(b.size());
    lv(i, b) {
        prefix[i] = ll(b[i].first) * b[i].second;
        prefix_second[i] = b[i].second;
        if (i) {
            prefix[i] += prefix[i - 1];
    ...
    rv(i, b) {
        suffix[i] = ll(b[i].first) * b[i].second;
        suffix_second[i] = b[i].second;
        if (i != b.size() - 1) {
            suffix[i] += suffix[i + 1];
    ...
    lv(i, b) {
        if (b[i].second >= k) {
            ...
            ll this_ans = 0;
            int need = k - b[i].second;
            int have_prefix = (i ? prefix_second[i - 1] : 0);
            int have_suffix = (i == b.size() - 1 ? 0 : suffix_second[i + 1]);
            ll cost_prefix =
                -(i ? prefix[i - 1] : 0) + ll(have_prefix) * (b[i].first - 1);
            ll cost_suffix = (i == b.size() - 1 ? 0 : suffix[i + 1]) -
                             ll(have_suffix) * (b[i].first + 1);
            if (have_prefix < need && have_suffix < need) {
                this_ans += cost_prefix + cost_suffix;
                this_ans += need;
            }
            elif (have_prefix >= need && have_suffix >= need) {
                this_ans += min(cost_prefix, cost_suffix);
                this_ans += need;
            }
            elif (have_prefix >= need) {
                this_ans += cost_prefix;
                this_ans += need;
            }
            else {
                this_ans += cost_suffix;
                this_ans += need;
            }
            tmin(ans, this_ans);
        }
