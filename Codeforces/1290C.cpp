...
template <class T = statistic> struct union_find_set {
    ...
    auto find(int x) {
        ...
        return make_pair(u.first, p_s[x]);
    }
    void link(int x, int y, typename T::path t) {
        auto u = find(x), v = find(y);
    ...
    auto &query_tree(int x) { return t_s[find(x).first]; }
...
int main() {
    ...
    union_find_set::union_find_set<union_find_set::bipartite> ufs(k);
    auto get_ans = [&](int x) {
        auto &t = ufs.query_tree(x);
        if (t.c == -1)
            return min(t.s[0], t.s[1]);
        else
            return t.c * t.s[0] + (1 - t.c) * t.s[1];
    };
    int ans = 0;
    lp(i, 0, n - 1) {
        if (p[i].size() == 1) {
            ans -= get_ans(p[i][0]);
            ufs.modify_single(p[i][0], 1 - (a[i] - '0'));
            ans += get_ans(p[i][0]);
        }
        elif (p[i].size() == 2) {
            if (ufs.find(p[i][0]).first != ufs.find(p[i][1]).first) {
                ans -= get_ans(p[i][0]) + get_ans(p[i][1]);
                ufs.link(p[i][0], p[i][1],
                         union_find_set::bipartite::path(1 - (a[i] - '0')));
                ans += get_ans(p[i][1]);
            }
        }
        ou(ans);
