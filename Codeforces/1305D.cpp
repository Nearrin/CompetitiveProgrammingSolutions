...
void merge(int u, int p, vi &newcur) {
    newcur.push_back(u);
    for (auto v : to[u])
        if (in_cur[v] && v != p)
            merge(v, u, newcur);
...
int main() {
    ...
    vi cur;
    vi ind(n + 1, 0);
    lp(i, 1, n) cur.push_back(i);
    in_cur = vi(n + 1, 1);
    while (1) {
        if (cur.size() == 1) {
            ou("!", cur.front()), cout.flush();
            break;
        }
        elif (cur.size() == 2) {
            ou("?", cur[0], cur[1]), cout.flush();
            int tmp;
            in(tmp);
            ou("!", tmp), cout.flush();
            break;
            ...
            for (auto v : to[r])
                if (in_cur[v])
                    ch.push_back(v);
            ou("?", ch[0], ch[1]), cout.flush();
            int tmp;
            in(tmp);
            vi newcur;
            if (tmp == r) {
                newcur.push_back(r);
                lp(i, 2, ch.size() - 1) merge(ch[i], r, newcur);
            } else {
                while (par[tmp] != r)
                    tmp = par[tmp];
                assert(tmp == ch[0] || tmp == ch[1]);
                if (tmp == ch[0]) {
                    merge(ch[0], r, newcur);
                } else {
                    merge(ch[1], r, newcur);
