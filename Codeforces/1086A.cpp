...
int main() {
    ...
    if (tmin(ans, dis(p[0], p[1]) + dis(p[1], p[2]))) {
        ansp.clear(), ansq.clear();
        ansp.push_back(p[0]);
        ansq.push_back(p[1]);
        ansp.push_back(p[1]);
        ansq.push_back(p[2]);
    }
    if (tmin(ans, dis(p[0], p[2]) + dis(p[1], p[2]))) {
        ansp.clear(), ansq.clear();
        ansp.push_back(p[0]);
        ansq.push_back(p[2]);
        ansp.push_back(p[1]);
    ...
    for (auto x : xs)
        for (auto y : ys) {
            if (tmin(ans, dis(p[0], {x, y}) + dis(p[1], {x, y}) +
                              dis(p[2], {x, y}))) {
                ansp.clear(), ansq.clear();
                ansp.push_back(p[0]);
                ansq.push_back({x, y});
    ...
    set<pii> st;
    lv(i, ansp) {
        int x1 = ansp[i].first, y1 = ansp[i].second;
        int x2 = ansq[i].first, y2 = ansq[i].second;
        st.insert({x1, y1});
        st.insert({x2, y2});
        while (x1 != x2) {
            if (x1 < x2)
