...
int main() {
    ...
    many {
        ...
        if (a < b)
            swap(a, b);
        ll t_min = 0, t_max = min(x / a, y / b);
        if (a == b)
            ou(t_max);
        elif (a * x - b * y <= 0) ou(x / b);
        else {
            ll t_mid = ceil((a * x - b * y), (a * a - b * b)),
               ans = max(min(x / a, y / b), min(y / a, x / b));
            if (t_mid <= t_max)
                tmax(ans, (x - a * t_mid) / b + t_mid);
            else
                t_mid = t_max + 1;
            if (t_mid - 1 >= t_min)
                tmax(ans, (y - b * (t_mid - 1)) / a + t_mid - 1);
            ou(ans);
        }
    }
    return 0;
