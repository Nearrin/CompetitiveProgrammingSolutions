...
int main() {
    ...
    int ans = -inf();
    vector<statistic::statistic> ttt;
    for (auto v : a)
        ttt.emplace_back(v);
    segment_tree::segment_tree<statistic::statistic> tree(ttt);
    lv(i, a) {
        int l = pre[i] + 1, r = bak[i] - 1;
        tmax(ans, tree.query_segment(l, i).r + tree.query_segment(i, r).l -
                      2 * a[i]);
    }
    ou(ans);
    return 0;
