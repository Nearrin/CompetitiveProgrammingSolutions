...
int main() {
    ...
    many {
        ...
        lp(i, 1, n / 2) {
            lp(j, 1, n) f1[j] += f1[j - 1];
            vi g1(n + 1), g2(n + 1), g1mx(n + 1), g2mx(n + 1);
            lp(j, 1, n) {
                if (j < n && a[j + 1] < a[j])
                    continue;
                int t = f1[j - 1] - (lt[j] == 1 ? 0 : f1[lt[j] - 2]);
                if (i == 1 && j == 2 && 0) {
                    ou(lt[j] - 2, j - 1, t);
                }
                if (t)
                    ...
                    g1[j] = 1, g1mx[j] = max(a[j], f2mx[j - 1]);
                    if (j < n && a[j + 1] < g1mx[j]) {
                        g1[j] = 0;
                    }
                }
            ...
            lp(j, 1, n) {
                ...
                if (t)
                    g2[j] = 1, g2mx[j] = a[j];
                elif (f2[j - 1]) g2[j] = 1, g2mx[j] = max(a[j], f2mx[j - 1]);
            }
            f1 = g1;
            f2 = g2;
            f1mx = g1mx;
        ...
        lp(i, 1, n) {
            if (!f1[i])
                continue;
            int fail = 0;
            lp(j, i + 1, min(i + 1, n)) if (a[j] < f1mx[i]) fail = 1;
            if (!fail)
                ++ans;
        }
        YEP(ans >= 1);
