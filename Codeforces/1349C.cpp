...
int main() {
    ...
    lp(i, 0, n - 1) lp(j, 0, m - 1) {
        if (good(i, j))
            f.distance[i * m + j] = 0, ++gc;
        if (i)
            f.add_edge(i * m + j, i * m - m + j);
        if (j)
            f.add_edge(i * m + j, i * m + j - 1);
    }
    f.run();
    while (k--) {
        ll i, j, p;
        in(i, j, p);
        --i, --j;
        if (p <= f.distance[i * m + j] || !gc)
            ou(a[i][j]);
        else
            ou(char('0' + ((a[i][j] - '0') ^ (f.distance[i * m + j] - p) & 1)));
    }
    return 0;
}
