...
void build_down(int u, int p) {
    for (auto v : e[u]) {
        if (v != p) {
            build_down(v, u);
            s_down[u] += s_down[v] + a[v];
            ss_down[u] += ss_down[v] + s_down[v] + a[v];
        }
    }
}
void build_up(int u, int p) {
    ll s = s_up[u], ss = ss_up[u];
    for (auto v : e[u])
        if (v != p) {
            s += s_down[v] + a[v];
            ss += ss_down[v] + s_down[v] + a[v];
        }
    for (auto v : e[u])
        if (v != p) {
            s_up[v] = s - s_down[v] - a[v] + a[u];
            ss_up[v] = ss - ss_down[v] - s_down[v] - a[v] + s_up[v];
            build_up(v, u);
        }
}
int main() {
    ...
    build_up(0, -1);
    lp(v, 0, n - 1) {
        // ou(v + 1, s_up[v], s_down[v]);
        // ou(v + 1, ss_up[v], ss_down[v]);
        tmax(ans, ss_up[v] + ss_down[v]);
    }
    ou(ans);
    return 0;
}
