...
int f2(int n) {
    ...
    for (auto d : divisor::divisor(
             integer_factorization_3::shankss_square_forms_factorization(n))) {
        auto tmp =
            integer_factorization_3::shankss_square_forms_factorization(n / d);
        if (tmp.size())
            ans += d * min(tmp).first;
    }
    return ans;
...
int main_() {
    ...
    ou(1., e1, e2, log(n));
    // notes: f2 is O(nlgn) we can relax it to prime omega function(n)* sum of
    // divsors(n)
    // the former is lgn/lglgn the later is nlgnlgn
    // then we have O(nlgn)!!!!
    return 0;
}
namespace eulers_totient_function {
auto eulers_totient_function(int n) {
    vector<int> p, ntp(n + 1), u(n + 1), sm(n + 1);
    ntp[1] = 1;
    u[1] = 1;
    sm[1] = -1;
    for (int i = 2; i <= n; ++i) {
        if (!ntp[i])
            p.push_back(i), u[i] = i - 1, sm[i] = i;
...
int main() {
    fast, prec(12);
    in(n);
    auto _ = eulers_totient_function::eulers_totient_function(n);
    e = _.first, d = _.second;
    str s;
    in(s);
    vi s2;
    for (auto ch : s)
