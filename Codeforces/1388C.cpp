...
int main() {
    ...
    many {
        ...
        function<pll(int, int)> dfs = [&](int u, int parent) {
            ll num_people = p[u], goods = 0;
            for (int v : e[u])
                if (v != parent) {
                    auto t = dfs(v, u);
                    goods += t.second;
            ...
            if (abs(num_people) % 2 != abs(h[u]) % 2)
                fail = 1;
            ll num_good = (num_people + h[u]) / 2;
            if (num_good < 0 || num_good > num_people)
                fail = 1;
            if (goods > num_good)
                fail = 1;
            return pll{num_people, num_good};
        };
        dfs(0, -1);
        yep(1 - fail);
    }
