...
ll pow(ll a, ll b, int &flag) {
    if (!b)
        return 1;
    auto t = pow(a, b / 2, flag);
    t *= t;
    if (t >= int(1e9 + 7)) {
        flag = 1;
        t %= int(1e9 + 7);
...
int main() {
    ...
    many {
        ...
        for (auto v : a) {
            // ou(v, current);
            int delta = previous - v;
            if (current) {
                int flag = 0;
                int m = pow(p, delta, flag);
                if (flag || ll(current) * m >= int(1e9 + 7)) {
                    large = true;
                }
                current = ll(current) * m % int(1e9 + 7);
            }
            if (current || large) {
                --current;
                if (current < 0)
        ...
        int flag;
        // ou(current, previous, pow(p, previous, flag));
        ou(current * pow(p, previous, flag) % int(1e9 + 7));
    }
    return 0;
}
