...
int check(int c, int s) {
    if (s == 1)
        return c;
    int ab = c / (s - 1), b = c % (s - 1);
    if (ab < b)
        return -1;
    return ab - (ab - b) / s;
}
int main() {
    ...
    many {
        ...
        for (auto v : c)
            ++d[v];
        vpii b;
        lv(i, d) if (i && d[i]) b.emplace_back(i, d[i]);
        int ans = inf(), ans2;
        lp(i, 1, b.front().first + 1) {
            int this_ans = 0;
            lv(j, b) {
                int t = check(b[j].first, i);
                if (t == -1)
                    goto again;
                this_ans += t * b[j].second;
                if (this_ans >= ans)
                    goto again;
            }
            if (tmin(ans, this_ans))
                ans2 = i;
        again:
            continue;
        }
