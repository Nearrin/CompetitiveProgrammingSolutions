...
void update(const vi &a, const vi &b, const vi &c, ll &ans) {
    int x = 0, z = 0;
    for (auto y : b) {
        while (x + 1 < a.size() && a[x + 1] <= y)
            ++x;
        while (z + 1 < c.size() && c[z] < y)
            ++z;
        auto sqr = [](ll tmp) { return tmp * tmp; };
        tmin(ans, sqr(y - a[x]) + sqr(y - c[z]) + sqr(a[x] - c[z]));
    }
}
int main() {
    fast, prec(12);
