...
auto get_idx(vpii &ops) {
    vi t;
    for (auto v : ops)
        t.push_back(abs(v.second));
    uniq(t);
    reve(t);
...
int main() {
    ...
    vpii ops(n);
    in(ops);
    auto idx = get_idx(ops);
    int cnt_fire = 0, cnt_li = 0;
    vl tree_sum(idx.size() + 1);
    vi tree_cntf(idx.size() + 1);
    vi tree_cnt(idx.size() + 1);
    set<int> val_fire;
    set<int> val_li;
    ll ts = 0;
    for (auto [tp, d] : ops) {
        int i = idx[abs(d)];
        if (d > 0) {
            for (int j = i; j <= idx.size(); j += j & -j) {
                tree_sum[j] += d;
                tree_cntf[j] += tp == 0;
                tree_cnt[j] += 1;
            }
            if (tp == 0)
                ++cnt_fire, val_fire.insert(d);
            else
                ++cnt_li, val_li.insert(d);
            ts += d;
        } else {
            d = -d;
            for (int j = i; j <= idx.size(); j += j & -j) {
            ...
            if (tp == 0)
                --cnt_fire, val_fire.erase(d);
            else
                --cnt_li, val_li.erase(d);
            ts -= d;
        }
        auto kth = [&](int k) {
            ll sm = 0;
            ...
            for (int i = 1 << 17; i; i >>= 1)
                if (r + i <= idx.size() && tree_cnt[r + i] < k)
                    k -= tree_cnt[r += i], sm += tree_sum[r],
                        cntf += tree_cntf[r];
            return pll(sm, cntf);
        };
        auto [sm, cntf] = kth(cnt_li + 1);
        if (cnt_li == 0) {
            ou(ts);
        }
        elif (cntf) { ou(ts + sm); }
            ...
            if (val_fire.size() == 0) {
                // ou(tree_sum);
                // ou(tree_cnt);
                // ou(ts, sm, *val_li.begin(), cnt_li);
                ou(ts + sm - *val_li.begin());
            } else {
                ou(ts + sm - *val_li.begin() + *val_fire.rbegin());
            }
        }
    }
    return 0;
