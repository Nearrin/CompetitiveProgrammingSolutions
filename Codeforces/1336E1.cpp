...
namespace basis {
template <int N> auto basis_z2(vector<bitset<N>> a) {
    for (int i = 0, s = 0; i < int(a.size()) && s < N; ++i) {
        int t = int(a[i]._Find_first());
        if (t == N)
            continue;
        ++s;
...
void dfs(ll cur, int idx, const vl &b, vi &ans) {
    if (idx == b.size()) {
        int cnt = bcnt(cur);
        ++ans[cnt];
    } else {
        dfs(cur, idx + 1, b, ans);
        dfs(cur ^ b[idx], idx + 1, b, ans);
    }
}
int main() {
    fast, prec(12);
    ...
    in(n, m);
    vector<bitset<35>> a;
    lp(i, 1, n) a.push_back(in<ll>());
    auto bb = basis::basis_z2<35>(a);
    vl b;
    // ou(numeric_limits<unsigned long>::max());
    for (auto &v : bb)
        b.push_back(v.to_ullong());
    int k = b.size();
    vi ans(m + 1);
    int delta = 1;
    lp(i, 1, n - k) delta = delta * 2 % 998244353;
    if ((1ll << k) <= k * k * (1ll << (m - k))) {
        dfs(0, 0, b, ans);
    } else {
        vi important(m);
        for (auto v : b) {
            important[bitset<35>(v)._Find_first()] = 1;
        }
        for (auto &v : b) {
            ll t1 = 0, t2 = 0;
            lp(i, 0, m - 1) {
        ...
        auto dp = vect<int>({}, k + 1, k + 1, 1 << (m - k));
        dp[0][0][0] = 1;
        lp(i, 1, k) {
            lp(j, 0, i) {
                lp(s, 0, (1 << (m - k)) - 1) {
                    if (j)
                        dp[i][j][s] += dp[i - 1][j - 1][s ^ b[i - 1]],
                            dp[i][j][s] %= 998244353;
                    if (j != i)
                        dp[i][j][s] += dp[i - 1][j][s],
                            dp[i][j][s] %= 998244353;
                    if (i == k) {
                        ans[j + bcnt(s)] += dp[i][j][s];
                        ans[j + bcnt(s)] %= 998244353;
                    }
                }
            }
        }
    }
    lv(i, ans) ans[i] = 1ll * ans[i] * delta % 998244353;
    if (k == 0)
        ans[0] = delta;
    ou(ans);
    return 0;
