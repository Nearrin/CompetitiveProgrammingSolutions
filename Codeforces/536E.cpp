  ...
  public:
    class Iterator {
      private:
        node *pointer;
        friend class Set;
        inline Iterator(node *_pointer = 0) : pointer(_pointer) {}
      ...
      public:
        ...
        inline const VAL &operator*() { return pointer->value; }
    ...
    inline const VAL &Front() { return *Begin(); }
    inline const VAL &Back() { return *ReverseBegin(); }
};
template <class IFO> class DynamicTree {
  private:
    ...
    inline void down(node *x) {
        ...
        x->ifo.Down(x->ch[0] ? &x->ch[0]->ifo : 0,
                    x->ch[1] ? &x->ch[1]->ifo : 0);
    }
    inline void update(node *x) {
        for (int i = 0; i < 2; ++i)
            if (x->ch[i])
                down(x->ch[i]);
        x->ifo.Update(x->ch[0] ? &x->ch[0]->ifo : 0,
                      x->ch[1] ? &x->ch[1]->ifo : 0);
    }
  ...
  public:
    class Pointer {
      ...
      public:
        inline Pointer(node *_p = 0) : p(_p) {}
        inline bool operator==(const Pointer &a) { return p == a.p; }
        inline bool operator!=(const Pointer &a) { return p != a.p; }
        inline const IFO &operator*() { return p->ifo; }
        inline const IFO *operator->() { return &p->ifo; }
    };
    inline Pointer Make(const IFO &a) { return new node(a); }
    ...
    inline void Evert(const Pointer &a) { evert(a.p); }
    ...
    inline IFO EdgeQuery(const Pointer &a) { return Query(a); }
    ...
    inline void Modify(const Pointer &a, const Pointer &b,
                       const std::function<void(IFO *, IFO *, IFO *)> &f) {
        if (a.p == b.p) {
            splay(a.p);
            f(0, &a.p->ifo, 0);
            update(a.p);
    ...
    inline void EdgeModify(const Pointer &a,
                           const std::function<void(IFO *)> &f) {
        Modify(a, f);
    }
    inline void EdgeModify(const Pointer &a, const Pointer &b,
                           const std::function<void(IFO *, IFO *)> &f) {
        access(a.p);
        node *c = access(b.p);
        IFO p = c->ifo;
        if (c == b.p) {
    ...
    void Update(Info *a, Info *b) {
        if (a == 0 && b == 0) {
            ...
            if (val == 0) {
                ...
                lc1 = a->lc1;
                rc1 = b->rc1;
                sum = (a->sum - f[a->rc1]) + (b->sum - f[b->lc1]) +
                      f[a->rc1 + b->lc1 + 1];
                ful1 = 0;
            }
        }
    }
    void Reverse() { swap(lc1, rc1); }
};
DynamicTree<Info>::Pointer ptr[100010];
void dfs(int u, int p) {
    vp(i, to[u]) if (to[u][i] != p) {
        prt[to[u][i]] = u;
        prtw[to[u][i]] = wi[u][i];
        dfs(to[u][i], u);
    }
...
bool operator<(Ans a, Ans b) { return a.l < b.l; }
void modi(Info *ii) { ii->val = 0; }
int ans[100010];
int main() {
    sf("%d%d", &n, &q);
    lp(i, 1, n - 1) sf("%d", f + i);
