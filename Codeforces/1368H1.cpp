...
int solve(str l, str r, str u, str d) {
    auto f = vect<int>(inf(), u.size() + 1, 2);
    f[0][0] = f[0][1] = 0;
    auto g = [](char c) { return c == 'R'; };
    lv(i, l)++ f[0][1 - g(l[i])];
    lv(i, u) lp(j, 0, 1) {
        tmin(f[i + 1][j], f[i][j] + (g(u[i]) != j) + (g(d[i]) != j));
        tmin(f[i + 1][1 - j],
             f[i][j] + int(l.size()) + (g(u[i]) == j) + (g(d[i]) == j));
    }
    lv(i, r)++ f[u.size()][1 - g(r[i])];
    return min(f.back());
}
int main() {
    ...
    str l, r, u, d;
    in(l, r, u, d);
    ou(min(solve(l, r, u, d), solve(u, d, l, r)));
    return 0;
}
