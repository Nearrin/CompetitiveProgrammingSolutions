...
int x[11],y[11];
bool check(){
    lp(i,1,8)Read(x[i]),Read(y[i]);
    set<int>sx,sy;
    lp(i,1,8)sx.insert(x[i]);
    lp(i,1,8)sy.insert(y[i]);
    if(sx.size()!=3)return false;
    if(sy.size()!=3)return false;
    set<pair<int,int> >sall;
    lp(i,1,8)sall.insert(mp(x[i],y[i]));
    if(sall.size()!=8)return false;
    set<int>::iterator i=sx.begin();++i;
    int mx=*i;
    i=sy.begin();++i;
    int my=*i;
    if(sall.count(mp(mx,my)))return false;
    return true;
}
int main(){
    if(check())Write("respectable\n");
    else Write("ugly\n");
    //ps;
    return 0;
}