...
int main() {
    ...
    vi a(n), b(m);
    in(a, b);
    vl cnta(n + 1), cntb(m + 1);
    lv(i, a) {
        if (a[i]) {
            int j = i;
            while (j + 1 < a.size() && a[j + 1])
                ++j;
            lp(k, 1, j - i + 1) { cnta[k] += j - i + 1 - k + 1; }
            i = j;
        }
    }
    lv(i, b) {
        if (b[i]) {
            int j = i;
            while (j + 1 < b.size() && b[j + 1])
                ++j;
            lp(k, 1, j - i + 1) { cntb[k] += j - i + 1 - k + 1; }
            i = j;
        }
    }
    for (auto d : divisor::divisor(
             integer_factorization_3::shankss_square_forms_factorization(k))) {
        if (d <= n && k / d <= m)
            ans += cnta[d] * cntb[k / d];
    }
    ou(ans);
