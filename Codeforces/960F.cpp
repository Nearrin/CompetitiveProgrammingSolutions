...
struct Data {
    Data(int _v, int _l) : v(_v), l(_l) {}
    int v, l;
};
bool operator<(const Data &a, const Data &b) { return a.v < b.v; }
int main() {
    fast, prec(12);
    int n, m;
    in(n, m);
    ...
    lp(i, 1, m) {
        ...
        if (f[u].empty()) {
            ...
            auto j = f[u].lower_bound(Data(w, 0));
            if (j == f[u].begin())
                l = 1;
            else {
                --j;
        ...
        tmax(ans, l);
        // w,l
        auto j = f[v].lower_bound(Data(w, 0));
        if (j != f[v].end() && j->v == w) {
            if (j->l < l) {
                auto k = j;
                ++j;
