...
using integer = intm<int, 998244353>;
int n;
vvi e;
vector<integer> f00, f10, f0all, f1all;
// inset & selbychild
void dfs(int u, int p) {
    ...
    for (auto v : e[u])
        if (v != p) {
            dfs(v, u);
            f00[u] *= f1all[v] - f10[v] + f0all[v];
            f10[u] *= f1all[v] - f10[v] + f0all[v];
            f0all[u] *= f1all[v] - f10[v] + f0all[v] + (f1all[v] + f0all[v]);
            f1all[u] *= f1all[v] - f10[v] + f0all[v] + (f0all[v]);
        }
    // ou(u + 1, f00[u], f10[u], f0all[u], f1all[u]);
}
int main() {
    fast, prec(12);
    in(n);
    ...
    f00.resize(n), f10.resize(n), f0all.resize(n), f1all.resize(n);
    dfs(0, -1);
    ou(f1all[0] - f10[0] + f0all[0] - 1);
    return 0;
}
