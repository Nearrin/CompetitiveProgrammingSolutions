...
using vvi = vector<vi>;
namespace divisor_function {
struct linear_sieve {
    linear_sieve(int n) : count(n + 1, 1) {
        vector<bool> is_prime(n + 1, 1);
        vector<int> c(n + 1, 1), p;
        is_prime[1] = false;
        for (int i = 2; i <= n; ++i) {
            is_prime[i] && (p.push_back(i), count[i] = 2);
            for (int j = 0; j < int(p.size()) && p[j] * i <= n; ++j) {
                is_prime[p[j] * i] = 0;
                if (i % p[j] == 0) {
                    c[p[j] * i] = c[i] + 1;
                    count[p[j] * i] = count[i] / (c[i] + 1) * (c[i] + 2);
                    break;
                } else
                    c[p[j] * i] = 1, count[p[j] * i] = 2 * count[i];
            }
        }
    }
    vector<int> count;
...
int main() {
    ...
    int n;
    in(n);
    divisor_function::linear_sieve df(n);
    vi dp(n + 1);
    dp[0] = 0;
    lp(i, 1, n) { dp[i] = (df.count[i] + dp[i - 1] * 2ll) % 998244353; }
    ou((dp[n] - dp[n - 1] + 998244353) % 998244353);
    return 0;
}
