...
int main() {
    ...
    many {
        vi a;
        in(a);
        vi f(a.size()), g(a.size());
        lv(i, a) f[i] = a[i] >= i, g[i] = a[i] >= a.size() - i - 1;
        lp(i, 1, a.size() - 1) f[i] = f[i] && f[i - 1];
        rp(i, a.size() - 2, 0) g[i] = g[i] && g[i + 1];
        int ans = f[a.size() - 1] || g[0];
        lv(i, a) ans = ans || (f[i] && g[i]);
        ou(ans ? "Yes" : "No");
    }
    return 0;
}
