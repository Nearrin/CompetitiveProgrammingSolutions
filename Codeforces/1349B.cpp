...
gene using bbst = tree<T, null_type, greater<T>, rb_tree_tag,
...
ll work(vpii &a, int l, int r) {
    if (l == r)
        return 0;
    int m = (l + r) / 2;
    ll ans = (work(a, l, m) + work(a, m + 1, r));
    sort(a.begin() + l, a.begin() + m + 1);
    reverse(a.begin() + l, a.begin() + m + 1);
    sort(a.begin() + m + 1, a.begin() + r + 1);
    reverse(a.begin() + m + 1, a.begin() + r + 1);
    int i = l, j = m + 1;
    bbst<pii> current;
    while (j <= r) {
        while (i <= m && a[i].first + 0 > a[j].first)
            current.insert({a[i].second + 1, i}), ++i;
        ans += current.order_of_key({a[j].second, inf()});
        ou("q", a[j]);
        for (auto t : current) {
            if (t > pii{a[j].second, inf()})
                ou("a", a[t.second]);
        }
        ++j;
    }
...
int main() {
    ...
    many {
        ...
        if (find(a, k) == a.end())
            ...
            int ans = 0;
            lp(i, 0, n - 2) ans += a[i] >= k && a[i + 1] >= k;
            lp(i, 0, n - 3) ans += a[i] >= k && a[i + 2] >= k;
            ans += n == 1;
            yep(ans);
        }
    }
