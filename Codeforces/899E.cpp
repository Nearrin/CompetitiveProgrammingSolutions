...
int main() {
    ...
    lp(i, 0, n - 1) {
        ...
        while (j + 1 < n && a[j + 1] == a[j])
            ++j;
        m.insert({-(j - i + 1), i});
        lp(k, i, j) s[i].push_back(k), p[k] = i;
        i = j;
    }
    set<int> cur;
    lp(i, 0, n - 1) cur.insert(i);
    int ans = 0;
    while (cur.size()) {
        ++ans;
        auto tmp = *m.begin();
        m.erase(m.begin());
        int mi = min(s[tmp.second]);
        int mx = max(s[tmp.second]);
        // ou(s[tmp.second]);
        for (auto v : s[tmp.second])
            cur.erase(v);
        auto i1 = cur.upper_bound(mi);
        auto i2 = cur.upper_bound(mx);
        if (i1 != cur.begin() && i2 != cur.end()) {
            --i1;
            if (a[*i1] == a[*i2]) {
                int p1 = find(*i1), p2 = find(*i2);
                m.erase({-int(s[p1].size()), p1});
                m.erase({-int(s[p2].size()), p2});
                // ou("merge", s[p1], s[p2]);
