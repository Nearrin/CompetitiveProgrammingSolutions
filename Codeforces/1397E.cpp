...
ll solve1(int n, ll r1, ll r2, ll r3, ll d, vl a) {
    vl ans{0, inf<ll>()};
    lv(i, a) {
        vl newans(2, inf<ll>());
        tmin(newans[0], ans[0] + d * !!i + min(r1, r3) * a[i] + r3);
        tmin(newans[1], ans[0] + d * !!i + min(r1, r3) * a[i] + min(r1, r2) +
                            2 * d + min(min(r1, r2), r3));
        tmin(newans[1], ans[0] + d * !!i + r2 + 2 * d + min(min(r1, r2), r3));

        tmin(newans[0], ans[1] + min(r1, r3) * a[i] + r3);
        // ou(i, ans[1], d * !!i + min(r1, r3) * a[i] + r3);
        tmin(newans[1], ans[1] + d * !!i + min(r1, r3) * a[i] + min(r1, r2) +
                            2 * d + min(min(r1, r2), r3));
        tmin(newans[1], ans[1] + d * !!i + r2 + 2 * d + min(min(r1, r2), r3));
        tmin(newans[0], ans[1] + d * !!i + min(r1, r3) * a[i] + min(r1, r2) +
                            min(min(r1, r2), r3));
        tmin(newans[0], ans[1] + d * !!i + r2 + min(min(r1, r2), r3));
...
ll solve2(int n, ll x, ll y, ll z, ll d, vl aa) {
    static ll a[1000005], dp[1000005][2];
    for (int i = 1; i <= n; i++)
        a[i] = aa[i - 1];
    dp[1][0] = a[1] * x + z;
    dp[1][1] = min((a[1] + 1) * x, y);
    for (int i = 2; i <= n; i++) {
        ll c1 = a[i] * x + z, c2 = min((a[i] + 1) * x, y);
        dp[i][0] = min(dp[i - 1][0] + d + c1, dp[i - 1][1] + 3 * d + c1 + x);
        dp[i][0] = min(dp[i][0], dp[i - 1][1] + 3 * d + c2 + 2 * x);
        dp[i][1] =
            min(dp[i - 1][0] + d + c2, dp[i - 1][1] + d + c2 + 2 * d + x);
    }
    ll ans = min(dp[n][0], dp[n][1] + 2 * d + x);
    ans = min(ans, dp[n - 1][1] + 2 * d + a[n] * x + z + x);
    return ans;
}
int main() {
    ...
    while (1) {
        int n = rint(1, 3);
        vi r;
        lp(i, 1, 3) r.push_back(rint(1, 3));
        sort(r);
        int d = rint(1, 3);
        vl a;
        lp(i, 1, n) a.push_back(rint(1, 3));
        ou(n, r, d);
        ou(a);
        ll ans1 = solve1(n, r[0], r[1], r[2], d, a);
        ll ans2 = solve2(n, r[0], r[1], r[2], d, a);
        ou(ans1, ans2);
        assert(ans1 == ans2);
    }
    return 0;
