...
int main() {
    ...
    many {
        ...
        ll s;
        in(n, s);
        vector<vector<pair<int, pii>>> e(n);
        lp(i, 2, n) {
            int u, v, w, c;
            in(u, v, w, c);
            --u, --v;
        ...
        std::priority_queue<pair<ll, pll>, vector<pair<ll, pll>>, cmp> a1, a2;
        ll curs = 0;
        function<int(int, int)> dfs = [&](int u, int p) {
            int siz = 0;
            for (auto t : e[u])
                if (t.first != p) {
                    int tsiz = dfs(t.first, u);
                    if (t.second.second == 1)
                        a1.push({ceil(ll(t.second.first), 2ll) * tsiz,
                                 {t.second.first, tsiz}});
                    else
                        a2.push({ceil(ll(t.second.first), 2ll) * tsiz,
                                 {t.second.first, tsiz}});
                    curs += ll(t.second.first) * tsiz;
                    siz += tsiz;
                }
            if (siz == 0)
                ++siz;
        ...
        if (need <= 0)
            ...
            ll wehave = 0;
            vl b1;
            while (wehave < need && a1.size()) {
                auto tt = a1.top();
                a1.pop();
                wehave += tt.first;
                b1.push_back(tt.first);
                tt.second.first /= 2;
                if (tt.second.first)
                    a1.push({ceil(ll(tt.second.first), 2ll) * tt.second.second,
                             {tt.second.first, tt.second.second}});
            }
            wehave = 0;
            vl b2;
            while (wehave < need && a2.size()) {
                auto tt = a2.top();
                a2.pop();
                wehave += tt.first;
                b2.push_back(tt.first);
                tt.second.first /= 2;
                if (tt.second.first)
                    a2.push({ceil(ll(tt.second.first), 2ll) * tt.second.second,
                             {tt.second.first, tt.second.second}});
            }
            int ans = inf();
            lv(i, b1) if (i) b1[i] += b1[i - 1];
            lv(i, b2) if (i) b2[i] += b2[i - 1];
            lp(c1, 0, b1.size()) {
                int cost = 0;
                cost += c1;
                ll still = need;
                ...
                elif (b2.size() && b2.back() >= still) {
                    auto it = lower_bound(rang(b2), still);
                    int c2 = it - b2.begin() + 1;
                    cost += c2 * 2;
                    tmin(ans, cost);
                }
            }
