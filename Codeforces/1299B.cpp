...
bool check(const pll &x, const pll &y) {
    return (x.first + y.first == 0 && x.second + y.second == 0);
}
int main() {
    fast, prec(12);
    vpll a;
    ...
    if (a.size() % 2 == 1) {
        ...
        vpll b;
        lv(i, a) {
            pll &x = a[i], &y = a[(a.size() + i - 1) % a.size()];
            b.emplace_back(x.first - y.first, x.second - y.second);
        }
        int fail = 0;
        int n = b.size();
        for (int i = 0; i < n / 2; ++i) {
            if (!check(b[i], b[i + n / 2]))
                fail = 1;
        }
        if (fail)
            ou("NO");
