...
int main() {
    ...
    cin.tie(0);
    int n = 2e6;
    auto f = vect<int>(0, n + 1, 2);
    lp(i, 3, n) {
        f[i][1] = f[i - 1][0] + 2 * f[i - 2][0] % int(1e9 + 7);
        if (i % 3 == 0)
            f[i][0] = f[i - 1][1] + 2 * f[i - 2][1] % int(1e9 + 7) + 1;
        else
            f[i][0] = f[i - 1][0] + 2 * f[i - 2][0] % int(1e9 + 7);
        f[i][0] %= int(1e9 + 7);
        f[i][1] %= int(1e9 + 7);
        // f[i][0] = max(f[i - 1][1] + 2 * f[i - 2][1] + 1,
        //               f[i - 1][0] + 2 * f[i - 2][0]);
        // int u = f[i - 1][1] + 2 * f[i - 2][1] + 1,
        //    v = f[i - 1][0] + 2 * f[i - 2][0];
        // ou(i, f[i][0], comp(u, v));
    }
    many {
