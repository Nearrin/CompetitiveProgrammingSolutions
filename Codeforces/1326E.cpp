...
struct statistic {
    statistic() {}
    statistic(int is_one, int has_bomb) {
        remain_one_cnt = max(is_one - has_bomb, 0);
        got_zero_cnt = has_bomb && !is_one;
    }
    ...
    static statistic merge(const statistic &a, const statistic &b) {
        statistic c;
        c.remain_one_cnt =
            b.remain_one_cnt + max(a.remain_one_cnt - b.got_zero_cnt, 0);
        c.got_zero_cnt =
            a.got_zero_cnt + max(b.got_zero_cnt - a.remain_one_cnt, 0);
        return c;
    }
    static void push(statistic &a, int &t) {}
    int remain_one_cnt, got_zero_cnt;
};
...
int main() {
    ...
    lv(i, p) pos[p[i]] = i;
    int ans = n;
    vector<statistic::statistic> a(n);
    lp(i, 0, n - 1) a[i] = statistic::statistic(p[i] == n, 0);
    segment_tree::segment_tree seg(a);
    vi bb(n);
    vi anss;
    lv(i, q) {
        if (i) {
            bb[q[i - 1] - 1] = 1;
            seg.modify_single(q[i - 1] - 1,
                              statistic::statistic(p[q[i - 1] - 1] >= ans, 1));
        }
        while (seg.root->s.remain_one_cnt == 0) {
            --ans;
            seg.modify_single(pos[ans], statistic::statistic(1, bb[pos[ans]]));
        }
        anss.push_back(ans);
    }
    ou(anss);
