...
int f(vvi a) {
    ...
    lp(i, 0, n - 1) {
        u.push_back(0);
        lp(j, 0, n - 1) u.back() += a[i][j];
    }
    lp(i, 0, n - 1) {
        v.push_back(0);
        lp(j, 0, n - 1) v.back() += a[j][i];
    }
    return (max(u) - min(u)) * (max(u) - min(u)) +
...
int main() {
    ...
    many {
        ...
        int now = 0, l = 0, r = p.size() - 1;
        lp(i, 1, k) {
            if (now == 0) {
                int x = p[l].back().first, y = p[l].back().second;
                a[x][y] = 1;
                p[l].pop_back();
                if (p[l].empty()) {
                    now = 1;
                ...
                int x = p[r].back().first, y = p[r].back().second;
                a[x][y] = 1;
                p[r].pop_back();
                if (p[r].empty()) {
                    now = 0;
