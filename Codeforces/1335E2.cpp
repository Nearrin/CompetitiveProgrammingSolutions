...
int main() {
    ...
    many {
        ...
        int ans = 0;
        vvi occ(201);
        lv(i, a) occ[a[i]].push_back(i);
        lp(candidate, 1, 200) {
            tmax(ans, (int)occ[candidate].size());
            int l = occ[candidate].size() / 2;
            if (!l)
                continue;
            int p1 = occ[candidate][l - 1];
            int p2 = occ[candidate][occ[candidate].size() - l];
            vi cnt(201);
            lp(i, p1 + 1, p2 - 1) { ++cnt[a[i]]; }
            int mx = max(cnt);
            rp(i, l - 1, 0) {
                int np1 = occ[candidate][i];
                int np2 = occ[candidate][occ[candidate].size() - i - 1];
                lp(j, np1 + 1, p1 - 1) {
                    ++cnt[a[j]];
                    tmax(mx, cnt[a[j]]);
                }
