...
string check(string s) {
    ...
    } while (next_permutation(rang(t)));
    return ans1;
}
bool check_2(string s) {
    set<char> c;
    lv(i, s) c.insert(s[i]);
    return c.size() >= 3 || s[0] != s.back() || s.size() == 1;
}
int main() {
    fast, prec(12);
    while (0) {
        string s;
        int n = rand() % 12 + 1;
        lp(i, 1, n) s.push_back('a' + rand() % 6);
        auto c1 = check(s);
        auto c2 = check_2(s);
        ou(s, c1, c2);
        assert(bool(c1.size()) == c2);
    }
    while (0) {
        string s;
        in(s);
