...
inte dfs(int u, int p, int s) {
    ...
    if (vis[u][s])
        return f;
    vis[u][s] = 1;
    // s=0 meaning u is not deleted by his parent's edge
    // s=1 meaning u is deleted by his parent's edge at time pid[u]
    // s=2 meaning u is not deleted by his parent's edge but must be deleted by
    // his children's edge before pid[u]
    // s=3 meaning u is not deleted by his parent's edge and has to survive
    // untill pid[u]
    int num_child = 0;
    lv(i, to[u]) {
        int v = to[u][i], w = id[u][i];
