...
struct scc {
    scc(int n_) : n(n_), num(0), to(n), rto(n), vis(n), seq(n), lab(n, -1) {}
...
int main() {
    ...
    many {
        ...
        vector<set<int> *> s(n);
        lp(u, 0, n - 1) {
            set<int> *nope = new set<int>();
            map<set<int> *, set<int>> tmp;
            for (auto v : to[u])
                if (s[v] == 0) {
                    s[v] = nope;
                    nope->insert(v);
            ...
            for (auto &tt : tmp) {
                if (tt.first->size() == tt.second.size())
                    continue;
                set<int> *hha = new set<int>();
                for (auto v : tt.second)
                    hha->insert(v), s[v] = hha, tt.first->erase(v);
            }
        }
        set<set<int> *> tmp;
        lp(i, 0, n - 1) tmp.insert(s[i]);
        ll ans = 0;
        for (auto ii : tmp) {
            if (!ii)
                continue;
