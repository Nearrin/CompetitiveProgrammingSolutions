...
int main() {
    ...
    for (auto &i : b) {
        int cost = indices.size();
        vi precost(i.second.size());
        lv(j, i.second) {
            if (j) {
                int l = i.second[j - 1], r = i.second[j];
                int c1 = lower_bound(rang(indices), r) - begin(indices);
                int c2 = end(indices) - lower_bound(rang(indices), l);
                int c3 = indices.size();
                c3 -= (c3 - c1) + (c3 - c2);
                precost[j] = -c3;
            }
        ...
        vi f(i.second.size());
        f[0] = 1;
        lv(j, i.second) if (j) { f[j] = max(1, 1 + f[j - 1] + precost[j]); }
        cost += max(f);
        tmax(ans, cost);
    }
    ou(ans);
