...
vi check(int n, vvi &a, vi p, vector<vvi> &seg_l_v) {
    vi vis(n + 1);
    for (auto v : p)
        vis[v] = 1;
    while (p.size() != n) {
        ...
        rv(i, p) {
            t[p[i]] = 1;
            int j = -1;
            int luck = rint(i, int(p.size()) - 1);
            for (auto k : seg_l_v[p.size() - i + 1][p[luck]]) {
                int ncnt = 0, nn;
                for (auto v : a[k])
                    if (!t[v]) {
                        ncnt++;
...
int main() {
    ...
    many {
        ...
        in(a);
        vi t;
        auto seg_l_v = vect<vi>({}, n + 1, n + 1);
        lv(i, a) {
            for (auto v : a[i]) {
                seg_l_v[a[i].size()][v].push_back(i);
            }
        }
        lv(i, a) {
            if (a[i].size() == 2) {
                t = check(n, a, {a[i][0], a[i][1]}, seg_l_v);
                if (t.size())
                    break;
                t = check(n, a, {a[i][1], a[i][0]}, seg_l_v);
                if (t.size())
                    break;
            }
        }
