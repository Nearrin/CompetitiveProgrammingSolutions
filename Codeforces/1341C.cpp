...
int main() {
    ...
    many {
        int n;
        in(n);
        vi count(n + 1, 1), next(n + 2), pre(n + 2);
        lp(i, 1, n) next[i] = i + 1, pre[i] = i - 1;
        multiset<int> counts;
        lp(i, 1, n) counts.insert(1);
        vi p(n + 1);
        lp(i, 1, n) { p[in<int>()] = i; }
        int fail = 0;
        lp(i, 1, n) {
            if (count[p[i]] != *counts.rbegin()) {
                fail = 1;
                break;
            }
            counts.erase(counts.find(count[p[i]]));
            next[pre[p[i]]] = next[p[i]];
            pre[next[p[i]]] = pre[p[i]];
            if (next[p[i]] != n + 1) {
                counts.erase(counts.find(count[next[p[i]]]));
                count[next[p[i]]] += count[p[i]];
                counts.insert(count[next[p[i]]]);
            }
        }
        ou(fail ? "No" : "Yes");
    }
