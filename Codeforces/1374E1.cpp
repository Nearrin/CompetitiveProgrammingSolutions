...
int main() {
    ...
    sort(a), sort(b), sort(both);
    auto fix = [](vi &t) { lv(i, t) if (i) t[i] += t[i - 1]; };
    fix(a), fix(b), fix(both);
    int ans = ~0u >> 1;
    if (a.size() + both.size() < k || b.size() + both.size() < k) {
        ou(-1);
    } else {
        if (a.size() >= k && b.size() >= k)
            tmin(ans, a[k - 1] + b[k - 1]);
        lv(i, both) {
            auto aa = [&](int j) { return j >= 0 ? a[j] : 0; };
            auto bb = [&](int j) { return j >= 0 ? b[j] : 0; };
            if (a.size() + i + 1 >= k && b.size() + i + 1 >= k) {
                tmin(ans, both[i] + aa(k - i - 1 - 1) + bb(k - i - 1 - 1));
            }
        }
        ou(ans);
    }
