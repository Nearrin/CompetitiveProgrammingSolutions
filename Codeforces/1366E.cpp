    ...
    if (min(a) < b[0])
        ...
        if (fail)
            ...
            lv(i, b) {
                if (i) {
                    vi x;
                    lp(j, r[i - 1], r[i]) x.push_back(a[j]);
                    vi y = x;
                    lv(j, x) if (j) tmin(x[j], x[j - 1]);
                    rv(j, y) if (j + 1 != y.size()) tmin(y[j], y[j + 1]);
                    int cnt = 0;
                    lv(j,
                       x) if (j && x[j - 1] == b[i - 1] && y[j] == b[i])++ cnt;
                    ans *= cnt;
                }
            }
            int tmp = inf();
            lp(i, r[m - 1], n - 1) tmin(tmp, a[i]);
            if (tmp < b[m - 1])
                ans = 0;
            ou(ans);
        }
