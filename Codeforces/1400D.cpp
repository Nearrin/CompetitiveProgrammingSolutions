...
ll dfs22(vi &a, int l, int r) {
    static int cl[3010 * 3010];
    static int cr[3010 * 3010];
    auto hash = [](int x, int y) { return x * 3001 + y; };
    int m = (l + r) / 2;
    lp(i, l, m) lp(j, i + 1, m)++ cl[hash(a[i], a[j])];
    lp(i, m + 1, r) lp(j, i + 1, r)++ cr[hash(a[i], a[j])];
    ll ans = 0;
    lp(i, l, m) lp(j, i + 1, m) {
        if (cl[hash(a[i], a[j])]) {
            ans += ll(cl[hash(a[i], a[j])]) * cr[hash(a[i], a[j])];
            cl[hash(a[i], a[j])] = 0;
        }
    }
    lp(i, m + 1, r) lp(j, i + 1, r) cr[hash(a[i], a[j])] = 0;
    return ans;
}
ll dfs13(vi &a, int l, int r) {
    static int cl[3010];
    static int cr[3010];
    ll ans = 0;
    int m = (l + r) / 2;
    ...
    lp(i, m + 1, r) {
        --cr[a[i]];
        lp(j, m + 1, i - 1) { ans += ll(cl[a[i]]) * cr[a[j]]; }
    }
    lp(i, l, m)-- cl[a[i]];
    return ans;
}
ll dfs31(vi &a, int l, int r) {
    static int cl[3010];
    static int cr[3010];
    ll ans = 0;
    int m = (l + r) / 2;
    ...
    lp(i, l, m) {
        --cl[a[i]];
        lp(j, l, i - 1) { ans += ll(cl[a[j]]) * cr[a[i]]; }
    }
    lp(i, m + 1, r)-- cr[a[i]];
    return ans;
}
ll dfs(vi &a, int l, int r) {
    if (l == r)
        return 0;
    int m = (l + r) / 2;
    ll ans = dfs(a, l, m) + dfs(a, m + 1, r);
    ans += dfs22(a, l, r);
    ans += dfs13(a, l, r);
    ans += dfs31(a, l, r);
    return ans;
