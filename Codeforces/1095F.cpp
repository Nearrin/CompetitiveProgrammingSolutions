...
bool operator<(const edge &a, const edge &b) { return a.we < b.we; }
int main() {
    fast, prec(12);
    int n, m;
    in(n, m);
    ...
    lv(i, a) {
        edge tmp;
        tmp.tp = 2;
        tmp.frm = i, tmp.to_idx = 0;
        tmp.to = b[tmp.to_idx].second;
        tmp.we = a[tmp.frm] + a[b[tmp.to_idx].second];
        // e.insert(tmp);
    }
    multiset<pair<ll, int>> comp;
    lv(i, a) { comp.insert({a[i], i}); }
    ...
    while (cnt != n - 1) {
        ...
        auto v = *comp.begin();
        comp.erase(comp.begin());
        if (ufs.find(u.second) == ufs.find(v.second)) {
            comp.insert({min(u.first, v.first), u.second});
            continue;
        }
        while (e.size() && ufs.find(e.begin()->frm) == ufs.find(e.begin()->to))
            e.erase(e.begin());
        if (e.size() && e.begin()->we < u.first + v.first) {
            comp.insert(u), comp.insert(v);
            auto eg = *e.begin();
            e.erase(e.begin());
            ufs.link(eg.frm, eg.to);
            debu(1, eg.we);
            ans += eg.we;
            ++cnt;
        } else {
            ufs.link(u.second, v.second);
            comp.insert({min(u.first, v.first), u.second});
            ans += u.first + v.first;
            debu(1, u.first + v.first);
            ++cnt;
        }
    }
    ou(ans);
