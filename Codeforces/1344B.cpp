...
int main() {
    ...
    int n, m;
    in(n, m);
    union_find_set::union_find_set<> f(n * m);
    vector<str> a(n);
    in(a);
    vi row_cnt(n), col_cnt(m);
    lp(i, 0, n - 1) lp(j, 0, m - 1) if (a[i][j] == '#') row_cnt[i] = 1,
                                                        col_cnt[j] = 1;
    if (sum(row_cnt) == n && sum(col_cnt) != m ||
        sum(row_cnt) != n && sum(col_cnt) == m) {
        ou(-1);
        return 0;
    }
    lp(i, 0, n - 1) {
    ...
    lp(i, 0, n - 1) lp(j, 0, m - 1) {
        if (i && a[i - 1][j] == a[i][j] && a[i][j] == '#') {
            f.link(i * m + j, i * m + j - m);
        }
        if (j && a[i][j - 1] == a[i][j] && a[i][j] == '#') {
            f.link(i * m + j, i * m + j - 1);
        }
    }
    unordered_set<int> ans;
    lp(i, 0, n - 1) lp(j, 0, m - 1) {
        if (a[i][j] == '#')
            ans.insert(f.find(i * m + j));
    }
    ou(ans.size());
    return 0;
}
