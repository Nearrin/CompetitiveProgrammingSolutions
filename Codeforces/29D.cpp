...
set<int> inlf;
vi ans;
void flatten(int u, int p, vi &tmp) {
    tmp.push_back(u);
    for (auto v : e[u]) {
        if (v != p) {
            flatten(v, u, tmp);
...
int check(int u, int p, int l) {
    vi tmp;
    flatten(u, p, tmp);
    return find(rang(tmp), l) != tmp.end();
}
void dfs(int u, int p) {
    ...
    if (inlf.count(u))
        lfans.push_back(u);
    while (true) {
        while (leaf_queue.size() &&
               find(rang(ans), leaf_queue.front()) != ans.end()) {
            leaf_queue.pop();
        }
        vpii chd;
        for (auto v : e[u]) {
            if (v != p && find(rang(ans), v) == ans.end()) {
                int tmp = leaf_queue.size() ? leaf_queue.front() : -1;
                chd.emplace_back(check(v, u, tmp), v);
            }
        }
        // ou(chd);
        sort(chd), reve(chd);
        if (chd.size())
            dfs(chd[0].second, u), ans.push_back(u);
        else
            break;
    }
}
