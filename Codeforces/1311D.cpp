...
int main() {
    ...
    lp(i, 1, 50000 - 1) {
        d[i] = divisor::divisor(
            integer_factorization_3::shankss_square_forms_factorization(i));
        sort(d[i]);
    }
    many {
        int a, b, c;
        ...
        int ans = inf();
        vi ans2;
        for (int newc = c; newc - c < ans; ++newc) {
            for (auto newb : d[newc])
                for (auto newa : d[newb]) {
                    int tmp = abs(newa - a) + abs(newb - b) + abs(newc - c);
                    if (tmp < ans) {
                        ans = tmp;
                        ans2 = {newa, newb, newc};
                    }
        ...
        for (int newc = c; newc >= 1 && c - newc < ans; --newc) {
            for (auto newb : d[newc])
                for (auto newa : d[newb]) {
                    int tmp = abs(newa - a) + abs(newb - b) + abs(newc - c);
                    if (tmp < ans) {
