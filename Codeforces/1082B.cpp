...
int main() {
    ...
    if (b.size() == 0)
        ou(0);
    elif (b.size() == 1) ou(b.back().second - b.back().first + 1);
    elif (b.size() == 2) {
        if (b[0].second + 2 < b[1].first) {
            ou(max(b[0].second - b[0].first, b[1].second - b[1].first) + 2);
        } else {
            ou(b[0].second - b[0].first + b[1].second - b[1].first + 2);
        }
    }
    else {
        int ans = 0;
        lv(i, b) {
            tmax(ans, b[i].second - b[i].first + 2);
            if (i && b[i - 1].second + 2 == b[i].first) {
                tmax(ans, b[i - 1].second - b[i - 1].first + b[i].second -
                              b[i].first + 3);
            }
        }
        ou(ans);
