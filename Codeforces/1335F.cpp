...
int main() {
    ...
    many {
        ...
        in(c, s);
        vvi has0, has1;
        auto vis = vect<int>({}, n, m);
        auto pos = vect<pii>({}, n, m);
        int tim = 0;
        lp(i, 0, n - 1) lp(j, 0, m - 1) {
            ...
            if (vis[mi][mj] == tim) {
                int k = 0;
                while (trace[k] != make_pair(mi, mj))
                    ++k;
                int clen = trace.size() - k;
                has0.push_back(vi(clen));
                has1.push_back(vi(clen));
                int cid = has0.size() - 1;
                lv(l, trace) {
                    pos[trace[l].first][trace[l].second] = {cid, l % clen};
                    if (c[trace[l].first][trace[l].second] == '0')
                        ++has0[cid][l % clen];
                    else
                        ++has1[cid][l % clen];
                }
            } else {
                int cid = pos[mi][mj].first;
                int oir = pos[mi][mj].second;
                int clen = has0[cid].size();
                int tt = 0;
                rv(l, trace) {
                    ++tt;
                    pos[trace[l].first][trace[l].second] = {
                        cid, ((oir - tt) % clen + clen) % clen};
                    if (c[trace[l].first][trace[l].second] == '0')
                        ++has0[cid][((oir - tt) % clen + clen) % clen];
                    else
                        ++has1[cid][((oir - tt) % clen + clen) % clen];
                }
            }
        }
        int ans1 = 0, ans2 = 0;
