...
int bf(const vvi &chd, const vi &l, const vi &r) {
    int n = chd.size() - 1;
    auto e = vect<int>(0, n + 1, n + 1);
    vi pr(n + 1);
    for (int i = 1; i <= n; ++i) {
        for (auto j : chd[i])
            pr[j] = i;
    ...
    lp(i, 1, n) {
        for (auto j = pr[i]; j; j = pr[j])
            e[i][j] = e[j][i] = 1;
    }
    lp(i, 1, n) for (int j = 1; j <= n; ++j) {
        int tmp = 1;
        if (l[i] <= l[j] && r[i] >= r[j])
            tmp = 0;
        if (l[j] <= l[i] && r[j] >= r[i])
    ...
    lp(m, 0, (1 << n) - 1) {
        if (bcnt(m) <= ans)
            continue;
        int fail = 0;
        lp(i, 1, n) if (barr(m, i - 1)) lp(j, 1, n) if (barr(m, j - 1)) {
            if (!e[i][j]) {
                fail = 1;
                goto done;
            }
        ...
        auto check = [&](int size) {
            ...
            function<void(int)> dfs = [&](int x) {
                if (have == 1)
                    return;
                stk.push_back(x);
                bad += sum(subtree, r[x]) - sum(subtree, l[x] - 1);
                bad += sum(parent, l[x]);
                add(parent, l[x], 1);
                add(parent, r[x] + 1, -1);
                add(subtree, l[x], 1);
                if (stk.size() > size) {
                    int x = stk[stk.size() - size - 1];
                    add(parent, l[x], -1);
                    add(parent, r[x] + 1, 1);
                    add(subtree, l[x], -1);
                    bad -= sum(subtree, r[x]) - sum(subtree, l[x] - 1);
