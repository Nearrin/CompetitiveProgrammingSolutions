...
int main() {
    ...
    lp(i, 0, n - 1) {
        ...
        lp(j, i, n - 1) {
            if (!qu.size())
                qu.push_back(a[j]);
            elif (qu.back() < a[j]) fail = 1;
            elif (qu.back() == a[j]) {
                qu.pop_back(), qu.push_back(a[j] + 1);
                while (qu.size() > 1 && qu.back() == qu[qu.size() - 2]) {
                    qu.pop_back();
                    ++qu.back();
                }
            }
            else qu.push_back(a[j]);
            if (fail || qu.size() != 1)
                can[i][j] = 0;
            else
                can[i][j] = 1;
    ...
    lp(i, 0, n - 1) {
        if (can[0][i])
            ...
            dp[i] = inf() / 2;
        lp(j, 0, i - 1) if (can[j + 1][i]) { tmin(dp[i], dp[j] + 1); }
    }
    ou(dp.back());
    return 0;
}
