...
void dfs(int u, int p) {
    ...
    for (auto v : e[u])
        if (v != p) {
            dfs(v, u);
            tmax(f[u][0], f[v][0] + d[u] - 2);
            tmax(f[u][0], f[v][1] + d[u] - 1);
            tmax(f[u][1], f[v][0]);
            c0.push_back(v);
            c1.push_back(v);
        }
    sort(c0, [&](int x, int y) { return f[x][0] > f[y][0]; });
    sort(c1, [&](int x, int y) { return f[x][1] > f[y][1]; });
    vi c;
    lv(i, c0) if (i < 2) c.push_back(c0[i]);
    lv(i, c1) if (i < 2) c.push_back(c1[i]);
    lv(i, c) {
        lv(j, c) if (i < j && c[i] != c[j]) {
            lp(si, 0, 1) lp(sj, 0, 1) {
                // s=1
                if (si == 0 && sj == 0) {
                    tmax(ans, 1 + f[c[i]][si] + f[c[j]][sj] - 2);
                }
                // s=0
                int tmp = d[u];
                if (si == 0)
