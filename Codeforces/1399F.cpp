...
int change(vpii &a) {
    ...
    uniq(b);
    for (auto &[u, v] : a)
        u = lower_bound(rang(b), u) - b.begin(),
        v = lower_bound(rang(b), v) - b.begin();
    return b.size();
}
int main() {
    ...
    many {
        ...
        for (auto [u, v] : a)
            b[u].push_back(v);
        lv(i, b) sort(b[i]);
        auto f = vect<int16_t>(-1, 2, m, m);
        function<int16_t(int, int, int)> dfs = [&](int d, int l, int r) {
            if (l > r)
                return int16_t(0);
            int16_t &g = f[d][l][r];
            if (g != -1)
                return g;
            if (b[l].empty() || b[l].front() > r)
                return g = dfs(0, l + 1, r);
            g = dfs(0, l + 1, r);
            for (int t : b[l]) {
                if (t > r)
                    break;
                if (d == 1 && t == r)
                    continue;
                if (g < (int16_t)(1) + (int16_t)dfs(1, l, t) +
                            (int16_t)dfs(0, t + 1, r))
                    g = (int16_t)(1) + (int16_t)dfs(1, l, t) +
                        (int16_t)dfs(0, t + 1, r);
            }
            // ou(l, r, g);
            return g;
