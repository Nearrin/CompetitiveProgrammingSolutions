    ...
    if (fail) {
        ...
        rp(i, m, 1) {
            if (i == m)
                ...
                reach[i] = max(ans[i] + l[i] - 1, reach[i + 1]);
        }
        vi delta(m + 1);
        int fail = 1;
        lp(i, 2, m) {
        ...
        if (fail)
            ...
            vi tmp2;
            lp(i, 1, m) tmp2.push_back(ans[i]);
            return tmp2;
        }
    }
}
bool check(int n, int m, vi l, vi ans) {
    if (ans[0] == -1)
        return true;
    vi color(n, -1);
    lp(i, 1, m) {
        int b = ans[i - 1], e = ans[i - 1] + l[i - 1] - 1;
        lp(j, b, e) color[j - 1] = i;
    }
    if (min(color) == -1)
        return false;
    set<int> tmp;
...
int main() {
    ...
    return 0;
    while (true) {
        int n = 100, m = rint(1, n);
        ou(n, m);
        vi l;
        lp(i, 1, m) { l.push_back(rint(1, n)); }
        ou(l);
        auto ans = solve(n, m, l);
        ou(ans);
        assert(check(n, m, l, ans));
    }
    return 0;
}