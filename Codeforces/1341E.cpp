...
int main() {
    ...
    while (q.size()) {
        ...
        if (t == 0) {
            ...
            if (i != m - 1 && t - (d[i + 1] - d[i]) >= 0 &&
                f[i + 1][t - (d[i + 1] - d[i])] == -1) {
                f[i + 1][t - (d[i + 1] - d[i])] = f[i][t] + d[i + 1] - d[i];
                q.push_front({i + 1, t - (d[i + 1] - d[i])});
            }
            if (i && t - (d[i] - d[i - 1]) >= 0 &&
                f[i - 1][t - (d[i] - d[i - 1])] == -1) {
                f[i - 1][t - (d[i] - d[i - 1])] = f[i][t] + d[i] - d[i - 1];
                q.push_front({i - 1, t - (d[i] - d[i - 1])});
            }
        }
    }
    ll ans = inf<ll>();
    lp(i, 0, g) if (f[m - 1][i] != -1) tmin(ans, f[m - 1][i]);
    if (ans != inf<ll>())
        ou(ans);
    else
        ou(-1);
