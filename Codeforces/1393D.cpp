...
int main() {
    ...
    static int h[4010][4010];
    lp(i, 0, n - 1) lp(j, 0, m - 1) { c[i + j][i - j + m - 1] = a[i][j]; }
    n = m + n - 1;
    lp(i, 0, n - 1) lp(j, 0, n - 1) {
        if (i && i + 1 < n && c[i - 1][j] == c[i + 1][j] && c[i + 1][j] &&
            !c[i][j]) {
            c[i][j] = c[i - 1][j];
        }
        if (j && j + 1 < n && c[i][j - 1] == c[i][j + 1] && c[i][j + 1] &&
            !c[i][j]) {
            c[i][j] = c[i][j + 1];
        }
    }
    ...
    lp(i, 0, n - 1) {
        ...
        lp(j, 0, n - 1) {
            ...
            q.push_back(j);
            while (j - q.front() + 1 > h[i][q.front()])
                st = q.front() + 1, q.pop_front();
            int l = min(j - st + 1, h[i][q.front()]);
            if (l % 2 == 0)
                --l;
            if ((i + j) % 2 == m % 2)
                continue;
