...
int main() {
    ...
    lv(i, ide) ide[i] = cnt++;
    assert(cnt == n + m + 2);
    maximum_flow::maximum_flow<ll> flow(cnt, src, snk);
    lp(i, 0, n - 1) {
        int a;
        in(a);
        flow.add_edge(idv[i], snk, a);
    }
    lp(i, 0, m - 1) {
        int u, v, w;
        in(u, v, w);
        --u, --v;
        flow.add_edge(ide[i], idv[u], inf<ll>());
        flow.add_edge(ide[i], idv[v], inf<ll>());
        flow.add_edge(src, ide[i], w);
        ans += w;
    }
    ou(ans - flow.run());
    return 0;
