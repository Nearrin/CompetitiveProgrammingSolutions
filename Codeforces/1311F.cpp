...
int main() {
    ...
    vpll a(n);
    vl all_v;
    lp(i, 0, n - 1) in(a[i].first);
    lp(i, 0, n - 1) in(a[i].second), all_v.push_back(a[i].second);
    uniq(all_v);
    m = all_v.size();
    fs.resize(m + 1);
    fc.resize(m + 1);
    auto vidx = [&all_v](ll v) {
        return lower_bound(rang(all_v), v) - begin(all_v) + 1;
    };
    sort(a);
    ll ans = 0;
    lp(i, 0, n - 1) {
        int vid = vidx(a[i].second);
        auto tmp = que(vid);
        ans += tmp.first * a[i].first - tmp.second;
        add(vid, a[i].first);
    }
    ou(ans);
    return 0;
