...
int main() {
    ...
    lp(i, 1, m) {
        ...
        if (u == 0) {
            ...
            unrelated[u].insert(v);
            unrelated[v].insert(u);
        }
    }
    int ans = 0;
    lp(i, 1, n - 1) { cnt_heap.insert({cnt[i], i}); }
    lp(i, 1, n - 1) {
        if (cnt_heap.begin()->first == i)
            ++ans;
        int u = cnt_heap.begin()->second;
        cnt_heap.erase(cnt_heap.begin());
        for (auto v : unrelated[u]) {
            unrelated[v].erase(u);
            cnt_heap.erase({cnt[v], v});
            cnt_heap.insert({++cnt[v], v});
        }
    }
    ou(ans);
    return 0;
