...
bool check(str s, vvi &next, str a, str b) {
    vvi dp(a.size() + 1, vi(b.size() + 1));
    lp(i, 0, a.size()) lp(j, 0, b.size()) {
        if (i == 0 && j == 0)
            dp[i][j] = -1;
        elif (i == 0) dp[i][j] = next[dp[i][j - 1] + 1][b[j - 1] - 'a'];
        elif (j == 0) dp[i][j] = next[dp[i - 1][j] + 1][a[i - 1] - 'a'];
        else dp[i][j] = min(next[dp[i][j - 1] + 1][b[j - 1] - 'a'],
                            next[dp[i - 1][j] + 1][a[i - 1] - 'a']);
    }
...
int main() {
    ...
    many {
        string s, t;
        in(s, t);
        vvi next(s.size() + 2, vi(26));
        lv(i, s) {
            lp(j, 0, 25) {
                int k = i;
                while (k < s.size() && s[k] != 'a' + j)
                    ++k;
                next[i][j] = k;
            }
        }
        lp(i, 0, 25) next[s.size()][i] = next[s.size() + 1][i] = s.size();
        int fd = 0;
        for (int i = 0; i < t.size(); ++i) {
            str a = t.substr(0, i + 1), b = t.substr(i + 1, t.size() - i - 1);
            if (check(s, next, a, b)) {
                fd = 1;
                break;
            }
