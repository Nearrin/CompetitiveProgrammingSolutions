...
int main() {
    ...
    many {
        ...
        vi tmp;
        in(tmp);
        integer_factorization_1::linear_sieve factor(max(tmp));
        for (int t : tmp) {
            auto fact = factor.run(t);
            for (auto tt : fact)
                lp(j, 1, tt.second) parts.push_back(tt.first);
        }
        while (parts.size() < n - 1) {
            parts.push_back(1);
        }
        sort(parts);
        ll ans = 0;
        lv(i, coef) {
            ll cur = coef[i] * parts[i];
            cur %= int(1e9 + 7);
            if (i + 1 == coef.size()) {
                lp(j, i + 1, parts.size() - 1) {
                    cur *= parts[j];
                    cur %= int(1e9 + 7);
                }
            }
