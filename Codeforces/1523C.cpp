...
void print(node *x, string prefix, int cur) {
    if (!x)
        return;
    auto name = prefix + conv<int, str>(cur);
    ou(name);
    print(x->son, name + ".", 1);
    print(x->bro, prefix, cur + 1);
    delete x;
}
int main() {
    ...
    many {
        ...
        in(a);
        vector<node *> b;
        b.push_back(new node{nullptr, nullptr, 1});
        auto root = b.front();
        lv(i, a) if (i) {
            if (a[i] == 1) {
                auto x = new node{nullptr, nullptr, 1};
                b.back()->son = x;
                b.push_back(x);
            } else {
                while (b.back()->cur + 1 != a[i])
                    b.pop_back();
                auto x = new node{nullptr, nullptr, a[i]};
                b.back()->bro = x;
                b.back() = x;
            }
        }
