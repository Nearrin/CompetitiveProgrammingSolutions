...
int main() {
    ...
    while (cur.size()) {
        ...
        lv(i, cur) {
            ...
            steps.back().push_back(cur[i]);
            if (cur[i] != 1) {
                if (i && cur[i - 1] + 2 == cur[i]) {
                } else {
                    if (a[cur[i] - 1] == 'R') {
                        new_cur.push_back(cur[i] - 1);
                    }
                }
            }
            if (cur[i] + 2 <= n) {
                char nc;
                if (i + 1 < cur.size() && cur[i] + 2 == cur[i + 1])
                    nc = 'L';
                else
                    nc = a[cur[i] + 2];
                if (nc == 'L')
    ...
    int mi_op = steps.size(), mx_op = 0;
    lv(i, steps) mx_op += steps[i].size();
    if (k < mi_op || k > mx_op) {
        ou(-1);
    } else {
        vvi ans;
