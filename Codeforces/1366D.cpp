...
pii f(int n) {
    lp(d1, 2, n) lp(d2, 2, n) {
        if (gcd(d1 + d2, n) == 1 && n % d1 == 0 && n % d2 == 0) {
            return {d1, d2};
        }
    }
    return {-1, -1};
}
integer_factorization_1::linear_sieve ls(int(1e7));
pii g(int n) {
    auto t = ls.run(n);
    if (t.size() <= 1)
        return {-1, -1};
    int u = t[0].first;
    for (auto v : divisor::divisor(t))
        if (gcd(u + v, n) == 1 && v > 1)
            return {u, v};
    return {-1, -1};
}
