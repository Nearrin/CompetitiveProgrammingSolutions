...
int main() {
    ...
    lp(cnt1, 0, 2) lp(cnt2, 0, 2) {
        ...
        if (cnt1 + cnt2 == a.size())
            continue;
        lp(i, 1, cnt1) fb[pos[1][i - 1]] = 1;
        lp(i, 1, cnt2) fb[pos[2][i - 1]] = 1;
        string tmp;
        int thi = cnt1 + cnt2;
        int sm = 0;
        lv(i, a) if (!fb[i]) tmp.push_back(a[i]), sm += a[i] - '0';
        int j = 0;
        string tmp2;
        while (j + 1 != tmp.size() && tmp[j] == '0')
            ++j;
        lp(i, j, tmp.size() - 1) tmp2.push_back(tmp[i]);
        thi += j;
        if (sm % 3 == 0) {
            if (ans == -1)
                ans = thi, ans2 = tmp2;
            else if (tmin(ans, thi))
                ans2 = tmp2;
        }
        lp(i, 1, cnt1) fb[pos[1][i - 1]] = 0;
        lp(i, 1, cnt2) fb[pos[2][i - 1]] = 0;
    }
    if (ans != -1)
        cout << ans2 << endl;
    else
