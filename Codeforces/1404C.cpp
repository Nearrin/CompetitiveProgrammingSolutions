...
int main() {
    ...
    vector<statistic::add_min<int>> tmp1;
    lv(i, a) tmp1.push_back(statistic::add_min(inf(), i));
    segment_tree::segment_tree<statistic::add_min<int>> tree1(tmp1);
    vector<statistic::add_sum<int>> tmp2;
    lv(i, a) tmp2.push_back(statistic::add_sum(0));
    segment_tree::segment_tree<statistic::add_sum<int>> tree2(tmp2);
    rv(i, a) {
        if (a[i] <= i + 1) {
            tree1.modify_single(i, statistic::add_min<int>(i + 1 - a[i], i));
        }
        while (tree1.query_segment(0, n - 1).v <= 0) {
            int i = tree1.query_segment(0, n - 1).i;
            tree2.modify_single(i, statistic::add_sum<int>(1));
            tree1.modify_single(i, statistic::add_min<int>(inf(), i));
            int tmp = -1;
            tree1.modify_segment(i, n - 1, tmp);
        }
        for (auto t : qs[i]) {
            int j = t.first;
            ans[t.second] = tree2.query_segment(i, j).s;
        }
    }
    for (auto t : ans)
        ou(t);
