...
void transform(vector<T> &a, const int &n, const int &s, const vector<int> &r) {
    ...
    for (int i = 0; i < n; ++i)
        if (i < r[i])
            swap(a[i], a[r[i]]);
    for (int i = 1, m = 1; 1 << i <= n; ++i, m <<= 1) {
        vector<T> wmk(m, 1);
        for (int k = 1; k < m; ++k)
            wmk[k] = multiply(wmk[k - 1], wm[i], p);
        for (int j = 0; j < n; j += m << 1)
            for (int k = 0; k < m; ++k) {
                T u = a[j + k], v = multiply(wmk[k], a[j + k + m], p);
                a[j + k] = (u + v) % p, a[j + k + m] = (u - v + p) % p;
            }
    }
}
...
int main() {
    ...
    // solve([&](int i) { return a[i] * a[i] * b[i] * b[i]; },
    //      [&](int i) { return intm<>(1); });
    solve(
    ...
    // solve([&](int i) { return intm<>(1); },
    //      [&](int i) { return c[i] * c[i] * c[i] * c[i]; });
