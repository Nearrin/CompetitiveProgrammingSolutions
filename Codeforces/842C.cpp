...
int main() {
    ...
    int m = max(a);
    vvi a_to_nodes(m + 1);
    lp(i, 0, n - 1) a_to_nodes[a[i]].push_back(i);
    vi ans(n);
    lp(i, 1, m) {
        vi cur;
        for (int j = i; j <= m; j += i) {
            for (auto t : a_to_nodes[j]) {
                cur.push_back(t);
            }
        }
        sort(cur, [](int u, int v) { return depth[u] < depth[v]; });
        for (auto u : cur) {
            if (u == 0) {
                info[u] = 2;
            } else if (info[parent[u]] > 0) {
                info[u] = info[parent[u]];
            } else if (parent[parent[u]] == -1) {
                info[u] = 1;
            } else if (info[parent[parent[u]]] == 2) {
                info[u] = 1;
            }
        }
        for (auto u : cur) {
    ...
    lp(i, 1, n - 1) { tmax(ans[i], toson[parent[i]]); }
    cout << ans << endl;
    return 0;
}
