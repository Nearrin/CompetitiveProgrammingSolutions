...
int main() {
    ...
    many {
        ...
        std::priority_queue<pair<ll, pll>, vector<pair<ll, pll>>, cmp> a;
        ll curs = 0;
        function<int(int, int)> dfs = [&](int u, int p) {
            ...
            for (auto t : e[u])
                if (t.first != p) {
                    ...
                    a.push({ceil(ll(t.second), 2ll) * tsiz, {t.second, tsiz}});
                    curs += ll(t.second) * tsiz;
                    siz += tsiz;
                }
            if (siz == 0)
                ++siz;
        ...
        while (curs > s) {
            ...
            tt.second.first /= 2;
            if (tt.second.first)
                a.push({ceil(ll(tt.second.first), 2ll) * tt.second.second,
