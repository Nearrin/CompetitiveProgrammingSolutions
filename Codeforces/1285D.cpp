...
int work(vi &a, int l, int r, int d) {
    if (d == 0) {
        return 0;
    }
    static int q1[100010], c1, q2[100010], c2;
    c1 = 0;
    c2 = 0;
    lp(i, l, r) {
        if ((a[i] >> (d - 1)) & 1)
    ...
    lp(i, 0, c1 - 1) a[l + i] = q1[i];
    lp(i, 0, c2 - 1) a[l + c1 + i] = q2[i];
    int nl = l, nr = l + c1 - 1, ml = nr + 1, mr = r;
    if (nl > nr || ml > mr) {
        return work(a, l, r, d - 1);
    }
    int tmp1 = work(a, nl, nr, d - 1), tmp2 = work(a, ml, mr, d - 1);
    return (1 << (d - 1)) + min(tmp1, tmp2);
}
int main() {
    fast;
    int n;
