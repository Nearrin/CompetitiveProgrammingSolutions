...
int main() {
    fast, prec(12);
    auto left = vect<int>({-1}, 1 << 15, 16);
    lp(t, 1, (1 << 15) - 1) lp(p, 0, 15) {
        if (t < 1 << p)
            break;
        left[t][p] = p + 1;
        while (!barr(t, left[t][p] - 1))
            ++left[t][p];
    }
    many {
        int n;
        ...
        in(a);
        vi all(1 << n);
        lp(s, 0, (1 << n) - 1) lv(i, a) if (barr(s, i)) all[s] += a[i];
        auto f = vect<int>({inf()}, 1 << n, n + 1, n + 1);
        auto g = vect<tuple<int, int, int>>({}, 1 << n, n + 1, n + 1);
        f[0][0][0] = -inf();
        lp(s, 0, (1 << n) - 2) {
            lp(l, 0, n - 1) {
                if (l > bcnt(s))
                ...
                lp(p, 0, n) {
                    ...
                    lm(t, ((1 << n) - 1) ^ s) {
                        ...
                        int new_v = all[t];
                        if (new_v > f[s][l][p] &&
                            tmin(f[s + t][l + 1][new_p], new_v))
                            g[s + t][l + 1][new_p] = {s, l, p};
                    }
                }
            }
        }
        int cur_s = (1 << n) - 1, cur_l = n, cur_p;
        while (true) {
            int fail = 1;
            lp(p, 1, n) if (f[cur_s][cur_l][p] != inf()) {
                fail = 0, cur_p = p;
                break;
            }
            if (!fail)
        ...
        ou(n - cur_l);
        vi numbers(n);
        lv(i, numbers) numbers[i] = i + 1;
        auto operate = [&](int i, int j) {
            int idi = find(rang(numbers), i) - numbers.begin() + 1;
            int idj = find(rang(numbers), j) - numbers.begin() + 1;
            ou(idi, idj);
            numbers.erase(numbers.begin() + idi - 1);
        };
        while (cur_s) {
            auto [next_s, next_l, next_p] = g[cur_s][cur_l][cur_p];
            lp(i, 1, n) if (i != cur_p && barr(cur_s - next_s, i - 1)) {
                operate(i, cur_p);
            }
            cur_s = next_s, cur_l = next_l, cur_p = next_p;
        }
    }
    return 0;
}
