...
int main() {
    ...
    in(a);
    a.push_back(0);
    auto f = vect<int16_t>(0, n, n + 1), g = f, h = f;
    lp(i, 0, n - 1) {
        lp(j, 0, n) {
            if (j > a[i])
                f[i][j] = inf<int16_t>();
            elif (i == 0) { f[i][j] = j + (j < a[i]); }
            else {
                f[i][j] = inf<int16_t>();
                tmin(f[i][j], int16_t(g[i - 1][j] + j + (j < a[i])));
                tmin(f[i][j], h[i - 1][j]);
                // ou(i, j, f[i][j]);
            }
        }
        lp(j, 0, n) {
            if (f[i][j] == inf<int16_t>())
                g[i][j] = inf<int16_t>();
            else
                g[i][j] = f[i][j] - j;
            if (j)
    ...
    int16_t ans = inf<int16_t>();
    lp(i, 0, n) tmin(ans, f[n - 1][i]);
    ou(ans);
    return 0;
}
