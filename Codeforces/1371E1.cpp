...
int main() {
    ...
    in(a);
    vi f(n + 1, 1);
    lp(i, 1, n) f[i] = f[i - 1] * i % p;
    auto c = vect<int>(0, n + 1, n + 1);
    lp(i, 0, n) lp(j, 0, i) {
        if (j == 0 || i == j)
            c[i][j] = 1;
        else
    ...
    lp(x, 0, max(a)) {
        vi r(n);
        lv(i, r) r[i] = max(a[i] - x, 0);
        sort(r);
        int ans = 1;
        lp(i, 0, n - 1) if (r[i] > i) ans = 0;
        if (ans == 0)
            continue;
        map<int, int> cnt;
        for (auto v : r)
        ...
        for (auto v : cnt)
            tt.push_back(v);
        reve(tt);
        int precnt = 0, prepos = -1;
        for (auto v : tt) {
            ans = ans * f[v.second] % p;
            if (prepos != -1)
                ans = ans * c[v.second][prepos - v.first] % p,
                ans = ans *
                      c[precnt + (v.second - (prepos - v.first))][precnt] % p;
            precnt += v.second;
            prepos = v.first;
        }
        if (ans) {
