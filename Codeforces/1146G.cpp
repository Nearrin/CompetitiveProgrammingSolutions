...
int main() {
    ...
    in(n, h, m);
    vi l(m), r(m), x(m), c(m);
    lp(i, 0, m - 1) in(l[i], r[i], x[i], c[i]), --l[i], --r[i];
    int cnt = -1;
    int src = ++cnt;
    int snk = ++cnt;
    vvi idx1(n, vi(h + 1));
    lp(i, 0, n - 1) lp(j, 0, h) idx1[i][j] = ++cnt;
    vi idx2(m);
    lp(i, 0, m - 1) idx2[i] = ++cnt;
    assert(++cnt == 2 + n * (h + 1) + m);
    maximum_flow::maximum_flow<int> mf(cnt, src, snk);
    lp(i, 0, n - 1) mf.add_edge(src, idx1[i][0], inf());
    lp(i, 0, n - 1) lp(j, 0, h) {
        if (j)
            mf.add_edge(idx1[i][j], idx1[i][j - 1], inf());
        if (j + 1 <= h)
            mf.add_edge(idx1[i][j], idx1[i][j + 1], h * h - j * j);
        lp(k, 0, m - 1) {
            if (l[k] <= i && r[k] >= i && x[k] + 1 == j)
                mf.add_edge(idx1[i][j], idx2[k], inf());
        }
    }
    lp(i, 0, m - 1) { mf.add_edge(idx2[i], snk, c[i]); }
    ou(n * h * h - mf.run());
    return 0;
}
