...
int main() {
    ...
    int n, k;
    in(n, k);
    vvi possible(1 << 7, vi(8, -1));
    vi value(1 << 7, -1);
    vector<string> numbers{"1110111", "0010010", "1011101", "1011011",
                           "0111010", "1101011", "1101111", "1010010",
                           "1111111", "1111011"};
    lv(i, numbers) value[bitset<7>(numbers[i]).to_ulong()] = i;
    lv(i, possible) {
        lp(j, 0, (1 << 7) - 1) {
            if ((i & j) == i && value[j] > -1) {
                tmax(possible[i][bcnt(j - i)], value[j]);
            }
        }
    }
    vector<bitset<2000 * 7 + 1>> f(n + 1);
    f[n][0] = 1;
    vi a(n);
    lp(i, 0, n - 1) {
        str t;
        in(t);
        a[i] = bitset<7>(t).to_ulong();
    }
    rp(i, n - 1, 0) {
        lp(j, 0, 7) {
            if (possible[a[i]][j] != -1) {
    ...
    if (!f[0][k])
        ...
        lv(i, a) {
            int ans = -1, ansj;
            lp(j, 0,
               7) if (possible[a[i]][j] != -1 && k >= j && f[i + 1][k - j]) {
                if (possible[a[i]][j] > ans) {
                    ans = possible[a[i]][j];
                    ansj = j;
                }
            }
