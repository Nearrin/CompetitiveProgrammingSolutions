...
vector<str> check(vector<str> all, str s) {
    // ou(all, s);
    vi suf(s.size()), pre(s.size());
    vector<str> ans;
    for (auto t : all) {
        if (!suf[t.size()] && s.substr(0, t.size()) == t) {
            suf[t.size()] = 1;
            ans.push_back("P");
            continue;
        }
        if (!pre[t.size()] && s.substr(s.size() - t.size(), t.size()) == t) {
            pre[t.size()] = 1;
            ans.push_back("S");
        }
    }
    if (ans.size() != all.size())
        ans.clear();
    return ans;
}
int main() {
    ...
    if (big[0].substr(1, n - 2) == big[1].substr(0, n - 2)) {
        auto ans = check(all, big[0] + big[1].substr(n - 2, 1));
        if (ans.size()) {
            for (auto t : ans)
                cout << t;
            ou();
    ...
    if (big[1].substr(1, n - 2) == big[0].substr(0, n - 2)) {
        auto ans = check(all, big[1] + big[0].substr(n - 2, 1));
        if (ans.size()) {
            for (auto t : ans)
                cout << t;
            ou();
