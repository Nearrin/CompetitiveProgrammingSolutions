...
struct node {
    ...
    int l, r;
    int mxl, mxr, mxs;
    int mxlp, mxrp, mxspl, mxspr;
    int mil, mir, mis;
    int milp, mirp, mispl, mispr;
    int rev, sum;
} * rt;
void down(node *u) {
    if (u->rev == 1) {
...
void update(node *u) {
    ...
    if (u->ch[0]->mxs >= u->ch[1]->mxs &&
        u->ch[0]->mxs >= u->ch[0]->mxr + u->ch[1]->mxl) {
        u->mxs = u->ch[0]->mxs;
        u->mxspl = u->ch[0]->mxspl;
        u->mxspr = u->ch[0]->mxspr;
    } else if (u->ch[1]->mxs >= u->ch[0]->mxs &&
               u->ch[1]->mxs >= u->ch[0]->mxr + u->ch[1]->mxl) {
        u->mxs = u->ch[1]->mxs;
    ...
    if (u->ch[0]->mis <= u->ch[1]->mis &&
        u->ch[0]->mis <= u->ch[0]->mir + u->ch[1]->mil) {
        u->mis = u->ch[0]->mis;
        u->mispl = u->ch[0]->mispl;
        u->mispr = u->ch[0]->mispr;
    } else if (u->ch[1]->mis <= u->ch[0]->mis &&
               u->ch[1]->mis <= u->ch[0]->mir + u->ch[1]->mil) {
        u->mis = u->ch[1]->mis;
...
int main() {
    ...
    lp(i, 1, q) {
        ...
        if (u == 0) {
            ...
            for (node t = query(rt, l, r); t.mxs > 0 && k > 0;
                 t = query(rt, l, r), --k) {
