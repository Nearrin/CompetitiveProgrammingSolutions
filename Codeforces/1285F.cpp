...
template <class T> vector<T> fact(T n) {
    vector<T> a;
    for (T i = 2; i * i <= n; ++i) {
        if (n % i == 0) {
            T t = 1;
...
void update(vi &cnt, const vvi &divisors, int v, int c) {
    for (auto t : divisors[v])
        cnt[t] += c;
}
int check(const vi &cnt, const vvi &divisors, const vi &mob, int v) {
    int ret = 0;
    for (auto t : divisors[v])
        ret += cnt[t] * mob[t];
    return ret;
...
int main() {
    ...
    lp(g, 1, m) {
        stack<int> s;
        for (int v = m / g * g; v > 0; v -= g) {
            if (!b[v])
                continue;
            // cout << vi{g, v} << endl;
            while (check(cnt, divisors, mobi, v / g)) {
                // cout << "upda " << v << " " << s.top() << endl;
                upda_g(ans, 1ll * v * s.top() / g);
                update(cnt, divisors, s.top() / g, -1);
                s.pop();
            }
            update(cnt, divisors, v / g, 1);
            s.push(v);
        }
        while (s.size()) {
            update(cnt, divisors, s.top() / g, -1);
