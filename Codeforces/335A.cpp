...
inline VALUE_TYPE PowMod(VALUE_TYPE a, POW_TYPE b, const VALUE_TYPE &c) {
...
int main() {
    ...
    vp(i, s)++ cnt[s[i]];
    int ty = 0;
    lp(i, 'a', 'z') if (cnt[i])++ ty;
    if (ty > n)
        cout << -1 << endl;
    else {
        lp(i, 'a', 'z') if (cnt[i])++ cnt2[i], --n;
        while (n) {
            int nowid = -1, nowans = -1;
            lp(i, 'a', 'z') {
                if (!cnt2[i])
                    continue;
                int t = cnt[i] / cnt2[i];
        ...
        cout << nowwans << endl;
        lp(i, 'a', 'z') lp(j, 1, cnt2[i]) cout << char(i);
        cout << endl;
    }
    // ps;
    return 0;
