...
int main() {
    ...
    in(a);
    vi one, one2, more;
    lv(i, a) if (a[i] > 1) more.push_back(i + 1);
    elif (one.size() >= 2) one2.push_back(i + 1);
    else one.push_back(i + 1);
    if (sum(a) < 2 * n - 2) {
        YEP(0);
    } else {
        ou("YES", one.size() + more.size() - 1);
        ou(n - 1);
        if (one.size() > 0)
            ou(one[0], more[0]), --a[more[0] - 1];
        lv(i, more) {
            if (i)
                ou(more[i], more[i - 1]), --a[more[i] - 1],
                    --a[more[i - 1] - 1];
        }
        if (one.size() > 1)
            ou(more.back(), one[1]), --a[more.back() - 1];
        int ptr = 0;
        for (auto u : one2) {
            while (a[more[ptr] - 1] == 0)
                ++ptr;
            ou(u, more[ptr]), --a[more[ptr] - 1];
        }
    }
    return 0;
}
