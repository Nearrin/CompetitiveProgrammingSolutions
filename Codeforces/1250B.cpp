...
int main() {
    ...
    vpii d;
    for (auto v : c)
        d.emplace_back(v.first, v.second);
    sort(d);
    auto cost = [&](int s) {
        auto td = d;
        // ou(s, td);
        ...
        while (l <= r) {
            if (l == r) {
                if (td[l].first > s)
                    return inf<ll>();
                if (td[l].first * 2 <= s)
                    ans += ll(td[l].second + 1) / 2 * s;
                else
                    ans += ll(td[l].second) * s;
                ++l, --r;
            } else {
                if (td[l].first + td[r].first <= s) {
                    int cnt = min(td[l].second, td[r].second);
                    td[l].second -= cnt;
                    td[r].second -= cnt;
                    ans += ll(cnt) * s;
                    if (td[l].second == 0)
    ...
    ll ans = inf<ll>();
    int mi = max(b), mx = 2 * mi;
    lp(s, mi, mx) tmin(ans, cost(s));
    ou(ans);
    return 0;
}
