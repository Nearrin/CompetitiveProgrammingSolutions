...
void transpose(int &n, int &m, vector<str> &a) {
    vector<str> b(m, str(n, '0'));
    lv(i, a) lv(j, a[i]) b[j][i] = a[i][j];
    a = b;
    swap(n, m);
}
bool check(int u, int v, int m) {
    if (m == 2) {
        return (barr(u, 0) + barr(u, 1) + barr(v, 0) + barr(v, 1)) % 2 == 1;
    } else {
        return (barr(u, 0) + barr(u, 1) + barr(v, 0) + barr(v, 1)) % 2 == 1 &&
               (barr(u, 2) + barr(u, 1) + barr(v, 2) + barr(v, 1)) % 2 == 1;
    }
}
int main() {
    ...
    if (m >= 4) {
        ...
        auto f = vect<int>(0, (1 << m), n);
        int ans = inf();
        lp(i, 0, n - 1) {
            lp(j, 0, (1 << m) - 1) {
                int t = 0;
                lp(k, 0, m - 1) if (a[i][k] != ((j >> k) & 1) + '0')++ t;
                if (i == 0) {
                    f[j][i] = t;
                } else {
                    f[j][i] = inf();
