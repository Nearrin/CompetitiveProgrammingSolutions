...
int f(vi a, vi b, vi c) {
    sort(a), sort(b), sort(c);
    reve(a), reve(b), reve(c);
    auto f = vect<int>(numeric_limits<int>::min(), a.size() + 1, b.size() + 1,
                       c.size() + 1);
    int ans = 0;
    f[0][0][0] = 0;
    lp(i, 0, a.size()) lp(j, 0, b.size()) lp(k, 0, c.size()) {
        if (f[i][j][k] == numeric_limits<int>::min())
            continue;
        tmax(ans, f[i][j][k]);
        if (i < a.size() && j < b.size())
            tmax(f[i + 1][j + 1][k], f[i][j][k] + a[i] * b[j]);
        if (i < a.size() && k < c.size())
            tmax(f[i + 1][j][k + 1], f[i][j][k] + a[i] * c[k]);
        if (k < c.size() && j < b.size())
            tmax(f[i][j + 1][k + 1], f[i][j][k] + c[k] * b[j]);
    }
    return ans;
}
int main() {
