...
#define sort_rang(a) sort(rang(a))
...
ll fact_1(ll a) { return a ? fact_1(a - 1) * a % modu : 1; }
...
int main() {
    ...
    many {
        ...
        for (auto &s : a) {
            ++n;
            e.push_back(make_pair(s.first, -n));
            e.push_back(make_pair(s.second, n));
        }
        sort_rang(e);
        vi record;
        //-1 empty
        ...
        lv(i, e) {
            int j = i;
            // do not miss single point
            while (j + 1 < e.size() && e[j + 1].first == e[j].first &&
                   ll(e[j + 1].second) * e[j].second > 0)
                ++j;
            lp(k, i, j) {
                if (e[k].second > 0)
                    cur.erase(e[k].second);
                else {
                    cur.insert(-e[k].second);
                }
                // cout << e[k] << endl;
            }
            // cout << endl;
        ...
        // cout << record << endl;
        vi delta(n + 1);
        for (int i = 1; i + 1 < record.size(); ++i) {
            if (record[i] > 0) {
                int rl = record[i - 1], rm = record[i], rr = record[i + 1];
                if (rl == -1 && rr == -1) {
                    delta[rm] -= 1;
                } else if (rl == -1 || rr == -1) {
                } else {
        ...
        cout << ans + *max_element(delta.begin() + 1, delta.end()) << endl;
    }
    return 0;
}