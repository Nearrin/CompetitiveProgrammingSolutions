...
int main() {
    fast, prec(12);
    static int f[31][31][51];
    lp(n, 1, 30) lp(m, 1, 30) lp(k, 1, min(50, n * m)) {
        if (n * m == k)
            continue;
        f[n][m][k] = inf();
        lp(i, 1, n - 1) lp(j, max(0, k - (n - i) * m), min(k, i * m))
            tmin(f[n][m][k], f[i][m][j] + f[n - i][m][k - j] + m * m);
        lp(i, 1, m - 1) lp(j, max(0, k - (m - i) * n), min(k, i * n))
            tmin(f[n][m][k], f[n][i][j] + f[n][m - i][k - j] + n * n);
    }
    many {
        int n, m, k;
        in(n, m, k);
