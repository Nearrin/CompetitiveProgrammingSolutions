...
int main() {
    ...
    many {
        int n;
        in(n);
        double p = acos(-1), a = p / n / 2;
        double l = 0, r = a;
        vector<complex<double>> points(1, {0, 0.5 / sin(a)});
        lp(i, 1, 2 * n - 1) {
            points.push_back(points.back() *
                             complex<double>(cos(p / n), sin(p / n)));
        }
        auto f = [&](double x) {
            double xl = 0, xr = 0, yl = 0, yr = 0;
            for (auto p : points) {
                auto q = p * complex<double>(cos(x), sin(x));
                tmin(xl, q.real());
                tmax(xr, q.real());
                tmin(yl, q.imag());
                tmax(yr, q.imag());
