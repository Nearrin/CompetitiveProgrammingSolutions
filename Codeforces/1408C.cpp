    ...
    many {
        ...
        rv(i, p) {
            if (i == p.size() - 1)
                ...
                t2[i] = t2[i + 1] + (p[i + 1] - p[i]) * 1. / (p.size() - i - 1);
            }
        }
        lv(i, p) {
            if (i) {
                ...
                double v1 = i;
                double v2 = p.size() - i;
                double tdiff = abs(t1[i - 1] - t2[i]);
                double len = p[i] - p[i - 1];
                if (t1[i - 1] > t2[i]) {
                    if (tdiff * v2 <= len + 1e-8) {
                        double tlen = len - tdiff * v2;
                        tmin(ans, max(t1[i - 1], t2[i]) + tlen / (v1 + v2));
                    }
                } else {
                    if (tdiff * v1 <= len + 1e-8) {
                        double tlen = len - tdiff * v1;
                        tmin(ans, max(t1[i - 1], t2[i]) + tlen / (v1 + v2));
                    }
                }
            }
