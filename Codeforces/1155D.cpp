...
vi a;
ll f[300010][3][3];
ll dfs(int i, int m_state, int s_state) {
    ll &g = f[i][m_state][s_state];
    if (g != -inf<ll>())
        return g;
    if (i == n)
        return g = 0;
    else {
        lp(is_m, 0, 1) lp(is_s, 0, 1) {
            if (is_m && m_state == 2)
                continue;
            if (is_s && s_state == 2)
                continue;
            ...
            if (is_s)
                ...
                s_s = 2;
            tmax(g, dfs(i + 1, m_s, s_s) + (ll)a[i] * is_s * (is_m ? x : 1));
        }
        return g;
    }
}
int main() {
    ...
    a.resize(n);
    in(a);
    lp(i, 0, n) lp(j, 0, 2) lp(k, 0, 2) f[i][j][k] = -inf<ll>();
    ou(dfs(0, 0, 0));
    return 0;
}
