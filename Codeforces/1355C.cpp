    ...
    lp(z, c, d) {
        ...
        if (b + b < z + 1) {
            ...
            if (a >= z + 1 - b) {
                ...
                ans += ll(b - a + 1) * (c - b + 1) - f(z + 1 - b - a);
            }
            else {
                // ou((b - (z + 1 - b) + 1), (b - (z + 1 - c) + 1),
                // (c - b + 1) / 2);
                ans += ((b - (z + 1 - b) + 1) + (b - (z + 1 - c) + 1)) *
                       (c - b + 1) / 2;
            }
        }
        // ou(z, ans);
...
int main() {
    ...
    while (0) {
        vl a;
        lp(i, 1, 4) a.push_back(rint(1, 5));
        a = {1, 3, 4, 5};
        sort(a);
        ou(a);
        ou(solve1(a[0], a[1], a[2], a[3]), solve2(a[0], a[1], a[2], a[3]));
        assert(solve1(a[0], a[1], a[2], a[3]) ==
               solve2(a[0], a[1], a[2], a[3]));
    }
    return 0;
}
