...
int main() {
    ...
    in(a);
    int mx = max(a);
    integer_factorization_1::linear_sieve f(mx);
    int n = f.p.size() + 1;
    shortest_cycle::shortest_cycle sc(n);
    for (auto v : a) {
        vi t;
        for (auto [p, k] : f.run(v))
            if (k % 2)
                t.push_back(p);
        assert(t.size() <= 2);
        if (t.size() == 0) {
            ...
            sc.add_edge(0, lower_bound(rang(f.p), t[0]) - f.p.begin() + 1);
        else {
            sc.add_edge(lower_bound(rang(f.p), t[0]) - f.p.begin() + 1,
                        lower_bound(rang(f.p), t[1]) - f.p.begin() + 1);
        }
    }
    int ans = inf();
    lp(i, 0, n - 1) {
        if (i && f.p[i - 1] * f.p[i - 1] > mx)
            break;
        tmin(ans, sc.run_single(i));
    }
    ou(ans == inf() ? -1 : ans);
    return 0;
}
