...
struct info {
    info(inte ps = 0) { x = ps, y = 1 - ps, z = 1; }
    void push() {}
    void merge(const info &a, const info &b) {
        // fl=a.x*fm+a.y*f1+a.z
        // fm=b.x*f(r+1)+b.y*f1+b.z
        x = a.x * b.x, y = a.y + b.y * a.x, z = a.z + b.z * a.x;
    }
    void tag(int a) {}
    int tag() { return 0; }
    inte ans() {
...
int main() {
    ...
    lp(i, 1, n) {
        inte p;
        cin >> p;
        a.push_back(info(p / 100));
    }
    auto tree = segment_tree::segment_tree(a);
    vi is_ckpt(n);
    is_ckpt[0] = 1;
    set<int> ckpt;
    ckpt.insert(0);
    inte ans = tree.root->s.ans();
    lp(i, 1, q) {
        int u;
        cin >> u;
        --u;
        if (is_ckpt[u]) {
            ...
            --it;
            l = *it;
            ans += tree.query_segment(l, r).ans();
            ans -= tree.query_segment(l, u - 1).ans();
            ans -= tree.query_segment(u, r).ans();
        } else {
            auto it = ckpt.lower_bound(u);
            int l, r;
            if (it == ckpt.end())
