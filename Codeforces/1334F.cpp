...
int main() {
    ...
    in(b);
    appl(x, --x, b);
    segment_tree::segment_tree<statistic::add_sum<ll>> tree(
        vector<statistic::add_sum<ll>>(n, statistic::add_sum<ll>(-inf<ll>())));
    vi index_b(n, -1);
    lv(i, b) { index_b[b[i]] = i; }
    lv(i, a) {
        if (p[i] > 0) {
            ll tmp = p[i];
            tree.modify_segment(a[i], n - 1, tmp);
        }
        if (index_b[a[i]] != -1) {
            ll f = p[i];
            if (a[i] != b[0]) {
                f += tree.query_single(b[index_b[a[i]] - 1]).s;
            }
            tree.modify_single(a[i], statistic::add_sum<ll>(f));
            // ou(a[i] + 1, f, tree.query_single(a[i]).s);
        }
    }
    ll ans = tree.query_single(b.back()).s;
    if (ans < ll(min(0, min(p))) * n) {
        ou("NO");
    } else {
        ou("YES");
        ou(sum(p) - ans);
