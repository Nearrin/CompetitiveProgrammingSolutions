...
int main() {
    ...
    int n1, n2, n3;
    in(n1, n2, n3);
    union_find_set::union_find_set<union_find_set::bipartite> graph(n);
    int fail = 0;
    lp(i, 1, m) {
        int u, v;
        in(u, v);
        ...
        if (graph.find(u) != graph.find(v)) {
            graph.link(u, v, union_find_set::bipartite::path(1));
        } else if (graph.query_path(u, v).s) {
        } else {
            fail = 1;
        }
    }
    if (fail)
        ...
        lp(u, 0, n - 1) {
            auto tmp = graph.query_single(u);
            if (tmp.c == -1)
                graph.modify_single(u, 0);
        }
        map<int, pair<vi, vi>> items;
        lp(u, 0, n - 1) {
            if (graph.query_single(u).c)
                items[graph.find(u)].first.push_back(u);
            else
                items[graph.find(u)].second.push_back(u);
        }
        vector<bitset<5001>> dp(items.size() + 1);
        dp[0][0] = 1;
        int ii = 0;
        for (auto tmp : items) {
            dp[ii + 1] = (dp[ii] << tmp.second.first.size()) |
                         (dp[ii] << tmp.second.second.size());
            ++ii;
            // ou(tmp.second.first);
            // ou(tmp.second.second);
        }
        if (YEP(dp.back()[n2])) {
            vi color(n, 1);
            ...
            while (n2) {
                dp.pop_back();
                if (it->second.first.size() <= n2 &&
                    dp.back()[n2 - it->second.first.size()]) {
                    n2 -= it->second.first.size();
                    for (auto t : it->second.first) {
                        color[t] = 2;
                    }
                } else {
                    n2 -= it->second.second.size();
                    for (auto t : it->second.second) {
                        color[t] = 2;
                    }
                }
                ++it;
