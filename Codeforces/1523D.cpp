...
int main() {
    ...
    static int go[1 << 15];
    static int go7[1 << 15][15];
    lp(I, 1, 20) {
        auto super = a[rint(0, n - 1)];
        // ou("super:", bitset<60>(super));
        validindices.clear();
        lp(i, 0, m - 1) if (barr(super, i)) validindices.push_back(i);
        if (validindices.empty())
            continue;
        int LT = validindices.size();
        lp(i, 0, (1 << LT) - 1) {
            go[i] = 0;
            lp(j, 0, LT - 1) go7[i][j] = 0;
        }
        for (auto v : a) {
            int tmp = 0;
        ...
        for (int i = (1 << LT) - 1; i >= 0; --i) {
            ...
            for (int j = 0; j < LT; ++j) {
                if (!barr(i, j)) {
                    if (ct == 1)
                        go7[i][ct] = go[i] - go[i | (1 << j)];
                    else
                        go7[i][ct] = go7[i][ct - 1] - go7[i | (1 << j)][ct - 1];
                    ct++;
                }
            }
        }
        ...
        auto mapback = [&](int tmp) {
            ll aa = 0;
            lp(i, 0, LT - 1) if (barr(tmp, LT - i - 1)) aa +=
                1ll << validindices[i];
            return aa;
        };
        int best = 0, bestcnt = 0;
        lp(i, 0, (1 << LT) - 1) {
            // ou(bitset<60>(mapback(i)), go[i]);
            if (go[i] >= (n + 1) / 2) {
                if (bcnt(i) > bestcnt) {
                    bestcnt = bcnt(i);
                    best = i;
        ...
        if (bestcnt > anscnt) {
            // ou("updatefrom", bitset<60>(best));
            anscnt = bestcnt;
            ans = 0;
            lp(i, 0, LT - 1) if (barr(best, LT - i - 1)) ans +=
                1ll << validindices[i];
            // ou("update", bitset<60>(ans));
        }
    }
    str ans2;
    lp(i, 0, m - 1) {
        ans2.push_back('0' + ans % 2);
        ans /= 2;
    }
    reve(ans2);
    ou(ans2);
