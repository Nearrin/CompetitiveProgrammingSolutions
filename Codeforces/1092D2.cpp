...
struct segment {
    segment(int a, int b, int c) { i = a, l = b, h = c; }
    int i, l, h;
};
struct comp1 {
    bool operator()(segment a, segment b) const { return a.i < b.i; }
};
struct comp2 {
...
int main() {
    ...
    in(a);
    set<segment, comp1> s;
    set<segment, comp2> low;
    lp(i, 0, n - 1) {
        int j = i;
        while (j + 1 < n && a[j + 1] == a[j])
            ++j;
        s.insert(segment(i, j - i + 1, a[j]));
        low.insert(segment(i, j - i + 1, a[j]));
        i = j;
    }
    while (s.size() != 1) {
        auto cand = *low.begin();
        // ou(cand.i, cand.l, cand.h);
        if (cand.l % 2 == 1)
            break;
        low.erase(cand);
        s.erase(cand);
        vector<segment> nei;
        auto it = s.lower_bound(cand);
        if (it != s.end()) {
            auto tmp = *it;
            nei.push_back(tmp);
        }
        it = s.lower_bound(cand);
        if (it != s.begin()) {
            --it;
            auto tmp = *it;
            nei.push_back(tmp);
