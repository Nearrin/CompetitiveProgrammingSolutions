...
struct node {
    node(int i_) : i(i_), c(0) {}
    int i;
    node *c;
};
struct line {
    line() : f(0), b(0) {}
    line(int i) : f(new node(i)), b(f) {}
    node *f, *b;
};
line cat(line a, line b) {
    line c;
    a.b->c = b.f;
    c.f = a.f;
    c.b = b.b;
...
vvi to;
int n;
pair<line, line> build(int u, int p) {
    line l(u), r(u);
    // ou("!", u);
    for (auto v : to[u])
        if (v != p) {
...
int main() {
    ...
    vi bg(n), ed(n);
    int cc = 0;
    for (node *i = tmp.first.f; i; i = i->c) {
        ++cc;
        int v = i->i;
        if (cnt[v] == 0)
            bg[v] = cc;
