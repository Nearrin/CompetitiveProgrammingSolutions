...
template <class F> struct sparse_ {
    ...
    auto build(const T &t, const K &... k) const {
        return map<T, decltype(build<V, K...>(k...))>();
    }
    template <class M, class V> V &find(M &m) const { return m; }
    template <class M, class V, class T, class... K>
    V &find(M &m, const T &t, const K &... k) const {
...
int main() {
    ...
    return 0;
    auto f = lru_cache::sparse()([](auto self, vi a, int p) -> int {
        int fail = 1;
        lv(i, a) if (a[i] && i != p) {
            --a[i];
            if (self(a, i) == 0)
    ...
    lp(i, 1, 20) lp(j, 1, 20) lp(k, 1, 20) lp(l, 1, 20) {
        if (i <= j && j <= k && k <= l &&
            f(vi{i, j, k, l}, -1) != (i + j + k + l) % 2)
            ou(i, j, k, l, f(vi{i, j, k, l}, -1), (i + j + k + l) % 2);
    }
    return 0;
}
