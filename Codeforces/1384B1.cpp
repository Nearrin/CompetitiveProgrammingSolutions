    ...
    many {
        ...
        for (int d : dd) {
            if (d + k <= l) {
                ...
                while (rise.size() && rise.back().first + rdd > t) {
                    rise.pop_back();
                }
                if (rise.size() && rise.back().second + rdd > t)
                    rise.back().second = t - rdd;
                if (rise.empty()) {
                    rdd = 0;
                    if (fall.first == -1) {
                        fail = 1;
                    ...
                    if (rise.back().second + rdd == k) {
                        fall.first = k;
                    }
                    ++rdd;
                    if (fall.first >= 0) {
                        --fall.first;
                        rise.push_front({-rdd + 1, -rdd + 1});
                    }
                    if (rise.back().second + rdd == k + 1) {
                        if (rise.back().first == rise.back().second) {
                            rise.pop_back();
                        } else {
                            --rise.back().second;
                        }
