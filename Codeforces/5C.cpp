...
int main() {
    ...
    vi a(1, 0);
    for (auto c : in<str>()) {
        a.push_back(map<char, int>{{'(', 1}, {')', -1}}[c] + a.back());
    }
    static_rmq::berkman_vishkin_algorithm<int, less<int>> rmq(a.size());
    rmq.a = a;
    rmq.build();
    int ans1 = 0, ans2 = 1;
    unordered_map<int, vi> b;
    lv(i, a) b[a[i]].push_back(i);
    for (auto t : b) {
        auto idx = t.second;
        int v = t.first;
        lv(i, idx) {
            int j = i;
            while (j + 1 < idx.size() && rmq.query(idx[i], idx[j + 1]) >= v) {
                ++j;
            }
            if (idx[j] - idx[i] > ans1) {
                ans1 = idx[j] - idx[i];
