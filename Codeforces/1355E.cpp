...
int main() {
    ...
    lp(i, 0, n - 1) {
        curs += h[i];
        ll in = (i + 1) * (ll)h[i] - curs;
        ll out = s - curs - (n - (i + 1)) * ll(h[i]);
        tmin(ans, a * in + r * out);
        auto upda = [&](ll ii, ll oo) {
            tmin(ans, min(ii, oo) * m + a * (ii - min(ii, oo)) +
                          r * (oo - min(ii, oo)));
        };
        upda(in, out);
        if (i != n - 1) {
            ll dmax = h[i + 1] - h[i];
            if (in <= out &&
                in + (i + 1) * dmax >= out - (n - (i + 1)) * dmax) {
                ll d1 = (out - in) / n, d2 = d1 + 1;
                if (d2 > dmax)
                    --d2;
                upda(in + (i + 1) * d1, out - (n - (i + 1)) * d1);
                upda(in + (i + 1) * d2, out - (n - (i + 1)) * d2);
            }
        }
    }
    ou(ans);
