...
int main() {
    ...
    for (int i = n - 1; i >= 0; --i) {
        auto g = f;
        for (int remain_k = 0; remain_k <= k; ++remain_k) {
            for (int cnt_t1 = 0; cnt_t1 <= n; ++cnt_t1) {
                f[remain_k][cnt_t1] = -1;
                if (i == n - 1) {
                    if (remain_k > 0) {
                        if (cnt_t1 == 1)
                    ...
                    if (remain_k > 0) {
                        if (cnt_t1 - (t[0] == t[1]) >= 0 &&
                            g[remain_k - 1][cnt_t1 - (t[0] == t[1])] >= 0)
                            f[remain_k][cnt_t1] =
                                max(f[remain_k][cnt_t1],
                                    g[remain_k - 1][cnt_t1 - (t[0] == t[1])] +
                                        cnt_t1 - (t[0] == t[1]));
                        if (cnt_t1 - 1 >= 0 &&
                            g[remain_k - 1][cnt_t1 - 1] != -1)
                            f[remain_k][cnt_t1] =
                                max(f[remain_k][cnt_t1],
                                    g[remain_k - 1][cnt_t1 - 1] +
                                        (t[0] == t[1]) * (cnt_t1 - 1));
                    }
                    if (cnt_t1 - (s[i] == t[1]) >= 0 &&
                        g[remain_k][cnt_t1 - (s[i] == t[1])] >= 0)
                        f[remain_k][cnt_t1] =
                            max(f[remain_k][cnt_t1],
                                g[remain_k][cnt_t1 - (s[i] == t[1])] +
                                    (s[i] == t[0]) * (cnt_t1 - (s[i] == t[1])));
                }
            }
        }
    }
