...
const int maxl = 200;
ll vis[maxl + 10][3][6][6][6];
ll dp[maxl + 10][3][6][6][6];
ll pi[6][6][6], pj[6][6][6];
ll sg_original(ll a, ll s, ll x, ll y, ll z) {
    a = max(a, 0ll);
    ll &tmp = dp[a][s][x][y][z];
    if (vis[a][s][x][y][z])
        return tmp;
    ...
    if (a != 0) {
        set<ll> t;
        t.insert(sg_original(a - x, 0, x, y, z));
        if (s != 1)
            t.insert(sg_original(a - y, 1, x, y, z));
        if (s != 2)
            t.insert(sg_original(a - z, 2, x, y, z));
        while (t.count(tmp))
            ++tmp;
    }
    return tmp;
}
ll sg(ll a, ll s, ll x, ll y, ll z) {
    if (a >= pi[x][y][z]) {
        ll d = a - pi[x][y][z];
        d %= (pj[x][y][z] - pi[x][y][z]);
        a = pi[x][y][z] + d;
    }
    return sg_original(a, s, x, y, z);
}
void get_period() {
    lp(x, 1, 5) lp(y, 1, 5) lp(z, 1, 5) {
        sg_original(maxl, 0, x, y, z);
        sg_original(maxl, 1, x, y, z);
        sg_original(maxl, 2, x, y, z);
        map<vl, int> vv;
        lp(i, 1, maxl) {
            vl state;
            lp(j, 1, 5) lp(k, 0, 2)
                state.push_back(sg_original(i - j, k, x, y, z));
            if (vv.count(state)) {
                pi[x][y][z] = vv[state];
                pj[x][y][z] = i;
                // ou(x, y, z, ':', pi[x][y][z], pj[x][y][z]);
                break;
            }
            vv[state] = i;
        }
...
int main() {
    ...
    many {
        ...
        for (auto v : a) {
            cnt += 3;
            if (ans ^ sg(v, 0, x, y, z) ^ sg(v - x, 0, x, y, z))
                --cnt;
            if (ans ^ sg(v, 0, x, y, z) ^ sg(v - y, 1, x, y, z))
                --cnt;
            if (ans ^ sg(v, 0, x, y, z) ^ sg(v - z, 2, x, y, z))
                --cnt;
        }
        ou(cnt);
    }
