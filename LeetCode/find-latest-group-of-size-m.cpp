    ...
    int findLatestStep(vector<int>& a, int m) {
        ...
        for(auto i:a){
            ...
            if(val[i-1]==0&&val[i+1]==0){
                ...
                tree.insert({i,t});
                ++cc[t];
            }elif(val[i-1]==0){auto r=tree.lower_bound({i,0});
                int t=r->second+1;
                tree.erase(r);
                --cc[t-1];
                tree.insert({i,t});
                ++cc[t];
            }elif(val[i+1]==0){auto r=tree.lower_bound({i,0});auto l=r;--l;
                 int t=l->second+1;
                tree.erase(l);
                tree.insert({i,t}); --cc[t-1];
                ++cc[t];
            }else{auto r=tree.lower_bound({i,0});auto l=r;--l;
                int t=l->second+r->second+1;
                 --cc[l->second]; --cc[r->second];
                tree.erase(r);
                tree.erase(l);
