...
public:
    int minJumps(vector<int>& a) {
        int n=a.size();
        map<int,int>name;
        for(auto v:a)if(!name.count(v))name[v]=n+name.size();
        shortest_path::dijkstras_algorithm<int>f(n+name.size());
        f.distance[0]=0;
        lp(i,0,n-2){
            f.add_edge(i,i+1,1);
            f.add_edge(i+1,i,1);
        ...
        lp(i,0,n-1){
            f.add_edge(i,name[a[i]],1);
            f.add_edge(name[a[i]],i,0);
        }
        f.run();
        return f.distance[n-1];
    }
};
