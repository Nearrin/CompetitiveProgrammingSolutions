    ...
    int minNumberOfSemesters(int n, vector<vector<int>>& d, int k) {
        ...
        lp(m,0,(1<<n)-2){
            ...
            for(auto e:d){
                int x=e[0],y=e[1];
                --x,--y;
                if(barr(m,x)==0)fail[y]=1;
            }
            vi cand;
            lp(i,0,n-1)if(barr(m,i)==0&&!fail[i])cand.push_back(i);
            lp(q,1,(1<<cand.size())-1){
                if(bcnt(q)>k)continue;
                int new_m=m;
                lv(i,cand)if(barr(q,i))new_m+=1<<cand[i];
                tmin(f[new_m],f[m]+1);
            }
        }
        return f.back();
