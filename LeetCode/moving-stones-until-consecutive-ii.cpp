...
int solve_max(vi a){
    vi b;
    sort(a);
    lv(i,a)if(i)b.push_back(a[i]-a[i-1]-1);
    return max(sum(b)-b.front(),sum(b)-b.back());
}
int solve_min(vi a){
    sort(a);
    int n=a.size();
    ...
    for(auto v:a){
        q.push(v);
        while(v-q.front()+1-q.size()>n-q.size())q.pop();
        ++cnt;
        int lc=cnt-q.size(),rc=n-cnt;
        if(max(lc,rc)==1&&v-q.front()+1-q.size()==0)continue;
        tmin(ans,int(n-q.size()));
    }
    return ans;
}
class Solution {
