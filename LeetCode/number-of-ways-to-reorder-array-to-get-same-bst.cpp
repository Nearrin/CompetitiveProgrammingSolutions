...
class Solution {
public:
    int numOfWays2(vector<int> nums) {
        if(combin.empty()){
            combin=vect<int>(0,1010,1010);
            lp(i,0,1005)lp(j,0,i){
                if(j==0||j==i)combin[i][j]=1;
                else combin[i][j]=(combin[i-1][j]+combin[i-1][j-1])%int(1e9+7);
            }
        }
        if(nums.size()<=1)return 1;
        vi l,r;
        lv(i,nums)if(nums[i]<nums[0])l.push_back(nums[i]);elif(nums[i]>nums[0])r.push_back(nums[i]);
        int ans=ll(numOfWays2(l))*numOfWays2(r)%int(1e9+7);
        return ll(ans)*combin[l.size()+r.size()][l.size()]%int(1e9+7);
    }int numOfWays(vector<int> nums) {
        int t=numOfWays2(nums);
        --t;
        if(t<0)t=int(1e9+6);
        return t;}
};
