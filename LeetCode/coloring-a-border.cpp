...
public:
    vector<vector<int>> colorBorder(vector<vector<int>>& a, int r0, int c0, int color) {
        ...
        while(q.size()){
            ...
            for(auto [nx,ny]:neighbours(x,y,n,m,1,4)){
                if(a[nx][ny]+a[x][y]==0){
                    a[nx][ny]*=-1;
                    q.push({nx,ny});
                }
        ...
        for(int x=0;x<n;++x)
            for(int y=0;y<m;++y){
                if(a[x][y]<0){
                    a[x][y]*=-1;
                    int work=0;
                    auto nei=neighbours(x,y,n,m,1,4);
                    for(auto [nx,ny]:nei){
                        if(abs(a[x][y])==abs(a[nx][ny]))++work;
                        //ou(nx,ny,a[nx][ny],a[x][y]);
                    }
                    //ou(x,y,work);
                    if(work!=4)todo.push_back({x,y});
                }
            }
        for(auto [x,y]:todo)a[x][y]=color;
        return a;
    }
};
