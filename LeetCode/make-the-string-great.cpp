...
public:
    string makeGood(string s) {
        while(1){
            int j=-1;
            lv(i,s)if(i&&s[i-1]>='a'&&s[i-1]<='z'&&s[i]>='A'&&s[i]<='Z'&&s[i-1]-'a'==s[i]-'A'){j=i;break;}
            lv(i,s)if(i&&s[i]>='a'&&s[i]<='z'&&s[i-1]>='A'&&s[i-1]<='Z'&&s[i]-'a'==s[i-1]-'A'){j=i;break;}
            if(j!=-1){
                string t;
                lv(i,s)if(i!=j&&i!=j-1)t.push_back(s[i]);
                s=t;
            }else break;
        }
        return s;
