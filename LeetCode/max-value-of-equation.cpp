...
lv(j,points){
    ...
    if(add.size()>1){
        if(add.begin()->second!=sub.begin()->second){
            tmax(ans,add.begin()->first+sub.begin()->first);
        }else{
            auto it=add.begin();++it;
            tmax(ans,it->first+sub.begin()->first);
            it=sub.begin();++it;
            tmax(ans,add.begin()->first+it->first);
        }
    }
}
return ans;
