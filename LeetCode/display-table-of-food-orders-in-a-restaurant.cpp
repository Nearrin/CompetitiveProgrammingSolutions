...
public:
    vector<vector<string>> displayTable(vector<vector<string>>& a) {
        vector<string>ff;
        vi tt;vector<string>tt2;
        map<string,int>ts;
        map<string,int>fs;
        lv(i,a){
            str t=a[i][1],f=a[i][2];
            tt.push_back(conv<str,int>(t));
            ff.push_back(f);
        }
        sort(tt),sort(ff);
        lv(i,tt){
            str t=conv<int,str>(tt[i]),f=ff[i];
            if(!ts.count(t))ts[t]=ts.size()+1,tt2.push_back(t);
            if(!fs.count(f))fs[f]=fs.size()+1;
        }
        auto ans=vect<str>({"0"},ts.size()+1,fs.size()+1);
        ans[0][0]="Table";
        int i=0;
        for(auto tmp:fs){
            ans[0][++i]=tmp.first;
        ...
        lv(i,a){
            int t=ts[a[i][1]],f=fs[a[i][2]];
            //ou(t,f);
            str&c=ans[t][f];
            c=conv<int,str>(conv<str,int>(c)+1);
        }
        return ans;
    }
};
