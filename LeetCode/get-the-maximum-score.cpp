...
public:
    int maxSum(vector<int>& a, vector<int>& b) {
        ...
        lv(i,a)pos[a[i]]={i,-1};
        lv(i,b)if(!pos.count(b[i]))pos[b[i]]={-1,i};else pos[b[i]].second=i;
        ...
        rv(i,c){
            ...
            if(p.first==-1){
                f[i]=c[i];
                if(p.second+1!=b.size())f[i]+=f[rc[b[p.second+1]]];
            }elif(p.second==-1){
                f[i]=c[i];
                if(p.first+1!=a.size())f[i]+=f[rc[a[p.first+1]]];
            }else{
                f[i]=c[i];
                ll delta=0;
                if(p.second+1!=b.size())delta=f[rc[b[p.second+1]]];
                if(p.first+1!=a.size())tmax(delta,f[rc[a[p.first+1]]]);
                f[i]+=delta;
            }
        }
        return max(f)%int(1e9+7);
    }
};
