...
public:
    double getMinDistSum(vector<vector<int>>& p) {
        double xl=0,xr=100,yl=0,yr=100,ans;
        lp(it,0,50){
            double bestx,besty,bests=1e100;
            auto test=[&](double px,double py){
                ...
                if(s<bests)bests=s,bestx=px,besty=py;
            };
            test(xl,yl);
            test(xl,yr);
            test(xr,yl);
            test(xr,yr);
         lp(ix,0,10){
             auto px=xl+(xr-xl)/10*ix;
             lp(iy,0,10){
                 auto py=yl+(yr-yl)/10*iy;
                  test(px,py);
             }
         }   
            double tt=(xr-xl)/4;
            xl=bestx-tt,xr=bestx+tt;
            yl=besty-tt,yr=besty+tt;
            ans=bests;
        }
        return ans;
    }
