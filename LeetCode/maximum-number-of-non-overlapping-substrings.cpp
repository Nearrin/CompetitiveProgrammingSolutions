...
public:
    vector<string> maxNumOfSubstrings(string s) {
        ...
        vpii a;
        vi aa;
        lv(i,p)if(p[i].size())a.emplace_back(p[i][0],p[i].back()),aa.push_back(i);
        auto check=[&](pii x,pii y,int ii,int jj){
            char cx=s[x.first],cy=s[y.first];
            cx='a'+ii,cy='a'+jj;
            int l=max(x.first,y.first);
            int r=min(x.second,y.second);
            int cnt=0;
            auto it=lower_bound(rang(p[cx-'a']),l);
            if(it!=p[cx-'a'].end()&&*it<=r)++cnt;
            it=lower_bound(rang(p[cy-'a']),l);
            if(it!=p[cy-'a'].end()&&*it<=r)++cnt;
            return cnt==2;
        };
        while(true){
            int ii=-1,jj=-1;
            lv(i,a)lv(j,a)if(a[i].first<=a[j].second&&a[j].first<=a[i].second&&(a[i].first!=a[j].first||a[i].second!=a[j]
                .second)&&check(a[i],a[j],aa[i],aa[j]))ii=i,jj=j;
            if(ii==-1)break;
            a[jj]=a[ii]=pii{min(a[ii].first,a[jj].first),max(a[ii].second,a[jj].second)};
        }
        int cur=-1;
        sort(a,[](pii x,pii y){return x.second<y.second;});
        vector<str>ans;
        for(auto v:a){
            if(v.first<=cur)continue;
            cur=v.second;
            ans.push_back("");
            lp(i,v.first,v.second)ans.back().push_back(s[i]);
        }
        return ans;
    }
};
