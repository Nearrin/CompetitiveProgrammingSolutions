...
public:
    int cherryPickup(vector<vector<int>>& a) {
        ...
        auto f=vect<int>({-inf()},n,m,m);
        f[0][0][m-1]=a[0][0]+a[0][m-1];
        int ans=0;
        lp(i,0,n-2){
            lp(j,0,m-1)lp(k,0,m-1){
                lp(u,-1,1)lp(v,-1,1){
                    if(u+j>=0&&u+j<m&&v+k>=0&&v+k<m){
                        if(u+j==v+k){
                            tmax(f[i+1][u+j][v+k],f[i][j][k]+a[i+1][u+j]);
                        }else{
                            tmax(f[i+1][u+j][v+k],f[i][j][k]+a[i+1][u+j]+a[i+1][v+k]);
                        }
                    }
                }
            }
