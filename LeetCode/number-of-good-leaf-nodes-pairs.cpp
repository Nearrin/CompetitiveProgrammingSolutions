...
void dfs(TreeNode*u){
    ...
    int t=n;
    ++n;
    if(u->left)e.push_back({t,n}),dfs(u->left),leaf[t]=0;
    if(u->right)e.push_back({t,n}),dfs(u->right),leaf[t]=0;
}
int gg;
...
struct bfs_forest {
    ...
    bfs_forest(int n)
        : to(n), visited(n), depth(n,inf()), component(n), component_number(0) {}
    void add_edge(int u, int v) {
        to.at(u).push_back(v), to.at(v).push_back(u);
    }
    void run_single(int s) {
        if (!visited[s]) {
            queue<int> q;
            q.push(s), visited[s] = true, component[s] = component_number++;
            depth[s]=0;
            while (q.size()) {
                int u = q.front();
                q.pop();
                if(depth[u]>gg)continue;
                for (int v : to[u])
                    if (!visited[v])
...
public:
    int countPairs(TreeNode* root, int distance) {
        ...
        dfs(root);
        int ans=0;
        bfs_forest::bfs_forest f(n);
        gg=distance;
        for(auto t:e)f.add_edge(t.first,t.second);
        for(int u=0;u<n;++u){
            if(leaf[u]){
                fill(rang(f.visited),0);
                fill(rang(f.depth),inf());
                fill(rang(f.component),0);
                f.component_number=0;
                f.run_single(u);
                for(int v=0;v<u;++v)if(leaf[v]&&f.depth[v]<=distance){
                    ++ans;
                }
            }
        }
