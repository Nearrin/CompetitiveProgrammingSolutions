...
class Solution {
public:
    int mergeStones(vector<int>& a, int m) {
        int n=a.size();
        vi s(n+1);
        lp(i,1,n)s[i]+=s[i-1]+a[i-1];
        auto f=vect<int>({inf()},n+1,n+1,m+1);
        lp(l,1,n)lp(i,1,n-l+1){
            int j=i+l-1;
            rp(k,m,1){
                if(k==1)
                    tmin(f[i][j][k],(i!=j)*(f[i][j][m]+s[j]-s[i-1]));
                else
                    lp(t,i,j-1)
                        if(f[i][t][k-1]!=inf()&&f[t+1][j][1]!=inf())
                            tmin(f[i][j][k],f[i][t][k-1]+f[t+1][j][1]);
            }
        }
