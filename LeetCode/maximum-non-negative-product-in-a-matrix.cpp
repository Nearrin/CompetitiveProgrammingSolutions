...
public:
    int maxProductPath(vector<vector<int>>& a) {
        int n=a.size();
        int m=a[0].size();
        auto fneg=vect<ll>(inf<ll>(),n,m),fpos=vect<ll>(-inf<ll>(),n,m);
        for(int i=0;i<n;++i){
            for(int j=0;j<m;++j){
                if(i==0&&j==0){
                    if(a[i][j]>=0)
                    ...
                    if(a[i][j]>=0){
                        if(i){
                        if(fpos[i-1][j]!=-inf<ll>())tmax(fpos[i][j],fpos[i-1][j]*a[i][j]);
                        if(fneg[i-1][j]!=inf<ll>())tmin(fneg[i][j],fneg[i-1][j]*a[i][j]);
                        }
                        if(j){
                        if(fpos[i][j-1]!=-inf<ll>())tmax(fpos[i][j],fpos[i][j-1]*a[i][j]);
                        if(fneg[i][j-1]!=inf<ll>())tmin(fneg[i][j],fneg[i][j-1]*a[i][j]);
                        }
                    }
                    if(a[i][j]<=0){
                        if(i){
                        if(fneg[i-1][j]!=inf<ll>())tmax(fpos[i][j],fneg[i-1][j]*a[i][j]);
                        if(fpos[i-1][j]!=-inf<ll>())tmin(fneg[i][j],fpos[i-1][j]*a[i][j]);
                        }
                        if(j){
                        if(fneg[i][j-1]!=inf<ll>())tmax(fpos[i][j],fneg[i][j-1]*a[i][j]);
                        if(fpos[i][j-1]!=-inf<ll>())tmin(fneg[i][j],fpos[i][j-1]*a[i][j]);
                        }
                    }
                }
            }
        }
        if(fpos[n-1][m-1]>=0)return fpos[n-1][m-1]%int(1e9+7);
        return -1;
    }
};
