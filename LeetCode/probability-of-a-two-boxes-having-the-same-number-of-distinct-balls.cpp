...
public:
    double getProbability(vector<int>& a) {
        int n=a.size(),l=sum(a)/2;
        auto f=vect<double>({},n+1,(1<<n),(1<<n),l+1);
        f[0][0][0][0]=1;
        auto choose=vect<double>({},10,10);
        choose[0][0]=1;
        lp(i,1,9)lp(j,0,i){
        ...
        lv(i,a){
            lp(m1,0,(1<<n)-1){
                lp(m2,0,(1<<n)-1){
                    lp(c,0,l){
                        if(f[i][m1][m2][c]==0)continue;
                        if(barr(m1,i))continue;
                        if(barr(m2,i))continue;
                        rp(g,min(a[i],l-c),0){
                            //cout<<vi{i+1,m1+(int(g>0)<<i),m2+(int(g<a[i])<<i),c+g}<<endl;
                            f[i+1][m1+(int(g>0)<<i)][m2+(int(g<a[i])<<i)][c+g]+=f[i][m1][m2][c]*(choose[a[i]][g]/(1<<a[i]));
                        }
                    }
                }
            }
