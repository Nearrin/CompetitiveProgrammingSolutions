...
inte dfs(const vector<vector<int>>&c,vector<int>&tsize,int x){
    if(c[x].size()==0){
        tsize[x]=1;
        return 1;
    }
    ...
    tsize[x]+=1;
    for(auto y:c[x]){
        tmp*=inte::binary_coefficient(tot,tsize[y]);
        tot-=tsize[y];
    }
    return tmp;
}
...
public:
    int waysToBuildRooms(vector<int>& prevRoom) {
        ...
        for(int i=0;i<prevRoom.size();++i){
            if(i)c[prevRoom[i]].push_back(i);
        }
        return dfs(c,tsize,0).v;
    }
};
