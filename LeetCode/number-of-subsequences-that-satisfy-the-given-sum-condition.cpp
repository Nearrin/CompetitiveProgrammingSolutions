...
public:
    int numSubseq(vector<int>& nums, int k) {
        bbst<pii>t;
        vi left(nums.size()),right=left;
        lv(i,nums){
            left[i]=i+1;
            //left[i]-=t.order_of_key({k-nums[i],+inf()});
            left[i]-=que(k-nums[i]);
           // left[i]-=t.size()-t.order_of_key({nums[i],inf()});
            left[i]-=i-que(nums[i]);
            //t.insert({nums[i],i});
            add(nums[i],1);
        }
        ...
        rv(i,nums){
            right[i]=nums.size()-i;
           // right[i]-=t.order_of_key({k-nums[i],+inf()});
            right[i]-=que(k-nums[i]);
            //right[i]-=t.size()-t.order_of_key({nums[i],-inf()});
            right[i]-=nums.size()-i-1-que(nums[i]-1);
            //t.insert({nums[i],i});
            add(nums[i],1);
        }
        rv(i,nums)add(nums[i],-1);
        int ans=0;
        vi pw(nums.size()+1);
        pw[0]=1;
        lv(i,pw)if(i){
