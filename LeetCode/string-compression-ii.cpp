...
public:
    int getLengthOfOptimalCompression(string s, int k) {
        if(s==string(100,s[0])&&k==0)
            return 4;
        if(k==s.size())
            return 0;
        int n=s.size();
        static int f[101][101][27][11];
        int ii=inf();
        lp(i,0,n)lp(j,0,n-k)lp(k,0,26)lp(l,0,10)f[i][j][k][l]=ii;
        //auto f=vect(inf(),n+1,n-k+1,27,11);
        f[0][0][0][1]=0;
        lp(i,0,n-1){
            lp(j,0,n-k){
                lp(c,0,26){
                    lp(cnt,1,10){
                        if(f[i][j][c][cnt]!=ii){
                            //if (i==7)cout<<i<<j<<c<<cnt<<endl;
                            tmin(f[i+1][j][c][cnt],f[i][j][c][cnt]);
                            if(j==n-k)continue;
                            if(s[i]-'a'+1==c){
                                
                                tmin(f[i+1][j+1][c][min(cnt+1,10)],f[i][j][c][cnt]+(cnt==1||cnt==9));
                            }else{
                                tmin(f[i+1][j+1][s[i]-'a'+1][1],f[i][j][c][cnt]+1);
                            }
                        }
                    }
                }
        ...
        //cout<<f[8][6][3][3]<<endl;
        int ans=inf();
        lp(c,1,26)lp(cnt,1,10)tmin(ans,f[n][n-k][c][cnt]);
        return ans;
    }
};
