...
int solve1(int n, str s, str t) {
    ...
    rv(i, s) {
        lp(j, 0, 1) {
            ...
            f[j][i] = f[carry][i + 1] + (vs != vt) + delta;
            {
                ...
                int carry = si == '1' && j == 1;
                char vt = t[i];
                tmin(f[j][i], f[carry][i + 1] + (vs != vt) + delta);
            }
        }
    }
    int ans = inf();
    ...
    while (qu.size()) {
        ...
        tmin(ans, f[car][l] + bcnt(s2 ^ tt) + dis[this_s]);
        lp(i, 0, l - 1) {
            if (!vis[this_s ^ (1 << i)]) {
                vis[this_s ^ (1 << i)] = 1;
                dis[this_s ^ (1 << i)] = dis[this_s] + 1;
                qu.push(this_s ^ (1 << i));
            }
        }
        if (this_s < (1 << (l + 1)) - 1 && !vis[this_s + 1]) {
            vis[this_s + 1] = 1;
            dis[this_s + 1] = dis[this_s] + 1;
            qu.push(this_s + 1);
        }
    }
    return ans;
}
int solve2(int n, str s, str t) {
    ll ss = 0, tt = 0;
    lv(i, s) {
        ss = ss + ((s[i] == '1') << i);
        tt = tt + ((t[i] == '1') << i);
...
int main() {
    ...
    while (1) {
        int n = 8;
        str s, t;
        lp(i, 0, n - 1) s.push_back('0' + rint(0, 1)),
            t.push_back('0' + rint(0, 1));
        int a1 = solve1(n, s, t);
        int a2 = solve2(n, s, t);
        ou(n);
        ou(s);
