...
int main() {
    ...
    cin.tie(0);
    vector<relation> r{{95, 100, 4.3}, {90, 94, 4.0}, {85, 89, 3.7},
                       {80, 84, 3.3},  {75, 79, 3.0}, {70, 74, 2.7},
                       {67, 69, 2.3},  {65, 66, 2.0}, {62, 64, 1.7},
                       {60, 61, 1.0},  {0, 59, 0}};
    auto check = [&](int s) {
        for (auto t : r)
            if (s >= t.l && s <= t.r)
    ...
    vector<double> f(401, -100000);
    f[0] = 0;
    lp(i, 1, 4) {
        vector<double> g(401, -100000);
        lp(s, 0, 100 * (i - 1)) lp(j, 0, 100) {
            tmax(g[s + j], f[s] + check(j));
        }
        f = g;
    }
    many {
