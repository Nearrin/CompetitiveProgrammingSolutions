...
int main() {
    ...
    many {
        ...
        lp(sx, 0, n - 1) lp(sy, 0, n - 1) { in(a[sx][sy]); }
        lp(sx, 0, n - 1) lp(sy, 0, n - 1) {
            int cur = 0;
            cur += ceil((int)abs(sx - x) + (int)abs(sy - y), 2);
            int cur_food = 0;
            int cur_speed = a[sx][sy];
            vi cand;
            lp(dx, -3, 3) lp(dy, -3, 3) {
                if (dx == 0 && dy == 0)
                    continue;
                ...
                if (abs(dx) + abs(dy) > 3)
                    continue;
                cand.push_back(a[tx][ty]);
            }
            sort(cand);
            lp(need, 2, 9) {
                int want = (need - 1) * (need - 1) * 8 - cur_food;
                if (want < 0)
                    want = 0;
                int how = ceil(want, cur_speed);
                cur += how;
                cur_food += cur_speed * how;
                if (cand.size()) {
                    cur_speed += cand.back();
                    cand.pop_back();
                }
