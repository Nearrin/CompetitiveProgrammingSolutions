...
int main() {
    ...
    many {
        ...
        if (a + b + c > n) {
            ...
            vi ans;
            lp(i, 1, a) { ans.push_back(1); }
            lp(i, 1, c - 1) ans.push_back(n);
            lp(i, 1, n - a - b - c) ans.push_back(1);
            ans.push_back(n);
            lp(i, 1, b) ans.push_back(1);
            ou(goog, ans);
        }
        elif (n == 1) { ou(goog, vi{1}); }
        elif (n == 2) {
            if (a == 1)
                ou(goog, vi{1, 2});
            elif (b == 1) ou(goog, vi{2, 1});
            else ou(goog, "IMPOSSIBLE");
        }
        elif (a + b == 0) { ou(goog, "IMPOSSIBLE"); }
        else {
            int rem = n - a - b - c;
            vi ans;
            lp(i, 1, a) { ans.push_back(2); }
            if (ans.size()) {
                lp(i, 1, rem) ans.push_back(1);
                rem = 0;
            }
            ans.push_back(n);
            lp(i, 1, rem) ans.push_back(1);
            lp(i, 1, b) ans.push_back(2);
            ou(goog, ans);
        }
    }
    return 0;
