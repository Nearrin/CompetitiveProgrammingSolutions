...
vvi e;
double ans;
vector<pair<int, map<int, int> *>> ma, mb;
pair<int, map<int, int> *> merge(pair<int, map<int, int> *> &x,
                                 pair<int, map<int, int> *> &y) {
    if (x.second->size() > y.second->size()) {
        swap(x, y);
    }
    y.first += x.first;
    for (auto t : *x.second)
        (*y.second)[t.first] += t.second;
    return y;
}
int query(map<int, int> *x, int y) {
    auto t = x->find(y);
    if (t != x->end())
        return t->second;
    return 0;
...
void dfs(int u, int p, int l) {
    ma[u].first++;
    ma[u].second = new map<int, int>;
    (*ma[u].second)[l % a]++;
    mb[u].first++;
    mb[u].second = new map<int, int>;
    (*mb[u].second)[l % b]++;
    for (auto v : e[u])
        if (v != p) {
            dfs(v, u, l + 1);
            ma[u] = merge(ma[v], ma[u]);
            mb[u] = merge(mb[v], mb[u]);
        }
    ll bada = n - query(ma[u].second, l % a);
    ll badb = n - query(mb[u].second, l % b);
    ans += 1 - (double(bada) / n) * (double(badb) / n);
}
int main() {
