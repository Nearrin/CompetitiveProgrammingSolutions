...
pair<int, ll> solve1(vpii a) {
    int n = a.size();
    auto b = a;
    sort(a,
         [](pii x, pii y) { return x.first + x.second < y.first + y.second; });
    {
        ll s = 0;
        int ans = 0;
        lv(i, a) {
            s += a[i].first;
            if (s >= a[i].second + a[i].first)
                tmax(ans, i + 1);
        }
        if (ans)
            return {n - ans, inf<ll>()};
    ...
    rv(i, b) {
        if (i + 1 == n) {
            ...
            suffix[i] = suffix[i + 1] + b[i + 1].first;
    }
    ll anst = 0;
    ...
    sort(c, [](pair<pii, int> x, pair<pii, int> y) {
    ...
    bbst<int> t2;
    lv(i, c) {
        for (int k = c[i].second + 1; k <= n; k += k & -k)
            tree[k] += c[i].first.first;
        t2.insert(c[i].second);
        ll curt = suffix[c[i].second];
        ll curt2 = curt;
        for (int k = c[i].second + 1; k; k -= k & -k)
            curt += tree[k], curt2 += tree[k] * 2;
        if (curt < c[i].first.second + c[i].first.first)
            continue;
        curt = curt2;
        ou(c[i].second, suffix[c[i].second], curt);
        int ccc = t2.order_of_key(c[i].second) + 1 + n - c[i].second - 1;
        if (curt > anst || (curt == anst && ccc > anscnt)) {
            anst = curt;
            anscnt = ccc;
        }
    }
    return {n - anscnt, anst};
}
pair<int, ll> solve2(vpii b) {
    ll anst = 0;
    int anscnt = 0;
    int n = b.size();
    lp(m, 1, (1 << n) - 1) {
        vpii a;
        lv(i, b) if (barr(m, i)) a.push_back(b[i]);
        ll curt = 0;
        vl last(n, -1);
        int cc = 0;
        for (int i = 0;; i = (i + 1) % a.size()) {
            if (last[i] == -1 || curt - last[i] >= a[i].second) {
                curt += a[i].first;
                last[i] = curt;
            } else
                break;
        ...
        if (curt > anst || (curt == anst && bcnt(m) > anscnt)) {
            anst = curt;
            anscnt = bcnt(m);
        }
    }
...
int main() {
    ...
    while (0) {
        int n = rint(1, 3);
        vpii a;
        lp(i, 1, n) a.push_back({rint(1, 10), rint(1, 20)});
        ou(n);
        for (auto t : a)
            ou(t);

        auto a1 = solve1(a), a2 = solve2(a);
        ou(a1, a2);
        assert(a1 == a2);
    }
    many {
        ...
        auto t = solve2(a);
        if (t.second == inf<ll>())
            ou(goog, t.first, "INDEFINITELY");
        else
            ou(goog, t.first, t.second);
    }
    return 0;
}
