...
int main() {
    ...
    lp(i, 1, m) {
        ...
        --x, --y;
        c[y] += 1;
        targets[x].push_back(y);
    }
    set<pii, greater<pii>> tree;
    lv(i, c) tree.insert({c[i], i});
    int ans = 0;
    lv(i, targets) {
        for (auto j : targets[i]) {
            --c[j];
            tree.insert({c[j], j});
        }
        while (true) {
            auto tmp = *tree.begin();
            if (c[tmp.second] != tmp.first) {
                tree.erase(tree.begin());
            } else {
                break;
            }
        }
        tmax(ans, int(targets[i].size() + tree.begin()->first));
        for (auto j : targets[i]) {
            ++c[j];
            tree.insert({c[j], j});
        }
