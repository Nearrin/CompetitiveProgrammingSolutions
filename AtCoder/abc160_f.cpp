...
info operator+(const info &a, const info &b) {
    info c;
    c.len = a.len + b.len;
    c.ways = a.ways * b.ways * intm<>::binary_coefficient(c.len, a.len);
    return c;
}
vector<info> down, up;
...
void build_up(int u, int p) {
    ...
    for (auto v : e[u])
        if (v != p)
            c.push_back(v);
    vector<info> prefix, suffix;
    for (auto v : c)
        prefix.push_back(down[v]), suffix.push_back(down[v]);
    lv(i, prefix) {
        if (i)
...
int main() {
    ...
    down.resize(n + 1), up.resize(n + 1);
    build_down(1, 0);
    build_up(1, 0);
    lp(i, 1, n) {
        // ou(i, down[i].len);
        --down[i].len;
        auto tmp = down[i] + up[i];
        ou(tmp.ways);
    }
    return 0;
}
