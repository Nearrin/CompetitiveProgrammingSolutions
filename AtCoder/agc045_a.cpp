...
using vvi = vector<vi>;
namespace basis {
template <int N> struct basis_z2 {
    basis_z2() {}
    basis_z2(vector<bitset<N>> a) {
    ...
    bitset<N> elimate(bitset<N> a) {
        for (const auto &v : b) {
            int t = int(v._Find_first());
            if (a[t])
                a ^= v;
        }
        return a;
    }
    void add_vector(bitset<N> a) {
        if ((a = elimate(a)).any())
            b.push_back(a);
    }
    vector<bitset<N>> b;
