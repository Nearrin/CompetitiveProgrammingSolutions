...
void check(str &a, str &b, str &c) {
    ...
    auto can_bc = can(b, c);
    lp(sb, 0, a.size()) {
        if (sb < a.size() && !can_ab[sb])
            continue;
        if (max(a.size(), b.size() + sb) >= ans)
            break;
        lp(sc, sb, max(a.size(), b.size() + sb)) {
            if (sc < a.size() && !can_ac[sc])
                continue;
            if (sc - sb < b.size() && !can_bc[sc - sb])
                continue;
            tmin(ans, (int)max({a.size(), b.size() + sb, c.size() + sc}));
        }
    }
}
int main() {
