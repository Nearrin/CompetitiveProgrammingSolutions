...
int main() {
    ...
    vi reach(a.size());
    int n = a.size();
    vector<statistic::statistic> tmp(n);
    segment_tree::segment_tree<statistic::statistic> seg(tmp);
    rp(i, n - 1, 0) {
        seg.modify_single(i, statistic::statistic(i));
        int r = n - 1;
        if (a[r].first >= a[i].first + a[i].second) {
            int l = i;
            while (l + 1 != r) {
                int m = (l + r) / 2;
        ...
        reach[i] = seg.query_segment(i, r).v;
        seg.modify_single(i, statistic::statistic(reach[i]));
    }
    vi dp(n + 1);
    dp[n] = 1;
    rp(i, n - 1, 0) { dp[i] = (dp[i + 1] + dp[reach[i] + 1]) % 998244353; }
    cout << dp[0] << endl;
    return 0;
}
