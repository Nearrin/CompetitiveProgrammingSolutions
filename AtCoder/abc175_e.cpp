    ...
    lv(x, a) lv(y, a[x]) {
        lp(c, 0, 3) {
            if (x == n - 1 && y == m - 1) {
                tmax(ans, f[c][x][y]);
                if (c)
                    tmax(ans, f[c][x][y] + a[x][y]);
            }
            if (x < n - 1) {
                tmax(f[3][x + 1][y], f[c][x][y]);
                if (c)
                    tmax(f[3][x + 1][y], f[c][x][y] + a[x][y]);
            }
            if (y < m - 1) {
                tmax(f[c][x][y + 1], f[c][x][y]);
                if (c)
                    tmax(f[c - 1][x][y + 1], f[c][x][y] + a[x][y]);
            }
        }
    }
    ou(ans);
