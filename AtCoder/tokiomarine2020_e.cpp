...
void dfs(vi &a, vector<vector<int>> &a_or, int i, int current, int count) {
    if (i == a.size()) {
        a_or[count].push_back(current);
        // ou(current);
    } else {
        dfs(a, a_or, i + 1, current, count);
        dfs(a, a_or, i + 1, current | a[i], count + 1);
    }
}
int main() {
    ...
    vi l, r;
    lv(i, b) if (i % 2 == 0) l.push_back(b[i]);
    else r.push_back(b[i]);
    vector<vector<int>> l_or(n / 2 + n % 2 + 1), r_or(n / 2 + 1);
    dfs(l, l_or, 0, 0, 0);
    dfs(r, r_or, 0, 0, 0);
    vi l_sum(1 << 18);
    ll ans = 0;
    lv(i, l_or) lv(j, l_or[i]) {
        if (i > k)
            continue;
        ans += l_or[i][j] == t;
        if (l_or[i][j] == t) {
    ...
    rp(r_cnt, k - 1, 1) {
        int l_cnt = k - r_cnt;
        auto dp2 = vect<int>(0, 1 << 18, 19);
        if (l_cnt <= n / 2 + n % 2)
            for (auto v : l_or[l_cnt])
                ++l_sum[v];
        if (r_cnt <= n / 2)
            for (auto v : r_or[r_cnt]) {
                ans += l_sum[t - v];
            }
    }
    ou(ans);
