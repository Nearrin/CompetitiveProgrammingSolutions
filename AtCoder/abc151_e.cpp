...
int main() {
    ...
    sort(a);
    vi cho(n);
    using namespace numb_theo;
    lp(i, k - 1, n - 1) {
        if (i == k - 1) {
            cho[i] = 1;
        } else {
            cho[i] = mult(cho[i - 1], i);
            cho[i] = divi(cho[i], i - k + 1);
        }
    }
    lv(i, a) {
        ll t1 = mult(cho[i], a[i]);
        ll t2 = mult(cho[n - i - 1], a[i]);
        ans = addi(ans, subs(t1, t2));
    }
    cout << ans << endl;
    return 0;
}