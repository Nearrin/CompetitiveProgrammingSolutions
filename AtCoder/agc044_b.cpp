...
int main() {
    ...
    in(n);
    auto d = vect<int>({}, n, n);
    auto a = vect<int>({1}, n, n);
    lp(i, 0, n - 1) lp(j, 0, n - 1) {
        d[i][j] = min({i, n - i - 1, j, n - j - 1});
    }
    int ans = 0;
    lp(i, 1, n * n) {
        int p;
        ...
        while (!qu.empty()) {
            ...
            if (xx && d[xx][yy] + a[xx][yy] < d[xx - 1][yy]) {
                d[xx - 1][yy] = d[xx][yy] + a[xx][yy];
                qu.push({xx - 1, yy});
            }
            if (xx < n - 1 && d[xx][yy] + a[xx][yy] < d[xx + 1][yy]) {
                d[xx + 1][yy] = d[xx][yy] + a[xx][yy];
                qu.push({xx + 1, yy});
            }
            if (yy && d[xx][yy] + a[xx][yy] < d[xx][yy - 1]) {
                d[xx][yy - 1] = d[xx][yy] + a[xx][yy];
                qu.push({xx, yy - 1});
            }
            if (yy < n - 1 && d[xx][yy] + a[xx][yy] < d[xx][yy + 1]) {
                d[xx][yy + 1] = d[xx][yy] + a[xx][yy];
                qu.push({xx, yy + 1});
            }
        }
