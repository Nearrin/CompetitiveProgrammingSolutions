...
int main() {
    ...
    int n, m;
    in(n, m);
    vvi a(n, vi(3)), b(m, vi(3));
    in(a, b);
    a.push_back({-inf(), inf(), -inf()});
    a.push_back({-inf(), inf(), inf()});
    b.push_back({-inf(), -inf(), inf()});
    b.push_back({inf(), -inf(), inf()});
    vi x, y;
    for (auto v : a)
        x.push_back(v[0]), x.push_back(v[1]), y.push_back(v[2]);
    for (auto v : b)
        x.push_back(v[0]), y.push_back(v[1]), y.push_back(v[2]);
    uniq(x), uniq(y);
    queue<pii> qu;
    auto vis = vect<int>({}, x.size() - 1, y.size() - 1);
    qu.push({upper_bound(rang(x), 0) - x.begin() - 1,
             upper_bound(rang(y), 0) - y.begin() - 1});
    vis[upper_bound(rang(x), 0) - x.begin() - 1]
       [upper_bound(rang(y), 0) - y.begin() - 1] = 1;
    auto canx = vect<int>({}, x.size(), y.size());
    auto cany = vect<int>({}, x.size(), y.size());
    for (auto v : a) {
        int xl = upper_bound(rang(x), v[0]) - x.begin() - 1;
        int xr = upper_bound(rang(x), v[1]) - x.begin() - 1;
        int yy = upper_bound(rang(y), v[2]) - y.begin() - 1;
        canx[xl][yy]++;
        canx[xr][yy]--;
    }
    for (auto v : b) {
        int xx = upper_bound(rang(x), v[0]) - x.begin() - 1;
        int yl = upper_bound(rang(y), v[1]) - y.begin() - 1;
        int yr = upper_bound(rang(y), v[2]) - y.begin() - 1;
        cany[xx][yl]++;
        cany[xx][yr]--;
    }
    lp(i, 0, x.size() - 1) {
        lp(j, 1, y.size() - 1) cany[i][j] += cany[i][j - 1];
    }
    lp(j, 0, y.size() - 1) {
        lp(i, 1, x.size() - 1) canx[i][j] += canx[i - 1][j];
    }
    while (qu.size()) {
        auto tmp = qu.front();
        qu.pop();
        int i = tmp.first, j = tmp.second;
        if (i && !vis[i - 1][j] && !cany[i][j]) {
            vis[i - 1][j] = 1;
            qu.push({i - 1, j});
        }
        if (i != x.size() - 2 && !vis[i + 1][j] && !cany[i + 1][j]) {
            vis[i + 1][j] = 1;
            qu.push({i + 1, j});
        }
        if (j && !vis[i][j - 1] && !canx[i][j]) {
            vis[i][j - 1] = 1;
            qu.push({i, j - 1});
        }
        if (j != y.size() - 2 && !vis[i][j + 1] && !canx[i][j + 1]) {
            vis[i][j + 1] = 1;
            qu.push({i, j + 1});
        }
    }
    ll ans = 0;
    lp(i, 0, x.size() - 2) lp(j, 0, y.size() - 2) if (vis[i][j]) {
        if (!i || !j || i == x.size() - 2 || j == y.size() - 2) {
            ans = -1;
            goto ok;
        }
