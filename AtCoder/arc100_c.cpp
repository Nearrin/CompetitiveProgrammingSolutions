...
int main() {
    ...
    vi a(1 << n);
    in(a);
    auto update = [](pair<pii, pii> &a, const pii &b) {
        if (b.second == a.first.second || b.second == a.second.second)
            return;
        if (b.first <= a.second.first)
            return;
    ...
    vector<pair<pii, pii>> f(1 << n);
    lp(i, 0, (1 << n) - 1) {
        f[i] = {{a[i], i}, {-inf(), -1}};
        int j = i;
        while (j) {
            update(f[i], f[i - (j & -j)].first);
            update(f[i], f[i - (j & -j)].second);
            j -= j & -j;
        }
    }
    int ans = 0;
    lp(i, 1, (1 << n) - 1) {
        tmax(ans, f[i].first.first + f[i].second.first);
        ou(ans);
    }
    return 0;
}
