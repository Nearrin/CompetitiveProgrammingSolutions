...
struct zero_one_bfs {
    ...
    vector<int> distance;
    zero_one_bfs(int _n)
        : n(_n), inf(numeric_limits<int>::max()), to(n), cost(n),
          distance(n, inf) {}
...
int main() {
    ...
    vector<str> a(n);
    in(a);
    shortest_path::zero_one_bfs f(n * m);
    lp(x, 0, n - 1) lp(y, 0, m - 1) {
        lp(dx, -2, 2) lp(dy, -2, 2) {
            ...
            if (a[nx][ny] == '#')
                continue;
            f.add_edge(id(x, y), id(nx, ny), 1);
            if (abs(dx) + abs(dy) == 1) {
                f.add_edge(id(x, y), id(nx, ny), 0);
            }
        }
    }
    f.distance[id(sx, sy)] = 0;
    f.run();
    if (f.distance[id(tx, ty)] == f.inf) {
        f.distance[id(tx, ty)] = -1;
    }
    ou(f.distance[id(tx, ty)]);
    return 0;
}
