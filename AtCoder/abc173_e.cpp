...
int main() {
    ...
    vi a(n);
    in(a);
    sort(a, [](int x, int y) { return abs(x) > abs(y); });
    vi u, v;
    lp(i, 0, k - 1) u.push_back(a[i]);
    lp(i, k, n - 1) v.push_back(a[i]);
    int si = 1;
    for (auto v : u)
        si *= sign(v);
    if (si == 0) {
        ...
        if (hasne && max(u) > 0) {
            ...
            while (u[tmp] < 0)
                --tmp;
            int tout2 = u[tmp];
            if (tout == inf() || ll(abs(tin2)) * ll(abs(tout)) >=
                                     ll(abs(tin)) * ll(abs(tout2))) {
                tin = tin2;
                tout = tout2;
            }
        }
        if (tout != inf()) {
            ...
            if (hasz)
                ...
                if (v.size() == 0) {
                    ...
                    u.clear();
                    lp(i, 1, k) u.push_back(a[a.size() - i]);
                    int ans = 1;
                    for (auto vv : u) {
                        int p = 1e9 + 7;
                        vv = (vv % p + p) % p;
