...
namespace mobius_function {
vector<int> mobius_function(int n) {
    vector<int> p, not_prime(n + 1), u(n + 1);
    not_prime[1] = 1, u[1] = 1;
    for (int i = 2; i <= n; ++i) {
        not_prime[i] || (p.push_back(i), u[i] = -1);
...
int main() {
    ...
    in(n, k);
    inte ans = 0;
    m = mobius_function::mobius_function(k);
    p.resize(k + 1);
    lp(i, 1, k) p[i] = inte(i) ^ n;
    lp(i, 1, k) m[i] += m[i - 1];
    lp(i, 1, k) { ans += solve(n, k / i) * i; }
    ou(ans);
    return 0;
}
