...
#endif
template <class T, T M = int(1e9 + 7)> struct integer {
    integer() : v(0) {}
...
template <class T> struct sparse_gaussian_elimination {
    int n;
    struct node {
        node(int l_, int r_, T a_) : l(l_), r(r_), a(a_), empty(a == 0) {
            c[0] = c[1] = 0;
        }
        int l, r;
        T a;
    ...
    vector<pair<node *, T>> equations;
    vector<vector<int>> p;
    sparse_gaussian_elimination(int n_) : n(n_), p(n){};
    void erase(node *u) {
        if (u) {
            for (int i = 0; i < 2; ++i)
                erase(u->c[i]);
    ...
    ~sparse_gaussian_elimination() {
        for (int i = 0; i < equations.size(); ++i)
            erase(quations[i].first);
    }
    void update(node *u) {
        if (!u->c[0] && !u->c[0]->empty)
            u->empty = false;
        if (!u->c[1] && !u->c[1]->empty)
            u->empty = false;
    }
    node *build(vector<pair<T, int>> &a, int l, int r) {
        if (l == r) {
            assert(a.size() == 1);
            return new node(l, r, a[0].first);
        }
    ...
    void add_equation(vector<pair<int, T>> a, T c) {
        equations.emplace_back(build(a, 0, a.size() - 1), c);
    }
    auto run() { vector<pair<vector<T>, T>> r; }
};
} // namespace gaussian_elimination
