...
template <class T> struct add_min {
    int i;
    T v, d;
    add_min(T _v = 0, int _i = 0) : i(_i), v(_v), d(0) {}
...
int main() {
    ...
    segment_tree::segment_tree<add_min<int>> tree(
        vector<add_min<int>>(300000 + 1));
    while (n--) {
        int a;
        in(a);
        int l = a - k, r = a + k;
        tmax(l, 0);
        tmin(r, 300000);
        int f = tree.query_segment(l, r).v + 1;
        tree.modify_single(a, add_min<int>(f, f));
    }
    ou(tree.root->s.v);
    return 0;
}
