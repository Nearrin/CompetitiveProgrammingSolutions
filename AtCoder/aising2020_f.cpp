...
auto build_function(int parity) {
    vector<integer> f{1};
    lp(i, 1, 5) {
        vector<integer> g;
        lp(j, 0, 3 * i) {
            int n = j * 2;
            lp(k, i, 5) n ^= barr(parity, k);
            g.push_back(0);
            lp(s1, 0, n)
                lp(s2, 0, n) if (s1 < s2 && s1 + s2 <= n &&
                                 (s1 + s2) % 2 == barr(parity, i - 1)) {
                g.back() += (s2 - s1) * integer::lagrange_interpolation(
                                            f, (n - s2 - s1) / 2);
            }
        }
        f = g;
...
int main() {
    ...
    lp(i, 0, (1 << 6) - 1) { f.push_back(build_function(i)); }
    many {
        int n;
        in(n);
        integer ans;
        lp(i, 0, (1 << 6) - 1) {
            if (barr(i, 5) == n % 2)
                ans += integer::lagrange_interpolation(f[i], n / 2);
        }
        ou(ans);
    }
    return 0;
