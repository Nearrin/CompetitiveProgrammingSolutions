...
template <class T> struct vector_2d {
    ...
    auto &operator+=(const vector_2d<T> &a) {
        x += a.x, y += a.y;
        return *this;
    }
    auto &operator-=(const vector_2d<T> &a) {
...
template <class T> double euclidean_norm(const vector_2d<T> &a) {
...
int main() {
    ...
    for (auto p : b)
        a.push_back(p);
    double ans = -1;
    lv(i, a) lv(j, a) if (i <= j && j - i + 1 <= n) {
        auto t = vector_2d<ll>();
        lp(k, i, j) t += a[k];
        // ou(i, j, t);
        tmax(ans, euclidean_norm(t));
    }
    ou(ans);
    return 0;
}
