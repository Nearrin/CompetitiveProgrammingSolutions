...
using inte = modular_arithmetic::integer<>;
inte pw[200010], inv9;
struct set_sum {
    int l;
    inte s, d;
    set_sum(inte _s = 1, int _l = 1) : l(_l), s(_s), d(0) {}
    inte modification() { return d; }
    void modify() {
        if (d.v)
            s = d * (pw[l] - 1) * inv9, d = 0;
    }
    void reverse() {}
    static set_sum merge(const set_sum &a, const set_sum &b) {
        return set_sum(a.s * pw[b.l] + b.s, a.l + b.l);
    }
    static void push(set_sum &a, inte &d) {
        if (d.v)
            a.d = d;
    }
};
int main() {
    ...
    cin.tie(0);
    pw[0] = 1;
    lp(i, 1, 200010 - 1) pw[i] = pw[i - 1] * 10;
    inv9 = inte(1) / 9;
    int n, q;
    inte ans;
    in(n, q);
    lp(i, 1, n) { ans = ans * 10 + 1; }
    vector<set_sum> tmp(n);
    segment_tree::segment_tree<set_sum> tree(tmp);
    while (q--) {
        int l, r, d;
        in(l, r, d);
        ans -= tree.query_segment(l - 1, r - 1).s * pw[n - r];
        ans += d * (pw[r - l + 1] - 1) * inv9 * pw[n - r];
        inte dd = d;
        tree.modify_segment(l - 1, r - 1, dd);
        ou(ans);
    }
    return 0;
}
