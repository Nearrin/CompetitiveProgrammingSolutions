...
ll run(ll x, ll y, ll z) {
    ll ans = 0;
    lp(i, 0, 62) {
        if ((barr(x, i) ^ barr(y, i)) != barr(z, i)) {
            return inf<ll>();
        }
        elif (barr(x, i) == 1 && barr(y, i) == 0) { continue; }
        elif (barr(x, i) == 0 && barr(y, i) == 1) {
            if (x > (1ll << i) && rint(0, 1)) {
                x -= (1ll << i);
                y += (1ll << i);
                ans += (1ll << i);
        ...
        elif (barr(x, i) == 1 && barr(y, i) == 1) {
            if ((barr(x, i + 1) ^ barr(y, i + 1)) != barr(z, i + 1)) {
                x -= (1ll << i);
                y += (1ll << i);
                ans += (1ll << i);
            }
        }
        elif (barr(x, i) == 0 && barr(y, i) == 0) {
            if ((barr(x, i + 1) ^ barr(y, i + 1)) != barr(z, i + 1)) {
                if (x < (1ll << i)) {
                    return inf<ll>();
                }
...
int main() {
    ...
    ll x = a[0], y = a[1], z = 0;
    lv(i, a) if (i > 1) z ^= a[i];
    auto fix = [&](ll t) { return t >= a[0] ? -1 : t; };
    ll ans = inf<ll>();
    lp(i, 1, 1000000) tmin(ans, run(x, y, z));
    ou(fix(ans));
    return 0;
}
