...
template <class T = int, T M = 998244353> struct integer {
    ...
    static auto lagrange_interpolation(const vector<T> &y, T x) {
        return lagrange_interpolation(y, integer<T, M>(x));
    }
    static auto lagrange_interpolation(const vector<T> &y, integer<T, M> x) {
        vector<integer<T, M>> t;
        for (auto v : y)
            t.push_back(integer<T, M>(v));
        return lagrange_interpolation(t, x);
    }
    static auto lagrange_interpolation(const vector<integer<T, M>> &y, T x) {
        return lagrange_interpolation(y, integer<T, M>(x));
    }
...
int main() {
    ...
    int n, m;
    in(n, m);
    using integer = modular_arithmetic::integer<int, int(1e9 + 7)>;
    integer a = integer::binary_coefficient(m, n) * integer::factorial(n) *
                integer::binary_coefficient(m, n) * integer::factorial(n);
    lp(i, 1, n) {
        if (i & 1) {
            a -= integer::binary_coefficient(n, i) *
                 integer::binary_coefficient(m, i) * integer::factorial(i) *
                 integer::binary_coefficient(m - i, n - i) *
