...
template <class T> T extended_euclidean_algorithm(T a, T b, T &x, T &y) {
    if (!b) {
        x = 1, y = 0;
        return a;
    } else {
...
pll f(pll a) {
    ll g = __gcd(abs(a.first), abs(a.second));
    if (g)
...
int main() {
    ...
    for (auto v : a)
        b[f(v)].push_back(v);
    using intm = modular_arithmetic::integer<int, int(1e9 + 7)>;
    intm ans = 1, ans2;
    set<pll> vis;
    for (auto tmp : b) {
        if (vis.count(tmp.first))
            continue;
        if (tmp.first == pll(0, 0)) {
            ans2 = (int)tmp.second.size();
        } else {
            auto d2 = tmp.first;
            swap(d2.first, d2.second);
            d2.second *= -1;
            d2 = f(d2);
            // ou(tmp.first, d2, ans);
            if (!b.count(d2)) {
                ans *= intm(1) << (int)tmp.second.size();
            } else {
                vis.insert(d2);
                ans *= (intm(1) << (int)tmp.second.size()) +
                       (intm(1) << (int)b[d2].size()) - 1;
            }
        }
    }
    ou(ans + ans2 - 1);
