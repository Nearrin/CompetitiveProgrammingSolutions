...
vi solve(vi a) {
    ...
    for (int current = 0;; ++current) {
        int sorted = 1;
        lv(i, a) if (i && a[i] < a[i - 1]) sorted = 0;
        if (sorted)
            break;
        if (must != -1 && must % 2 == current % 2) {
            o.push_back(must + 1);
            swap(a[must], a[must + 1]);
            must = -1;
        } else {
            int now = -1;
            lv(i, a) if (i % 2 == current % 2 && i + 1 < a.size()) {
                if (a[i] > a[i + 1]) {
                    now = i;
                    break;
                }
            ...
            if (now != -1) {
                ...
                now = rint(0, (int)a.size() - 2);
                while (now % 2 != current % 2)
                    now = rint(0, (int)a.size() - 2);
                must = now;
                o.push_back(now + 1);
                swap(a[now], a[now + 1]);
