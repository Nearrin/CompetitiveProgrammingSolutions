...
int main() {
    ...
    vpii a(n);
    cin >> a;
    sort(a, [](const pii &x, const pii &y) {
        return 1ll * y.first * (x.second + 1) < 1ll * x.first * (y.second + 1);
    });
    vpii a1, a0;
    lv(i, a) if (a[i].first) a1.push_back(a[i]);
    ...
    lv(i, a1) {
        rp(j, 38, 0) {
            if (dp[j] == inf<ll>())
                continue;
            ll tmp = (a1[i].first + 1) * (dp[j] + 1) + a1[i].second;
            if (tmp > t)
                tmp = inf<ll>();
            tmin(dp[j + 1], tmp);
        }
    ...
    lp(cnt1, 0, 39) {
        ...
        int ans = cnt1;
        lv(i, a0) {
            cur = (cur + 1) * (a0[i].first + 1) + a0[i].second;
            if (cur > t)
                break;
            ++ans;
        }
