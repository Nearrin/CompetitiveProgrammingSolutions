...
template <class T> struct add_min {
    ...
    T v, d;
    add_min() : d(inf()) {}
    add_min(T _v, int _i) : i(_i), v(_v), d(inf()) {}
    ...
    static void push(add_min &a, T &d) { tmin(a.d, d); }
};
...
int main() {
    ...
    int n, q;
    in(n, q);
    vector<statistic::add_min<int>> tmp;
    lp(i, 0, n - 1) { tmp.push_back(statistic::add_min<int>(n - 1, i)); }
    segment_tree::segment_tree<statistic::add_min<int>> row(tmp), col(tmp);
    ll ans = ll(n - 2) * (n - 2);
    while (q--) {
        int op, x;
        in(op, x);
        --x;
        if (op == 1) {
            int p = col.query_single(x).v;
            // ou(p);
            ans -= p - 1;
            row.modify_segment(0, p, x);
            // ou(0, p, x);
        } else {
            int p = row.query_single(x).v;
            // ou(p);
            ans -= p - 1;
            col.modify_segment(0, p, x);
            // ou(0, p, x);
        }
        // ou(col.query_single(3).v);
    }
    ou(ans);
    return 0;
}
