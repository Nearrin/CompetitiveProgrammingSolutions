...
int main() {
    ...
    in(p);
    vi d;
    multiset<pair<pii, int>> by_x, by_y;
    ...
    lv(i, p) {
        ...
        tmp.clear();
        j = 0;
        for (auto k = by_x.begin(); k != by_x.end() && j != 2; ++k, ++j) {
            tmp.push_back(k->second);
        }
        j = 0;
        for (auto k = by_x.rbegin(); k != by_x.rend() && j != 2; ++k, ++j) {
            tmp.push_back(k->second);
        }
        j = 0;
        for (auto k = by_y.begin(); k != by_y.end() && j != 2; ++k, ++j) {
            tmp.push_back(k->second);
        }
        j = 0;
        for (auto k = by_y.rbegin(); k != by_y.rend() && j != 2; ++k, ++j) {
            tmp.push_back(k->second);
        }
        uniq(tmp);
        for (auto k : tmp) {
            // ou(i, k, distance(p[i], p[k]));
            d.push_back(distance(p[i], p[k]));
        }
        by_x.insert({p[i], i});
