...
int main() {
    ...
    auto check = [&](int x) {
        vector<tuple<int, int, int>> segments(
            1, tuple<int, int, int>{0, a[0] - 1, 0});
        lp(i, 1, a.size() - 1) {
            if (a[i] - 1 > get<1>(segments.back())) {
                segments.push_back(tuple<int, int, int>{
                    get<1>(segments.back()) + 1, a[i] - 1, 0});
            } else {
                while (get<0>(segments.back()) > a[i] - 1) {
                    segments.pop_back();
                }
                get<1>(segments.back()) = a[i] - 1;
                while (segments.size() && get<2>(segments.back()) == x - 1)
                    segments.pop_back();
                if (segments.empty())
                    return false;
                if (get<0>(segments.back()) != get<1>(segments.back())) {
                    auto t = segments.back();
                    --get<1>(segments.back());
                    get<0>(t) = get<1>(t);
                    segments.push_back(t);
                }
                ++get<2>(segments.back());
                if (get<1>(segments.back()) != a[i] - 1)
                    segments.push_back(tuple<int, int, int>{
                        get<1>(segments.back()) + 1, a[i] - 1, 0});
            }
        }
