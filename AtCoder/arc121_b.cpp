...
ll check(const vector<pair<ll, char>> &r, const vector<pair<ll, char>> &g) {
    vector<pair<ll, char>> rg;
    for (auto d : r)
        rg.push_back(d);
    for (auto d : g)
        rg.push_back(d);
    sort(rg);
    ll ans = inf<ll>();
    lv(i, rg) if (i && rg[i - 1].second != rg[i].second)
        tmin(ans, rg[i].first - rg[i - 1].first);
    return ans;
}
ll solve1(const vector<pair<ll, char>> &dogs) {
    vector<pair<ll, char>> r, g, b;
    for (auto [a, c] : dogs) {
        if (c == 'R')
            r.push_back({a, c});
        elif (c == 'G') g.push_back({a, c});
        else b.push_back({a, c});
    }
    if (r.size() % 2 == 0)
        swap(r, g);
    if (g.size() % 2 == 0)
        swap(g, b);
    if (g.size() % 2 == 0 && b.size() % 2 == 0) {
        return 0;
    } else {
        return min(check(r, g), check(r, b) + check(g, b));
    }
}
ll solve2(const vector<pair<ll, char>> &dogs) {
    vi a(dogs.size());
    lv(i, a) a[i] = i;
    ll ans = inf<ll>();
    do {
        ...
        lv(i, a) if (i % 2 == 0) {
            int p = a[i], q = a[i + 1];
            if (dogs[p].second != dogs[q].second) {
                cur += abs(dogs[p].first - dogs[q].first);
            }
        }
        tmin(ans, cur);
...
int main() {
    ...
    int n;
    in(n);
    vector<pair<ll, char>> dogs(2 * n);
    in(dogs);
    ou(solve1(dogs));
    return 0;
    while (1) {
        int n = rint(1, 2);
        vector<pair<ll, char>> dogs;
        lp(i, 1, 2 * n) { dogs.emplace_back(rint(1, 10), "RGB"[rint(0, 2)]); }
        ou(dogs);
        auto ans1 = solve1(dogs), ans2 = solve2(dogs);
        ou(ans1, ans2);
        if (ans1 != ans2)
            break;
    }
