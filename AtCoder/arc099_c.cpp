...
int main() {
    fast, prec(12);
    using graph = union_find_set::union_find_set<union_find_set::bipartite>;
    int n, m;
    in(n, m);
    auto e = vect<bool>({}, n, n);
    lp(i, 1, m) {
        int u = in<int>() - 1, v = in<int>() - 1;
        e[u][v] = e[v][u] = 1;
    }
    graph g(n);
    lp(u, 0, n - 1) lp(v, u + 1, n - 1) {
        if (!e[u][v]) {
            if (g.find(u) != g.find(v)) {
                g.link(u, v, union_find_set::bipartite::path(1));
            } else {
                if (g.query_path(u, v).s == 0) {
                    ou(-1);
                    return 0;
                }
            }
    ...
    vpii a;
    lp(u, 0, n - 1) {
        if (g.query_single(u).c == -1) {
            g.modify_single(u, 0);
            auto t = g.query_tree(u);
            a.emplace_back(t.s[0], t.s[1]);
        }
    }
    bitset<701> f;
    f[0] = 1;
    for (auto t : a)
        f = (f << t.first) | (f << t.second);
    int ans = inf();
    lp(i, 0, n) if (f[i]) {
        tmin(ans, i * (i - 1) / 2 + (n - i) * (n - i - 1) / 2);
    }
    ou(ans);
    return 0;
}
